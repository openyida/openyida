...
const SKILL_COVERAGE = {
  ...
  'yida-design': { level: 'offline-unit', tests: ['skill metadata and packaging validation', 'routing eval scenarios', 'tests/create-form.test.js', 'tests/skill-contracts.test.js'], reason: 'visual direction and theme token guidance produce design.md before page implementation; PRD is owned by yida-prd, so validate routing, packaging, skill contracts and form theme payloads rather than mutating Yida resources or tenant-wide theme config' },
