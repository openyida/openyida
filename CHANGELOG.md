...
- **补齐原生表单 Divider 辅助色**：应用主题新增 `--yida-divider-secondary-color`，共享主题模板同步声明并引用 `--color-brand1-2`。深色内容模板将品牌弱背景调整为与深色画布协调的颜色，兼顾原生组件内联绑定和直接消费品牌弱背景的内嵌标题，避免仅修改根级辅助色仍出现亮底浅字。
- **补齐平台弱图标色阶**：基础 token 合约新增 `--color-fill1-6`，公共主题模板默认映射到 `var(--color-text1-3, #878f95)`，34 套主题的 `design.md` 与 `app_theme.css` 同步输出。成员选择、树形展开和选择器末尾按钮等原生图标不再依赖固定 fallback，在暗色主题下跟随当前辅助前景色。
- **主题契约文档和回归测试同步更新**：设计技能、表单技能和主题模板 README 增加 Divider 与弱图标 token 语义说明；主题模板测试覆盖辅助色绑定、`--color-fill1-6` 和公共 CSS 模板，不新增分割线配色的运行时校验或阻断。

### 前台独立页面入口

- **按业务场景补齐前台入口**：面向办事用户的自定义页面可根据服务入口角色与页面场景推导独立入口；保留显式导航选择，不将业务后台一并隐藏。搭建流程要求保存页面导航设置、回读确认并交付前台访问地址，不再因遗漏 `entryMode` 静默跳过前台。
- **发布和查询返回已验证的页面地址**：`publish` 与 `get-form-config` 根据平台保存的页面类型及导航状态返回 `standaloneUrl`、`workbenchUrl` 和 `navigationVerification`。已确认隐藏导航的自定义页面优先返回 `/custom/<页面ID>`，不依赖 URL 参数隐藏导航；状态未知或导航未隐藏时不声称已获得独立入口。
- **区分发布成功与入口验证结果**：发布后的导航回读失败会返回警告，不把已成功保存的页面误报为发布失败，也不将后台地址冒充前台交付。相关命令契约、Skill 与回归测试同步更新。

### 深色表单设计引导

- **分割线按实际承载背景配色**：明确主色、次色和标题色的默认主题绑定，不根据 `light-*`、`dark-*` 名称判断文字颜色。默认继承主题；复合样式需要独立配色时，可通过已有 `colorType: custom` 成组设置 `backgroundColor`、`secondaryColor` 和 `titleColor`，无需用户逐项指定颜色。同页同层级保持一致，并区分整页深色与仅导航深色。
- **实色标题优先考虑浅色文字**：为 `dark-bar` 及左侧、居中、屋顶、六边形标题等样式补充引导，在足够深的主色承载面上优先使用白色或近白色文字；亮紫等高亮底色不适合白字时，优先保持色相并加深局部分割线底色，不修改全局正文色。浅色承载面不强制白字，内嵌标题按自身背景选择文字色，避免沿用默认深色文字或出现浅底浅字。
- **分割线与字段 label 整体设计**：标题、首个字段标签和输入框共同考虑字体、字号、字重、对齐及间距；标签保持可读的中性文字色，不靠降低可读性制造层级。区分 `--pod-form-label-color` 与分割线标题色，避免居中装饰标题带动字段居中及重复堆叠强装饰。
- **补齐主题变量语义与命令契约**：说明 `--color-brand1-2` 的弱品牌背景用途、分割线局部变量的覆盖关系，以及 `--color-fill1-6` 在弱前景图标上的配色注意事项。CLI 已有字段的创建、更新和 patch 用法与 Skill、manifest、完整及精简能力摘要保持一致；分割线配色决策以引导表达，不新增 CLI 参数、校验或阻断行为。

### 升级说明

- 已生成或已上传的历史应用主题不会自动获得模板修复。需要修复已发布应用的暗色 Divider 或弱图标颜色时，基于最新 `design.md` 重新生成并上传应用主题 CSS。
- 配色引导不会自动修改已发布表单或历史自定义颜色。已有页面需按实际承载背景调整主题或分割线配置，保存后查看真实页面；固定自定义颜色不代表已具备自动明暗切换能力。
