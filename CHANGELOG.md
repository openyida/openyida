...
## [2026.9.24-beta.0] - 2026-09-24

### 登录认证

- **支持设备码登录**：`openyida login --device` 新增 OAuth 2.0 Device Authorization Grant（RFC 8628）登录方式，可在沙箱、无头环境和 CI 中输出验证地址与用户码，并在其他设备完成授权后自动保存登录态。
- **支持环境提示与机器可读进度**：`--env-hint` 可向服务端传递目标环境提示；设备登录过程输出 `device_login_state` JSON 事件，事件字段统一使用 camelCase，登录结果增加 `type: "login_result"`，方便 agent 和 CI 稳定解析。

### 稳定性与兼容性

- **完善设备登录轮询边界**：正确处理等待授权、服务端降速、授权拒绝、设备码过期等状态；永久 OAuth 错误会立即停止，临时的 HTTP 400/429 响应可按截止时间继续重试，并由调用方超时共同限制最长等待时间。
- **统一令牌解析与登录文案**：设备登录复用通用令牌 payload 解析逻辑，支持服务端响应包装；用户可见提示接入中英文国际化，并同步 CLI 帮助、README、命令清单与语言包。

## [2026.9.23-beta.1] - 2026-09-23

### 登录认证

- **支持设备码登录**：`openyida login --device` 新增 OAuth 2.0 Device Authorization Grant（RFC 8628）登录方式，可在沙箱、无头环境和 CI 中输出验证地址与用户码，并在其他设备完成授权后自动保存登录态。
- **支持环境提示与机器可读进度**：`--env-hint` 可向服务端传递目标环境提示；设备登录过程同时输出 `device_login_state` JSON 事件，方便 agent、CI 面板等运行环境展示验证地址与轮询状态。

### 稳定性与兼容性

- **完善设备登录轮询边界**：正确处理等待授权、服务端降速、授权拒绝、设备码过期等状态；永久 OAuth 错误会立即停止，临时的 HTTP 400/429 响应可按截止时间继续重试，并由调用方超时共同限制最长等待时间。
- **修复验证地址与命令说明**：服务端返回相对验证地址时会基于业务域名补全；CLI 帮助、README、命令清单和全部语言包同步公开 `--device` 与 `--env-hint` 参数。
