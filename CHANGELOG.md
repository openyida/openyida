...
- 新增独立图片素材技能，按页面判断图片是必需、有帮助还是无需使用；Fast 和 Plan 共用设计槽位与素材清单，已就绪页面可独立继续开发。
- 素材能力检测分别报告联网搜索、图片搜索和图片生成能力；能力未确认时检查实际宿主工具，不把环境默认值当作工具可用的证明。

### Changed

- 图片默认下载后上传宜搭图片附件，直接使用返回的公开链接，无需配置自有 CDN；不额外探测原图或上传后的链接。超过 20 MiB 的外链保留原地址，下载失败跳过并记录缺口。
- 统一素材来源顺序：用户提供、Unsplash / Pexels 搜图、允许时由宿主生成；保留来源、署名和用途，生成图明确标记为示意素材。
- `asset resolve` 支持通用槽位和清单输入，通过 `--design` 对齐页面、槽位和最小尺寸；输出每页及每张图的状态与缺口，必需素材缺失只阻塞对应页面。
- 明确先选图、保存草稿，再在取得真实 appType 后上传的顺序；同步更新 CLI 帮助、能力清单、设计与页面开发技能。

### Fixed

- 根据实际图片内容读取格式与尺寸，避免非图片内容或手填宽高通过检查；补齐重复槽位、设计漏项和尺寸要求校验。
- 重跑保留原始输入、尺寸要求和来源信息，原图内容未变时复用已有附件链接；离线模式不联网、不上传，未落地素材保持草稿。

### 升级说明

- 上传需有效宜搭登录态与目标应用 appType，可传 `--app-type` 或读取当前项目 `config.json.appType`。`--upload-assets` 保留为兼容参数，默认已上传；超过 20 MiB 且没有原始外链的本地图需换小图或补原链接。
- 旧命令 `asset generate` 改用 `asset sources`；旧 `--hero`、`--product`、`--require-hero` 和 `--mirror-external` 参数需迁移为 `--slot` 或 `--input` 清单，槽位必需性由设计或清单声明。
