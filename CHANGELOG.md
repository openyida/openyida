...
- **补齐原生表单 Divider 辅助色**：应用主题新增 `--yida-divider-secondary-color`，所有共享主题模板同步声明。浅色内容界面默认跟随品牌浅底，深色内容界面跟随弱边界色，避免暗底提交页和抽屉表单中章节分隔条过亮。
...
- **主题契约文档和校验同步更新**：设计技能、表单技能和主题模板 README 增加 Divider 与弱图标 token 语义说明；主题模板测试新增暗色 Divider、`--color-fill1-6` 和公共 CSS 模板回归，防止后续主题漏声明。

### 升级说明

- 已生成或已上传的历史应用主题不会自动获得新变量。需要修复已发布应用的暗色 Divider 或弱图标颜色时，基于最新 `design.md` 重新生成并上传应用主题 CSS。
