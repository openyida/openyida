...
- **补齐主题变量语义与命令契约**：说明 `--color-brand1-2` 的弱品牌背景用途、分割线局部变量的覆盖关系，以及 `--color-fill1-6` 在弱前景图标上的配色注意事项。CLI 已有字段的创建、更新和 patch 用法与 Skill、manifest、完整及精简能力摘要保持一致；本轮仅补充引导，不新增 CLI 参数、校验或阻断行为。

### 升级说明

- 本轮分割线改动属于设计引导，不会自动修改已发布表单、主题或历史自定义颜色。已有页面需按实际承载背景调整主题或分割线配置，保存后查看真实页面；固定自定义颜色不代表已具备自动明暗切换能力。
