...
- 新增技能层 Canvas 主题适配脚本，可根据本地主题文件和已上传的主题地址生成带 `CanvasThemeProvider` 的页面，统一为 antd 控件提供主题颜色；支持本地配色预览。
- 新增 `canvas-theme` 示例片段，便于维护既有自定义页面的主题适配。

### Changed

- 自定义页面底色统一使用 `--pod-page-bg-color`，卡片和面板使用 `--pod-card-bg-color`，移除新生成页面、设计模板和规范中重复的 `--oyd-page-background` 来源。
- 统一设计文档、主题 CSS 与 Canvas 开发规范的配色职责：主按钮、链接和选中态跟随应用品牌色，次要控件保持中性，危险等状态保留语义色。

### Fixed

- 改善表单抽屉、批量录入和图表示例中控件配色未跟随应用主题的问题；从页面实际作用域读取颜色，并响应主题变化。

- 修复详情抽屉 iframe 最小高度导致的双滚动和外层卡片露底；标题栏跟随平台头部高度，默认 48px，背景与抽屉统一。

### Migration

- 此次适配不自动修改既有主题文件或已发布页面。旧背景别名需先核对底色再迁移；使用 Provider 的页面需重新生成派生源码，并编译、发布该文件。
- Provider 自动适配颜色角色，尺寸、圆角、字体和图表色组仍需按设计配置；本地预览不代表线上主题已验收。
