...
describe('OpenYida skill contracts', () => {
  ...
  test('unified full app build applies one verified navigation plan', () => {
    ...
    expect(appStages).toContain('读取阶段 2C 的 `page-spec.json` 和真实 `appType/formUuid/fieldId`');
    expect(appStages).toContain('从项目 `scaffolds/canvas.canvas.jsx` 扩展');
  ...
  test('requirement analysis feeds yida-prd and yida-design in parallel', () => {
    ...
    expect(skill).toContain('`page-spec.json` 是两份设计产物的派生交接');
    expect(skill).toContain('项目视觉脚手架由 `yida-design` 准备');
    expect(root).toContain('加载子技能 `yida-app`，详细流程见 `yida-app`');
    ...
    expect(appBuildStages).toContain('为本轮页面派生 `page-spec.json`');
    expect(skill).not.toContain('PRD 中已有 `pageSpecHandoff` 时，优先从 `pageSpecHandoff` 提取');
    expect(skill).not.toContain('页面生成器修复策略');
    expect(pageGeneration).toContain('完整应用在设计对齐阶段读取一次 `prd.md + design.md`');
    expect(pageGeneration).toContain('修复路径');
    expect(pageGeneration).toContain('先补事实源，再重新派生 spec 和项目脚手架');
    expect(pageGeneration).toContain('只有实现偏差才小范围修改源码');
    ...
    expect(outputDesign).toContain('完整应用页面消费派生 `page-spec.json` 和项目脚手架');
    ...
    expect(pageGeneration).toContain('spec 包含业务区块、数据来源、主操作、移动端要求');
    expect(pageGeneration).toContain('当前页面实际使用的布局、层级、材质、圆角、密度、间距、组件和状态摘要');
    ...
    expect(pageGeneration).toContain('`prd.md` 和 `design.md` 是事实源');
    expect(pageGeneration).toContain('`page-spec.json` 和项目脚手架是派生产物');
    expect(pageGeneration).toContain('sourceOfTruth.conflictPolicy = "prd-design-win"');
    expect(pageGeneration).toContain('页面结构和视觉值来自 `page-spec.json` 与项目脚手架');
    expect(pageGeneration).toContain('## 修复路径');
    ...
    expect(pageGeneration).toContain('回写 `design.md`，再生成 `design-runtime.json`、项目脚手架和 `page-spec.json`');
    expect(pageGeneration).toContain('只有实现偏差才小范围修改源码');
    expect(pageGeneration).toContain('近白画布必须用渐变、装饰、素材焦点或内容密度形成层次');
    expect(pageGeneration).toContain('`.oy-page-root` 承载基础底色');
    expect(pageGeneration).toContain('主题关系、token、页面结构、材质、圆角、密度、间距、组件、状态或响应式规则不足或错误');
    expect(pageGeneration).toContain('所有展示型页面都按 `page-spec.visualImplementation` 实现');
    expect(pageGeneration).toContain('`page-spec.visualImplementation` 必须包含当前页的 `rootShell`');
    expect(pageGeneration).toContain('如果 `page-spec.json` 或项目脚手架缺少页面需要的视觉值');
    ...
    expect(styleSelection).toContain('视觉 DNA、布局、组件样式、主题 token 和状态规则来自 design.md');
    expect(styleSelection).toContain('页面实现消费两者的派生产物');
  ...
  test('yida-design centralizes application theme token presets and keeps application theme profiles explicit', () => {
    ...
    expect(canvasStyleGuide).toContain('本文件只说明 Code Canvas 如何实现样式，不产出设计事实');
    expect(canvasStyleGuide).toContain('完整应用优先读取项目 Canvas 脚手架和 `page-spec.json`');
  ...
  test('custom-page-dependent skills keep Code Canvas as creation path', () => {
    ...
    expect(canvas).toContain('完整应用读取 `page-spec.json` 和 `.cache/openyida/<项目名>/scaffolds/canvas.canvas.jsx`');
    ...
    expect(canvasStyleGuide).toContain('完整应用读取 `page-spec.json.visualImplementation`');
    ...
    expect(pageGeneration).toContain('`page-spec.json` 只保存当前页面要执行的视觉摘要，不复制整份 design.md');
    expect(pageGeneration).toContain('| `sourceOfTruth` |');
    expect(pageGeneration).toContain('"conflictPolicy": "prd-design-win"');
    expect(pageGeneration).toContain('完整应用的 Code Canvas 消费 `page-spec.json`、项目 Canvas 脚手架和真实资源 ID');
