...
describe('OpenYida skill contracts', () => {
  ...
  test('requirement analysis feeds yida-prd and yida-design in parallel', () => {
    ...
    expect(appBuildStages).toContain('校验 PRD 页面清单、页面场景、`prdRefs` 与 design.md 的 `designRefs`');
    ...
    expect(step3).toContain('页面区块 / contentBlocks：<逐条列出当前页面真实需要的区块');
  ...
  test('workbench pages avoid low-density giant card templates', () => {
    ...
    expect(pageUiux).toContain('页面丰富度服从 PRD');
    expect(pageUiux).toContain('用户或 PRD 已限定页面/模块时按限定范围设计，不为了数量补 PRD 没有的功能');
    expect(step4).toContain('工作台、首页、门户、看板、展示页和业务入口页默认要拆出足够支撑首屏判断和操作的业务区块');
    expect(step4).toContain('KPI 组只能算 1 个');
    expect(step4).toContain('快捷入口组只能算 1 个');
    expect(qualityGates).toContain('## 1. 区块范围门禁');
    expect(qualityGates).toContain('## 2. 页面结构槽位门禁');
    expect(qualityGates).toContain('## 3. 低密大卡片门禁');
    expect(step3).toContain('`contentBlocks` 数量由页面任务和 PRD 范围决定');
    ...
    expect(step4).toContain('默认要拆出足够支撑首屏判断和操作的业务区块');
    expect(step4).toContain('内容区块：<当前页面真实区块列表 + 目的');
    expect(step4).toContain('KPI 组只能算 1 个，快捷入口组只能算 1 个，列表组只能算 1 个');
    expect(step4).toContain('却额外补“合同看板、商机大屏、团队排行、客户画像”等 PRD 没有的模块');
    expect(step5).toContain('4 个等宽大 KPI 白卡 + 图标快捷卡 + 大空态白卡');
    expect(step5).toContain('满足页面质量门禁中的区块数量要求');
    expect(qualityGates).toContain('`contentBlocks` 必须来自 PRD 页面章节、共享需求简报的 `explicitScope`、真实业务对象或当前单页上下文');
    expect(qualityGates).toContain('KPI 组、快捷入口组、列表组、图表组各只算 1 个区块');
    expect(outputPrd).toContain('区块数量按当前页面任务、用户明确范围和数据来源决定');
    expect(outputPrd).toContain('不能为了凑数增加 PRD 没有的功能');
    ...
    expect(pageGeneration).toContain('按 PRD 原文落地有业务目的的区块');
    expect(pageGeneration).toContain('KPI 子项、快捷入口子项和列表行不计入区块数量');
    expect(pageGeneration).toContain('也不能为了数量新增 PRD 没有的功能');
    ...
    expect(canvasStyleGuide).toContain('页面整体按 PRD 和 design.md 的 `contentBlocks` 落地有业务目的的区块');
