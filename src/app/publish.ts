...
// @ts-ignore
import babelTransform from '../core/babel-transform';
// @ts-ignore
...
// ── 配置读取 ──────────────────────────────────────────
const CONFIG = fs.existsSync(path.resolve(findProjectRoot(), 'config.json'))
  ? JSON.parse(fs.readFileSync(path.resolve(findProjectRoot(), 'config.json'), 'utf-8'))
  : {};
const DEFAULT_BASE_URL: string = CONFIG.defaultBaseUrl || 'https://www.aliwork.com';
const SCHEMA_VERSION: string = 'V5';
const DOMAIN_CODE: string = 'tEXDRG';
const PREFIX: string = '_view';
...
function resolveBaseUrl(loginResult: LoginResult): string {
...
function compileSource(sourcePath: string): CompileResult {
  ...
  if (babelResult.error instanceof Error) {
    ...
    const hintLines = [
      ...
      '💡 常见编译错误原因及解决方案：',
      '',
      '  1. 使用了 ES6+ 高级语法（如 ??、?.、class 字段）',
      '     → 改为 ES2015 兼容语法',
      '',
      '  2. 事件绑定用了 onClick={this.handle} 而非箭头函数',
      '     → 改为 onClick={(e) => { this.handle(e); }}',
      '',
      '  3. style 属性用了 kebab-case（如 background-color）',
      '     → 改为 camelCase（如 backgroundColor）',
      '',
      '  4. 标签未正确闭合（如 &lt;input&gt; 而非 &lt;input /&gt;）',
      '',
      '  5. 尝试使用 import/require 引入三方包',
      '     → 使用 this.utils.loadScript() 动态加载',
      '',
      '详细规范请查看：skills/yida-custom-page/SKILL.md',
      '═══════════════════════════════════════════════════════════════',
    ];
...
function buildSchemaContent(sourceCode: string, compiledCode: string, formUuid: string): string {
...
function sendSaveRequest(
  ...
  return new Promise((resolve, reject) => {
    ...
    const request = requestModule.request(requestOptions, (response: any) => {
      let responseData = '';
    ...
    request.on('error', (requestError: any) => { reject(requestError); });
...
function sendUpdateConfigRequest(
  csrfToken: string,
  cookies: any[],
  baseUrl: string,
  appType: string,
