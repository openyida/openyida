...
async function run(args: string[]): Promise<void> {
  ...
  const targetOperation = currentOperations.find((op: any) => op.operationId === operationId);

  if (!targetOperation) {
    console.error(`❌ 未找到动作 operationId: ${operationId}`);
    console.log('\n当前动作列表:');
    currentOperations.forEach((op: any, index: number) => {
  ...
  const filteredOperations = currentOperations.filter((op: any) => op.operationId !== operationId);
  ...
  filteredOperations.forEach((op: any, index: number) => {
    console.log(`  ${index + 1}. ${op.operationId}: ${op.summary}`);
  });
}
