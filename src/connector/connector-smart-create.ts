...
import { parseCurl, detectAuthType, filterBrowserHeaders } from './curl-parser';
import { generateOperation } from './action-generator';
import { generateConnectorDesc } from './desc-generator';
...
function parseArgs(args: string[]): SmartCreateOptions {
 ...
 * 阶段 1: 解析接口信息
 */
function phase1Parse(options: SmartCreateOptions): { curlData: any; authType: any } {
 ...
 * 阶段 2: 查找匹配的已有连接器
 */
async function phase2Match(curlData: any, authType: any, authRef: any): Promise<any[]> {
  console.log('🔍 阶段 2: 查找匹配的已有连接器\n');

  const { connectors } = await listConnectors({ pageSize: 100 }, authRef);

  const matchingConnectors = connectors.filter((connector: any) => {
  ...
  if (matchingConnectors.length > 0) {
    ...
    const rows = matchingConnectors.map((connector: any, index: number) => {
      let authTypeStr = '无';
      try {
 ...
 * 阶段 3: 生成执行动作配置
 */
function phase3Generate(curlData: any): { operation: any; connectorDesc: string } {
 ...
 * 阶段 4: 保存配置并提供操作建议
 */
function phase4Output(
  options: SmartCreateOptions,
  curlData: any,
  authType: any,
  matchingConnectors: any[],
  operation: any,
  ...
  operation.inputs.forEach((input: any) => {
    if (input.childList && input.childList.length > 0) {
      console.log(`   - ${input.name}: ${input.childList.map((child: any) => child.name).join(', ')}`);
    }
  });
  console.log();
  console.log('═══════════════════════════════════════════════════');
