...
import { generateOutputs, generateExample, generateChildList } from './response-parser';
...
class MarkdownParser extends BaseDocParser {
  ...
  private parseRequestHeaders(): void {
  ...
  private parseRequestBody(): void {
    ...
    if (fields.length > 0 && this.result.requestInfo.body && this.result.requestInfo.body.schema) {
      fields.forEach(field => {
        const propName = field.name || field.fieldName;
        if (propName && this.result.requestInfo.body!.schema!.properties && this.result.requestInfo.body!.schema!.properties[propName]) {
  ...
  private parseFieldType(typeStr: string): { type: string; items?: JsonSchema; description?: string } {
  ...
  private inferSchemaFromExample(example: any): JsonSchema {
    const schema: JsonSchema = { type: 'object', properties: {} };
  ...
  private inferType(value: any, key: string = ''): JsonSchema {
    if (value === null) {
      return { type: 'null' };
    }
  ...
  private parseTable(section: string): any[] {
...
class DocParserFactory {
  static createParser(filePath: string, content: string): BaseDocParser {
 ...
 * 解析 API 文档主函数
 */
function parseAPIDoc(filePath: string): ParseResult {
  try {
 ...
 * 将解析结果转换为连接器操作配置
 */
function convertToOperationConfig(parseResult: ParseResult): Operation {
