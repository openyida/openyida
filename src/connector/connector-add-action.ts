...
function parseArgs(args: string[]): AddActionOptions {
  const options: AddActionOptions = {
    operationsFile: null,
    hostFilter: null,
    connectorId: null,
    confirm: false,
...
async function run(args: string[]): Promise<void> {
  ...
  const mergedOperations = [
    ...existingOperations.filter((op: any) => !newOperations.find((newOp: any) => newOp.operationId === op.operationId)),
    ...newOperations,
  ];
