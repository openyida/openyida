...
function parseArgs(): { appType: string; reportTitle: string; chartsJsonOrFile: string } {
 ...
 *   }
 */
function readReportConfig(chartsJsonOrFile: string): { charts: any[]; filters: any[] } {
  let raw;
  ...
  if (fs.existsSync(chartsJsonOrFile)) {
    ...
    try {
      ...
      console.error('配置解析失败（既不是有效文件路径，也不是有效 JSON）:', (e as Error).message);
      process.exit(1);
    }
  }
...
async function main(): Promise<void> {
  ...
  const authRef = { csrfToken, cookies, baseUrl, cookieData } as any;
  ...
  const reportId = (createResult.content as any).formUuid || createResult.content;
  ...
  if (filters.length > 0) {
    ...
    const rootHeader = componentsTree.children.find((c: any) => c.componentName === 'RootHeader');
    const pageHeader = rootHeader && rootHeader.children && rootHeader.children.find((c: any) => c.componentName === 'YoushuPageHeader');
    const pageHeaderContent = pageHeader && pageHeader.children && pageHeader.children.find((c: any) => c.componentName === 'PageHeaderContent');
    ...
    if (pageHeaderContent) {
      ...
      filterComponents.forEach((compName) => {
        if (!page.componentsMap.some((c: any) => c.componentName === compName)) {
      ...
      const rootContent = componentsTree.children.find((c: any) => c.componentName === 'RootContent');
      builtFilters.forEach((builtFilter, fi) => {
        ...
        targetCharts.forEach((target: any) => {
