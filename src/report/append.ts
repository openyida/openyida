...
async function getReportSchema(baseUrl: string, cookies: any[], appType: string, reportId: string): Promise<any> {
  return httpGet(
    baseUrl,
...
function parseArgs(args: string[]): { appType: string; reportId: string; chartsJsonOrFile: string } {
...
function readChartsDefinition(chartsJsonOrFile: string): any[] {
  let charts;
  if (fs.existsSync(chartsJsonOrFile)) {
    try {
      charts = JSON.parse(fs.readFileSync(chartsJsonOrFile, 'utf-8'));
    } catch (e) {
      console.error('读取图表定义文件失败:', (e as Error).message);
      process.exit(1);
    }
  } else {
    try {
      ...
      console.error('图表定义解析失败:', (e as Error).message);
      process.exit(1);
    }
  }
...
function buildChartNode(chart: any, cubeTenantId?: string): {
  fieldId: string;
  nodeId: string;
  ...
  const userConfig = buildUserConfigWithFields(chartType);
...
async function main(args: string[]): Promise<void> {
  ...
  const rootContent = page.componentsTree[0].children.find(
    (c: any) => c.componentName === 'RootContent'
  );
  ...
  // Step 4: 计算新图表 Y 起始位置
  let maxBottom = 0;
  rootContent.props.layout.forEach((l: any) => {
