...
- 非目标：复制整套钉钉开放平台文档、让页面直连钉钉域名、用 HTTP 连接器模拟事件接收器
...
### 2.1 服务端 API 与事件订阅必须分开

- 服务端 REST API 可以映射为 HTTP Connector Action。
- 客户端 JSAPI 不能用服务端连接器代替。
- 事件订阅是钉钉向业务方推送事件，不是普通出站 HTTP Action。
- Stream 模式需要 WebSocket 客户端；HTTP 推送需要公网接收服务及验签解密。普通宜搭 HTTP 连接器不能充当事件接收器。参考：[事件订阅概述](https://open.dingtalk.com/document/development/event-subscription-overview)。

新技能遇到事件订阅页面时必须停止 Action 生成并返回：

```json
{
  "integrationType": "event_subscription",
  "connectorActionSupported": false,
  "nextStep": "prepare_stream_or_http_event_receiver"
}
```

### 2.2 官方文档是唯一接口事实源

技能不在仓库内维护完整 API 清单。每次根据用户提供的官方 URL 或明确接口名称读取当前 `open.dingtalk.com` 文档，并记录来源 URL、文档更新时间、应用类型、权限和接口版本。
...
`SKILL.md` 只保留路由、四步工作流和完成条件；字段映射、鉴权和事件边界放入引用文件，避免入口膨胀。
...
{
  "provider": "dingtalk",
  "sourceUrl": "https://open.dingtalk.com/...",
...
契约必须保留嵌套 Object、Array、Map 和必填信息。`appKey`、`appSecret`、access token、Cookie 不得进入契约或 Action 文件。
...
当前 `connector create` 虽然要求传入 `--app-key` 和 `--app-secret`，但这两个值只被用于参数完整性校验；连接器创建载荷只包含 `securitySchemes`，不会保存这两个值。连接器创建完成后仍需单独执行 `create-connection`。
...
现有命令通过 CLI 参数接收 `--app-key`、`--app-secret`，可能出现在 shell history 或短暂出现在进程参数中。新技能不能把真实 AK/SK 写入任务文档、Action JSON、日志或页面源码。

后续建议为 `create-connection` 增加安全输入方式，例如：

- 交互式隐藏输入；
- 受控环境变量引用；
- 凭证文件描述符或系统钥匙串引用。

这属于凭证输入安全优化，不阻塞新技能第一版；第一版必须至少保证生成物、结构化输出和测试日志不回显凭证。
...
| 文档属于事件订阅 | 返回接收器能力缺口，不生成出站 Action |
...
- 增加服务端 API、事件订阅、Canvas、旧 JSX 的路由回归。
...
| 订阅通讯录变更事件 | 识别为入站事件，不生成普通 Connector Action |
...
- 官方文档中的应用类型、权限、参数位置、嵌套类型和错误码进入标准化契约。
...
- 事件订阅不会被误实现为普通 HTTP Connector Action。
