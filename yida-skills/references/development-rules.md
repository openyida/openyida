...
**F1 技能加载唯一入口**：执行任何子技能前，支持 `use_skill` 的宿主必须调用 `use_skill("<技能名>", "<本阶段目的>")` 加载对应技能；不要用 `Read` / `read_file` / `cat` 读取 `SKILL.md` 路径。`skills-index.json` 是给能读取索引的工具快速找到技能用的；不能读取它的工具直接忽略。完全没有 `use_skill` / `search_skills` 的本地工具，才允许按根技能路由表和技能包相对路径读取当前要用的技能文档；完整应用按依赖加载对应子技能。
...
- 不直接给页面选择内部编译器。`.canvas.jsx` / `.canvas.tsx` 的检查结果必须是 `mode: "canvas"`；编译器不匹配错误先修正文件后缀或命令，不按普通 JSX 规则改 Canvas 源码；
...
| # | 规则 | 操作细节 |
|---|------|---------|
| 1 | 按依赖加载子技能 | 按意图选定 1 个主技能；完整应用按依赖加载对应子技能，仅在已加载技能明确要求时才读对应 `references/`。 |
| 2 | 优先复用缓存 | `appType`/`formUuid`/`fieldId`/`reportId` 先从 `.cache/<项目名>-schema.json` 读，缺失或不确定再 `openyida get-schema`。 |
| 3 | 页面规格优先 | 完整应用页面规格关系由 `yida-app` 引用 `yida-design` 产物处理；单页视觉设计交给 `yida-design`，实现交给页面技能。 |
...
| 7 | 配置分两处存 | 详见 [配置信息分两处存储](#配置信息分两处存储) 与 [PRD/design 归属](#prddesign-归属)。 |
...
`.cache/openyida/<项目名>/requirement-brief.json` 由 `yida-requirement-analysis` 生成，供 `yida-prd` 和 `yida-design` 同时读取。`prd/<项目名>/prd.md` 归 `yida-prd`，`prd/<项目名>/design.md` 归 `yida-design`。`yida-design` 生成项目脚手架；页面技能通过薄 page-spec 找到当前页需要读取的 PRD/design.md 章节。真实 `appType`、`formUuid`、`fieldId` 等 ID 写入 `.cache/<项目名>-schema.json`。
