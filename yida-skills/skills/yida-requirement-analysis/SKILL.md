...
| `explicitScope` | 用户明确指定的页面、表单、流程、报表、导航项和本轮交付范围；没有明确指定时写 `null` |
...
3. 用户明确给出页面、表单、流程、报表、导航项或本轮交付范围时，原样写入 `explicitScope`；不要为了完整应用额外推导同级页面。
4. 用户没有明确页面清单时，`explicitScope` 写 `null`，再根据业务目标和角色任务推导必要页面场景。
5. 用户明确给出品牌色、主题 key、参考页面或已有应用主题时，原样写入 `brandHints` 或 `colorHints`。
6. 用户没有指定颜色时，记录业务气质和视觉目标，`colorHints` 写 `null`；最终主题色由 `yida-design` 决定。
7. 事实不足但不影响资源范围时写入 `assumptions`；会改变应用范围、数据对象或权限时写入 `openQuestions`。
8. 写入共享需求简报后，同时加载 `yida-prd` 和 `yida-design`。两个技能读取同一文件，互不等待。
...
- 用户明确页面或资源范围时，`explicitScope` 已原样记录；没有明确范围时已写 `null`。
