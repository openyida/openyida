...
- 读取成功后，根据用户要求提取会议摘要、关键决策或行动项；若要转 PRD，切换到 `yida-flash-note-to-prd` 生成会议需求稿
...
4. 如果用户要求转 PRD，将听记正文作为输入加载 `yida-flash-note-to-prd`；本技能不直接生成会议需求稿。
...
命令成功返回指定 `taskUuid` 对应的真实听记详情；转 PRD 场景已把内容交给 `yida-flash-note-to-prd` 生成会议需求稿。
