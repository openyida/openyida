...
| 4. resolve forms/processes | `yida-form-detail`、`yida-create-form-page`；PRD 命中审批/流程时加载 `yida-create-process` | 已有目标表单时 update/patch/rule/bind-datasource；缺少核心业务表单且允许创建时才 create；创建或更新表单前先让 `yida-form-detail` 确定表单视觉引导和 Divider 分组；需要多字段映射时，每个目标表单最多一次性获取完整 `--field-map-json` 并合并写回 `.cache/<项目名>-schema.json` | 拿到或确认表单/流程表单 `formUuid`，字段结构和必要 ID 映射可供页面阶段使用 |
...
| 7. 编写/更新页面 | 默认 `yida-canvas-custom-page`；明确要求 JSX/Jsx 组件链路或实例桥强依赖时选择 `yida-custom-page` | 每个页面实现前读取 `prd.md` 与 `design.md`；需要生成器时从两份文件派生 `page-spec.json`，可直接手写时跳过 spec 写最终源码；展示列表、看板或详情记录时，优先接本轮真实表单数据 | 本地源码通过对应页面技能的基础校验；未发布时仍是“源码已修改，尚未发布” |
