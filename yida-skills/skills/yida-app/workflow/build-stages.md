...
本文件用于阶段 1-7。按依赖执行，每个阶段只加载当前需要的子技能。
...
| 5C. 编写/更新页面（并行） | `yida-canvas-custom-page` | 阶段 4 后即可执行；每个页面实现前读取 `prd.md` 与 `design.md`，使用真实 `appType/formUuid/fieldId` 写数据绑定；新建自定义页面从 `canvas.canvas.jsx` 扩展，写 `.canvas.jsx` / `.canvas.tsx`；需要生成器时从两份文件派生 `page-spec.json`，可直接手写时跳过 spec 写最终源码 | Canvas 源码通过基础校验；未发布时仍是“源码已修改，尚未发布” |
