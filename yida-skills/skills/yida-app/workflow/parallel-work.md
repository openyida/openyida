...
| 图片要求就绪（Plan 已确认） | 搜图与创建应用、表单、无图页面同时推进；同页各图片位置也并发搜索 | 每页草稿和 appType 就绪即上传；图片只在所属页面接图和验收时汇合 |
...
3. 任务先读取已有页面清单，复用符合当前用途与尺寸要求的 final 图片，继续剩余位置和轮次。搜索任务返回选图及来源信息，由该页唯一写入者组装草稿。每页使用 `assetTasks` 返回的 draft/manifest 路径；上传 argv 中的 `<appType>` 替换为真实应用 ID。草稿和 appType 就绪即上传，页内下载上传由 CLI 并发处理。
