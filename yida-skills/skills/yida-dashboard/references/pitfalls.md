...
>
> 涉及 Recore、`_customState`、`renderJsx`、`didMount`、`this.utils`、ECharts CDN 的条目均为平台 JSX 组件页面排障资料；Canvas 优先检查 hooks cleanup、同源 DataBridge、Recharts 响应式容器和 `runtimeCode`。业务级数据、待办、截图和 URL 结论仍适用。
>
> 建议时机：
> - 发布前最后一次通读
> - 运行时报错排查时按编号对照
