...
创建成功结果同时包含 `url` / `appUrl` / `workbenchUrl`（应用工作台）和 `adminUrl`（开发者管理后台）。完整应用最终交付默认保留 `adminUrl`，与前台、可选业务后台区分；不因云端登录态省略。地址可生成不等于收件人已获得管理权限。历史结果缺少该字段时可通过 `app-list` 分页匹配真实 `appType` 回查。
