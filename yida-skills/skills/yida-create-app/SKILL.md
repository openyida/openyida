...
| `themeFile` | 否，推荐 | — | 基于 `yida-design/sub_skill/yida-design-fast/references/theme/app-custom-theme-template.css` 生成的 CSS；必须完整声明平台实际生成的 `--color-brand1-1/2/3/5/6/9/10`，不要补造 `4/7/8`；其中 `--color-brand1-6` 必须为字面量，CLI 自动以它保存 `themeColor` |
...
主题颜色不受平台预置 key 限制。读取 `yida-design/sub_skill/yida-design-fast/references/theme/app-custom-theme-template.css` 生成主题文件，将任意设计主色写入 `--color-brand1-6`，再执行：
...
完整应用主题 key、颜色倾向和 token 变量统一维护在 `yida-design/sub_skill/yida-design-fast/references/theme/theme-token-presets.md`，本技能不重复维护完整清单。
