...
官方示例中心的报表范式是“原生报表先聚合，自定义页面后增强”。因此，除非用户明确要做高级视觉看板，否则先创建或复用原生报表；新建展示层默认让 `yida-rechart` 或 `yida-canvas-custom-page` 承担。只有维护已有普通自定义页面图表时，才选择 `yida-chart` 或 `yida-custom-page`。
