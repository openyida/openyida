...
- 一次性造数、旧数据修正、字段迁移脚本可以使用 Python 或 JS，优先选择更快更清晰的实现；脚本放在 `.cache/` 下，并复用真实查询到的 appType/formUuid/fieldId/formInstId
...
- 批量脚本可以用 Python `subprocess` 调用 `openyida data ...`，也可以用 JS 复用 Node 工具；不要为了语言一致性牺牲速度和可读性
