...
tokens:
  application-global:
    colors:
      ...
      "--color-line1-1": "#EAEAEA" # 弱边界、辅助线和表格行分隔线
      ...
      "--color-text1-4": "#181818" # 默认一级文字色，文字对比最强
      "--color-text1-10": "#606060" # 表头和 placeholder
      "--color-text1-3": "#8E8E8E" # 二级文字色，弱于 text1-10
...
| 主题浅状态底、当前项背景与图标浅底 | `color-mix(in srgb, var(--color-brand1-6) 12%, #FFFFFF)` | 自定义页 |
| 复合趋势图的主题浅柱 | `color-mix(in srgb, var(--color-brand1-6) 18%, #FFFFFF)` | 自定义页 |
| 高对比主题聚焦卡背景 | `color-mix(in srgb, var(--color-brand1-6) 78%, #111111)` | 自定义页 |
| 聚焦卡内低对比抽象对象 | `color-mix(in srgb, var(--color-brand1-6) 42%, #FFFFFF)` | 自定义页 |
| 图表中的反相事件焦点 | `--color-text1-4` | 自定义页 |
| 第一至第四真实等权类别的方形图标与窄色轨 | `#28B87A / #F28A22 / #9B63E6 / #E84FA7` | 自定义页 |
...
`--color-brand1-6` 是唯一主题色种子。其余六个 Brand Token 与 `color-mix(in srgb, var(--color-brand1-6) 12%, #FFFFFF) / color-mix(in srgb, var(--color-brand1-6) 18%, #FFFFFF) / color-mix(in srgb, var(--color-brand1-6) 78%, #111111) / color-mix(in srgb, var(--color-brand1-6) 42%, #FFFFFF)` 均由它派生并在项目实例化时写入实际色值。`#28B87A / #F28A22 / #9B63E6 / #E84FA7` 是独立分类色，仅在 PRD 确认多个等权类别且颜色区分有必要时使用，不参与主题同色推演，不承担平台状态或主操作。无彩区域不少于页面面积的 82%，主题聚焦卡通常不超过首屏面积的 12%。

### 本主题的配色约束

- `--color-line1-1 / --color-line1-2`、`--color-fill1-1 / --color-fill1-2 / --color-fill1-3`、`--color-text1-4 / --color-text1-10 / --color-text1-3` 与 `--color-text1-4` 均为 `neutral-gray`，Hex 满足 `R = G = B`；`--color-fill1-10` 的前三通道相等。
- `color-mix(in srgb, var(--color-brand1-6) 12%, #FFFFFF)` 与 `color-mix(in srgb, var(--color-brand1-6) 18%, #FFFFFF)` 是 `theme-gray` / 主题浅色，分别由 `--color-brand1-6` 12% / 18% 与 `#FFFFFF` 混合；实例化时写入实际色值。
- `color-mix(in srgb, var(--color-brand1-6) 78%, #111111)` 与 `color-mix(in srgb, var(--color-brand1-6) 42%, #FFFFFF)` 是主题派生色；前者由主题色 78% 与 `#111111` 22% 混合，后者由主题色 42% 与白色 58% 混合。
- `#28B87A / #F28A22 / #9B63E6 / #E84FA7` 是独立分类色，只占小面积；成功、警告、错误和信息状态仍直接消费平台语义色并配合文字或图标。
...
- 所有间距消费 YAML 的 `--s-1` 至 `--s-10`：图标与文字用 `--s-2 / --s-3`，工具组用 `--s-3 / --s-4`，卡片内距用 `--s-4 / --s-6`，大区块用 `--s-6 / --s-8`。
...
- 常规面板使用 `--pod-card-bg-color`、1px `--color-line1-1`、`--corner-4 / --corner-5`，默认不投影；通过边界、留白和局部浅填充建立层级。
...
固定公式为“48-56px 方形图标块 / 两级文字 / 主值与行内变化”。图标块只使用主题色或真实独立分类色，白色线性图标 18-20px；主值消费 `subhead`。变化信息使用平台状态色加方向符号或文字，不能只靠颜色。

### 复合焦点趋势场

绘图区固定高度；底层为 `color-mix(in srgb, var(--color-brand1-6) 18%, #FFFFFF)` 浅柱，中层最多两条 1px 中性对照线，顶层为 2.5px 主题主线。事件节点直径 8-12px，特殊焦点可使用 36-44px 圆形标记并与垂直虚线连接；tooltip 复用深底反色文字浮层。缺少真实对照或事件时删除对应层，不用装饰数据补齐。
...
卡体高 220-280px，背景 `color-mix(in srgb, var(--color-brand1-6) 78%, #111111)`，左侧为标题、`subhead` 主值、比较说明与头像叠层，右侧为 `color-mix(in srgb, var(--color-brand1-6) 42%, #FFFFFF)` 低对比抽象对象。文字使用白色，抽象对象面积不超过卡片 46%；无真实头像时删除头像组，不用虚拟人物补齐。
