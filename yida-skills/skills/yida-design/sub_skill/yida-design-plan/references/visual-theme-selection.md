...
首次搭建的风格未明确、需要准备选项时读取；已有风格要求直接沿用。视觉方向是面向当前项目的一组完整决策；主题模板只是内部实现基底，不作为用户选择对象。
...
用户确认的是整体色彩氛围时，将其落到 `colorStrategy.surfaceTone=brand-tinted`；不要因为所选模板默认中性灰就降级成仅改强调色。明确仅改强调色或忠实保留参考配色时使用 `theme`，无需为这个实现字段另起提问。
