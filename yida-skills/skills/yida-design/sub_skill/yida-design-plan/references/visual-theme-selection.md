...
视觉方向只决定跨页面共享的视觉气质、主题色、导航结构、导航明暗、表面层级和形状语言，不改变页面内容、首屏重点、页面模式、信息密度或操作优先级。

## 输入与优先级

候选生成读取 Step 1 的应用场景事实、视觉证据和 [主题索引](../templates/design-themes/index.json)。视觉证据优先级：
...
4. 任意两项至少在主题基底、主题色、导航结构、导航明暗中的两项不同。
5. “企业内部、专业、稳重”不能单独成为蓝色 + 浅色侧边导航的依据。
6. 用户明确品牌色时不得改色；三项通过主题基底、导航结构和导航明暗形成差异。
7. 项目色彩和导航可覆盖模板默认变量；表面层级、形状语言、组件机制等核心视觉 DNA 与模板冲突时必须换一个基底。
...
```json
{
  ...
  "title": "选择整体视觉方向",
  "prompt": "以下三种方向都适合当前应用，你更希望采用哪一种？",
  "options": [
    {
