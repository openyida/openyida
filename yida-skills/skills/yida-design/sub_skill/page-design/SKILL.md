...
先读取当前应用的 `design.md`，沿用已确认主题和组件规则；不重新选型或为单页另建主题库。尚无主题契约且本次需要确定风格时，按 [共享主题选择规则](../../references/theme-selection.md) 只读 [主题索引](../../templates/design-themes/index.json) 摘要，选中后读取一份完整主题，再补当前页面的项目化设计。局部美化仍保持应用主题、导航和功能契约。
