...
视觉方向要从“高级 / 简洁 / 商务”继续落细。共享需求简报只记录品牌和色彩偏好；`design.md` 写完整 `themeProfile`、主题 token、主色、辅助色、中性色、字体层级和组件基调。
...
| 2 | `application-theme` | 已有应用、工作区或历史命令输出中能读到应用主题和已有页面 token | 单页美化和已有应用改造默认跟随；页面主按钮、链接、选中态和图表主序列跟随应用主题 |
| 3 | `platform-preset-match` | 根据业务气质推导出的主题刚好命中平台预置主题 key | 写 `themePresetKey=<key>`，`shouldPassCreateAppTheme=true` |
| 4 | `business-inferred` | 无明确主题证据，需要根据行业、品牌气质、业务情绪和视觉目标推导 | 设计自定义品牌色盘并写 token，`shouldPassCreateAppTheme=false` |
...
4. 自定义色 token 写入 `design.md`，至少包含 `--color-brand1-6`、`--color-brand1-9`、`--color-brand1-2`、`--color-brand-1` ~ `--color-brand-4` 和 `--color-group`；脚手架负责应用这些 token。
...
- 需要页面主色明显不同于应用主题时，共享需求简报或用户原话必须有业务原因：隐藏平台导航、独立品牌/活动页、公开落地页，或用户明确要求全局换肤；`design.md` 写 `themeScope=app/page`、`themeRelation` 和 token。
...
`openyida create-app --theme <key>` / `openyida update-app --theme <key>` 只填写平台预置主题 key。完整 key 与 token 见 [应用主题与 token 参考](../references/theme/theme-token-presets.md)。自定义色盘不传 `--theme`，token 写入 `design.md`。
...
统一按钮、卡片、表格、标签、抽屉、弹窗、图标、空态、加载态和错误态。写清页面级或应用级主题作用域，不写注入代码。
