...
先识别文字真正落在哪个色块，再选择前景色，不从 `light-*` / `dark-*` 的样式名推断文字颜色：

| 标题承载面 | 样式示例 | 配色方向 |
| --- | --- | --- |
| 主色实底或主色标签 | `dark-bar`、`light-left-title`、`light-center-title`、`light-house-title`、`light-hexagon-title` | 深色或较强的品牌底上，优先考虑白色或近白色标题，不机械继承深色正文色 |
| 弱背景条 | `light-left-bar`、`light-bar` | 按该弱背景的实际明暗配标题色，不因页面是暗黑就一律用白字 |
| 内椭圆 | `inner-ellipse-title` | 标题匹配内椭圆的辅助色，不能照搬外条的主色配法 |

饱和度高不等于底色足够暗。优先白字仍需考虑实际亮度；若白字在亮紫等主色上显得发虚，可以单独加深该组件的 `backgroundColor`，保留品牌色相和协调的 `secondaryColor`。浅黄、浅青等浅底不机械使用白字。选择自定义配色时显式填写 `titleColor`，避免只改主色/辅助色后遗漏标题，继续使用默认的 `#171a1d`。不要为一个色块标题把全局 `--color-text1-4` 或所有 label 改成白色。

例如深紫色实底章节可以从以下配色开始；它只是局部设计示例，不是统一 preset，也不代表截图中应用已经修复：

```json
{ "type": "Divider", "title": "交易信息", "dividerType": "dark-bar", "colorType": "custom", "backgroundColor": "#6D28D9", "secondaryColor": "#2D2244", "titleColor": "#FFFFFF" }
```

分割线是章节标题，表单 label 是字段标题，应放在一起设计，而不是各自挑选颜色：

- 使用同一字体家族；章节标题可比 label 稍大或稍重，例如常规密度下从 16px/600 与 14px/400–500 开始，再按已有主题调整，不用夸张字号争夺注意力。
- 深色表单的品牌底标题可以用白色/近白色；label 保持足够清楚的中性浅色。层级主要由底色、字重和间距建立，不靠把 label 压暗到难以阅读。label 的主题角色仍是 `--pod-form-label-color`，不与分割线 `titleColor` 混为一个设置。
- 左对齐章节的文字起点与下面字段组协调；居中或几何标题可以保留，但不带动字段 label 居中。章节上方留白一般大于标题到首行字段的距离，label 到输入框保持更紧凑，让字段自然归属于章节。
- 章节已有色块时避免再叠加同样强的标签底色、粗边框和大面积高饱和色。重复多组时观察整张表单的节奏，减少装饰抢占输入内容的注意力。

预览时以“章节标题＋首行 label＋输入框”为一个组合看效果，兼顾同页其他章节、长标题和抽屉宽度。在现有 `design.md` 记录配色与层级选择即可，不增加 CLI 参数、固定布局或自动阻断。
