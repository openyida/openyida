...
`--color-brand1-2` 表示品牌弱背景，不代表任何主题下都接近白色。根据 `themeProfile.contentTone` 与实际内容底色选择：浅色内容可与白色混合；暗黑/深色内容应与深色画布混合。内置深色模板以主色 12% + `--pod-page-bg-color` 88% 的 sRGB 混合作为起点，最终仍按标题对比度与项目风格调整；不能因 `navTheme: dark` 就把浅色内容的品牌弱背景改暗。
...
`check-design` 会检查 `--color-text1-4` 与 `--color-brand1-2` 以及辅助色的对比度。生成、上传主题后，分别在实际提交页和详情页检查 `light-left-bar`、`light-bar`、`inner-ellipse-title`：读取分割线元素及标题/背景子节点的 computed style，而不只看根节点 token。浅字与实际弱背景至少达到 4.5:1，同时观察其余主色标签、选中底等消费者。主题上传成功不等于视觉验收通过。
