...
这是一个绿色柔和的进度分析工作台。它用左侧主摘要和指标栈建立状态判断，用右侧浮动时间轴呈现阶段或节点，用趋势图和底部明细表完成分析与检索。共享需求简报 决定内容是否存在，DESIGN.md 只约束视觉 DNA。主题色只替换绿色强调，不改变白色面板、浅边框、时间轴和图表工艺。
...
当 共享需求简报 包含主进度、多个辅助指标、节点或日程、趋势分析和明细记录时选择该模板。主题色替换摘要卡、选中节点、主折线和状态点；背景、白色卡片、浅灰表头、中性文字和圆角机制保持。
...
- 禁止默认折线图、裸表格、自由高度时间轴。
- 禁止瀑布流和零散 margin。
- 禁止 共享需求简报 无时间语义时强行创建时间轴。

## 11. Agent 使用提示

先读 共享需求简报，判断主进度、指标、时间节点、趋势和明细是否存在。再用主题色替换绿色强调。不要凭空创建模块。页面落地时必须保留纹理摘要、对齐时间轴、精制折线和工具化表格。
