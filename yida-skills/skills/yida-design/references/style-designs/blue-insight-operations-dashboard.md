...
这是一个经典、清晰、蓝色主导的运营分析仪表盘视觉 DNA。它由顶部火花线指标卡、主面积趋势图、右侧彩色洞察卡、底部活动表和进度侧列表组成。共享需求简报 管业务内容，DESIGN.md 管视觉机制。主题色可以替换蓝色主图表和强调控件，但不能改变浅灰画布、白色卡片和彩色洞察堆叠。
...
当 共享需求简报 有多个 KPI、一个主趋势、若干洞察提醒、活动记录和进度列表时选择该模板。主题色只替换主图表、火花线、链接、focus ring 和部分图标底；背景、卡片、表格分隔线、低饱和洞察卡结构保持。
...
先读 共享需求简报，确认 KPI、趋势、洞察、活动和进度列表是否存在。再按主题色替换蓝色强调。页面落地时必须保留火花线、面积图 tooltip、洞察卡堆叠、状态表和进度侧列表。

## 12. 交付自检清单

- 内容是否全部来自 共享需求简报。
- 视觉 DNA 是否按 共享需求简报 条件落地。
- 主题色是否只改强调色。
- 图表、表格、列表是否有完整微细节。
- 响应式、键盘焦点和状态可访问性是否达标。
