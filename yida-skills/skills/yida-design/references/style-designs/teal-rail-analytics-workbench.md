...
共享需求简报 决定页面有哪些内容、功能和模块，DESIGN.md 只决定这些模块长什么样。
...
共享需求简报 是内容、功能、信息架构、业务优先级和模块是否存在的唯一来源。DESIGN.md 只负责 UI 风格：视觉 DNA、布局气质、组件形态、密度、状态、响应式和可访问性。

当 共享需求简报 与本文的槽位示例不一致时，以 共享需求简报 为准。不要为了套用 `primary_metrics`、`right_insight_rail`、`ranked_items`、`detail_table` 或 `quick_actions` 而凭空创造业务内容；只需把 共享需求简报 中真实存在的模块映射到最接近的视觉槽位。

禁止复用模板来源中的业务名词、人名、指标名、示例数据和页面专属文案。所有业务内容必须来自 共享需求简报。
...
桌面端必须优先形成左侧主工作区和右侧洞察栏的结构。右栏不是浮动卡片，也不是普通侧边导航，而是用于承载排行、小型趋势、关键对象、状态摘要或辅助洞察的持续信息栏。共享需求简报 不需要右栏时可以折叠为下方区块，但主辅层级和边界感必须保留。
...
明细区域应包含横向筛选标签、轻量搜索/筛选/主操作工具，以及带浅灰表头、图形化单元格和行操作的稳定列表。共享需求简报 没有表格时，可以替换为同等密度的列表，但仍要保留标签筛选与轻工具栏语言。

## 5. 视觉品质基线

这个模板要求页面结果达到精致数据工作台的品质下限。只使用白底、卡片、表格还不够，必须保留足够的微观设计信号。

- 若 共享需求简报 包含多个核心状态、摘要或分类，使用顶部四卡摘要状态条：一行等高白底卡片，标签弱、数值强，卡片之间间距稳定。
- 若 共享需求简报 包含排行、重点对象、辅助摘要或小趋势，使用持续可见的右侧洞察栏：竖线分隔、独立宽度、纵向 section、轻操作链接和细分割线。
- 若 共享需求简报 包含分组比较、时间序列或进度数据，使用分层图表面板：浅灰上下文柱/线、当前主题高亮、深色对比层、白底 tooltip。
- 若 共享需求简报 包含记录列表，使用细线标签筛选和圆角明细表：选中 tab 有细线和数量徽标，表头浅灰，行高稳定，图形化单元格可扫读。
- 若 共享需求简报 包含排行或侧栏列表，使用头像/图标式条目：圆形图标容器、标题、副说明、短数值和状态徽章组合。

品质失败表现：页面只是普通白卡片、普通表头、普通按钮和普通表格的堆叠；没有右侧洞察栏边界、没有分层图表、没有标签筛选细节、没有图标化行和状态微纹理。

## 6. 布局规则

以下槽位是参考映射，不是业务结构要求。生成 UI 时先读 共享需求简报，再选择、重命名、合并、省略或扩展槽位；共享需求简报 没有的模块不要强行补，但必须保留视觉 DNA 和布局稳定性规则。
...
| 局部洞察、重点对象、小型排行 | `right_insight_rail` / `ranked_item_list` | 放入右侧栏；若 共享需求简报 不需要右栏，可折叠为主区下方洞察 section。 |
...
| 记录、任务、资源、审批、消息 | `detail_table` / `detail_list` | 表格和列表由 共享需求简报 数据形态决定，必须保留稳定行高和图形化状态单元格。 |
| 常用入口、快捷操作、快速发起 | `quick_actions` | 是否渲染由 共享需求简报 决定；需要时使用独立区块，不塞进表格工具栏或右侧栏尾部。 |
...
- 快捷入口：若 共享需求简报 需要，使用独立区块承载 4-8 个入口；线性图标、短标签、轻 hover，不做彩色宫格或插画入口。
...
- 禁止让 DESIGN.md 覆盖 共享需求简报 的内容决策；共享需求简报 没有的业务模块不要强行生成。
...
先读取 共享需求简报，确定真实模块、内容优先级和数据形态；再使用流程已解析的主题色改写强调色 token；最后用本 DESIGN.md 约束视觉表达。不要硬套槽位，也不要凭空创造 共享需求简报 未要求的业务内容。主题色可以替换默认色相，但内容替换和换色后仍必须保留五个视觉 DNA：`左宽主工作区 + 右侧洞察栏`、`通透白底细边框卡片网格`、`深墨 + 主题强调色的数据纹理`、`分层图表分析面板`、`标签筛选 + 轻工具栏 + 图标化明细表`。页面交付时必须达到视觉品质基线：四卡摘要状态条、右侧洞察栏、分层图表、圆角明细表、图标化排行条目和完整交互状态应按 共享需求简报 条件尽量落地。

## 12. 交付自检清单

- [ ] 输出文件名来自 front matter 的 `name`，当前文件名为 `teal-rail-analytics-workbench.md`。
- [ ] 内容、模块和业务优先级均来自 共享需求简报，而不是 DESIGN.md 的示例槽位。
- [ ] 共享需求简报 没有的业务模块没有被强行生成。
