...
这个视觉 DNA 是白底、细边框、蓝色主操作的生产力工作台。核心结构是顶部命令筛选、横排指标卡、大幅柱状分析面板、右侧堆叠行动栏和底部记录表。共享需求简报 决定具体内容，DESIGN.md 决定视觉层级和组件工艺。主题色只替换主操作、图表和 focus ring，不改变细边框和右侧栏机制。
...
当 共享需求简报 同时包含核心指标、周期图表、右侧焦点卡和记录列表时选择该模板。主题色替换 `brand`、图表柱、按钮和 focus ring；背景、白色面板、浅灰边框、表格分隔线和右栏堆叠结构保持。
...
| 今日焦点、下一步、提醒 | `right_rail` | 右侧堆叠卡片，由 共享需求简报 决定数量。 |
...
- 禁止 共享需求简报 没有行动队列时强加右栏。

## 11. Agent 使用提示

先读 共享需求简报 判断是否存在指标、周期分析、行动栏和记录表。再按主题色替换蓝色强调。页面落地时必须保留细边框、圆角柱状图、右栏堆叠卡片和工具化表格。

## 12. 交付自检清单

- 内容是否来自 共享需求简报。
- 指标、图表、右栏、表格是否按 共享需求简报 条件落地。
- 主题色是否只改写强调色。
