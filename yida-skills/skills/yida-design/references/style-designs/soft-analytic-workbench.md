...
quality_anchors:
  - id: inset_metric_mosaic
    name: 内嵌式指标拼图区
    required_when: 共享需求简报 包含 3 个以上摘要、状态或核心指标
...
共享需求简报 决定展示什么内容、哪些模块存在、数据优先级如何排序；本文件只定义 UI 风格、布局稳定性、组件形态和视觉品质。主题色可以替换默认强调色，但不能改变白底面板、圆角系统、轻网格图表和小面积强调机制。

不适合用在强品牌叙事、营销转化、沉浸式大屏或信息极少的页面。页面结果必须呈现足够的微观细节，不能退化成默认后台组件的堆叠。

## 2. 使用边界

共享需求简报 决定内容、功能、信息架构、业务优先级、字段结构、模块数量和交互目标。DESIGN.md 约束视觉 DNA、密度、表面层级、圆角、阴影、状态、响应式和可访问性。

不要为了匹配槽位而凭空增加 共享需求简报 没有要求的指标、图表、列表或快捷入口。不要复用任何具体业务字段、示例数据、人名、机构名、分类名称或页面专属文案；所有文案、数据和操作都必须来自 共享需求简报 或产品信息架构。

当 共享需求简报 没有图表数据时，可以使用同等品质的主内容面板承载摘要、流程、状态分布或核心列表，但仍要保留浅面板、稳定高度、精细分隔和小面积强调。
...
当 共享需求简报 有 3 个以上摘要或状态时，必须落地内嵌式指标拼图区。它用等高拼图、强数值层级和小型图标胶囊建立首屏扫描路径；如果没有量化数据，可以用状态摘要或关键对象卡替代。

当 共享需求简报 有趋势、对比或分布时，必须落地图表微纹理。浅网格、圆角柱、平滑线、半透明填充、图例点和精致 tooltip 是区别默认图表的关键；如果没有图表，应以同等高度的主内容面板承载高优先级内容。

当 共享需求简报 有待办、提醒、事件、消息、资源或人员型条目时，应使用右侧明细流。它通过固定宽度、稳定行高、圆形视觉位和描边按钮形成快速处理区；没有此类内容时，不要强行生成右侧列表。

当 共享需求简报 有记录型数据时，应使用柔和明细表格。表头要轻，行高要稳，状态标签要低饱和，行操作应图标化并有 tooltip；没有记录数据时，可替换为卡片列表或主内容面板。

交互式图表应提供悬浮数据提示。浮层使用主题色小面积承载、圆角、阴影和指向锚点，信息简短且不遮挡主体。

## 6. 布局规则

推荐桌面布局为非对称分析网格：左侧主列承载指标组、主图表和明细表格，右侧或相邻列承载辅助列表或短摘要面板。槽位只是视觉映射，不是业务结构要求；共享需求简报 没有的模块不要补。
...
| 辅助提醒、任务、消息、资源 | `supporting_panel` | 使用右侧或相邻纵向明细流；共享需求简报 没有时不渲染。 |
| 结构化记录、明细、审批、资源表 | `detail_table` | 使用柔和表头、稳定行高、状态胶囊和图标化操作。 |
| 高频入口或快捷命令 | `quick_actions` | 仅 共享需求简报 明确需要时渲染为独立区块，不塞入表格工具栏或侧栏尾部。 |
...
快捷入口只在 共享需求简报 需要时出现，作为独立区块。入口项可使用圆形图标、短标签和浅灰底，数量建议 4-8 个，保持等宽或稳定栅格。
...
先阅读 共享需求简报，确定实际内容、模块和业务优先级；再读取本 preset，应用输入主题色到强调 token；最后按视觉 DNA 生成 UI。不要硬套槽位，不要凭空创造内容，不要复用任何页面专属文案或示例数据。

页面落地时优先保证浅灰画布、白色大圆角面板、等高网格、指标拼图、轻网格图表、右侧明细流和柔和表格这些品质机制。主题色只替换色相和小面积强调，不改变圆角、面板、留白、图表微纹理和布局稳定规则。

交付前必须检查图表、表格、列表、按钮、tooltip、hover、focus、loading、empty、error、disabled 和 selected 状态，确保视觉结果不是默认后台组件的简单拼装。

## 12. 交付自检清单

- [ ] 页面内容、字段、数据、动作和模块是否全部来自 共享需求简报。
- [ ] 是否保留浅灰画布、白色面板、大圆角、轻边框和低强度阴影。
- [ ] 指标、图表、列表、表格是否按 共享需求简报 条件落地，而不是强行补齐。
