...
这个视觉 DNA 适合以大图卡片为核心的筛选浏览体验。页面先用标题和分类胶囊建立上下文，再用左侧柔和筛选面板收敛条件，右侧以等高视觉卡片承载候选对象。共享需求简报 决定对象、字段、筛选项、分类和动作是否存在；本文件只定义柔和画廊式布局、控件形态、密度、状态和响应式。主题色可以替换当前强调色，但不能改变浅灰画布、白色卡片、大图留白和小面积角落动作这些机制。

## 2. 使用边界

该模板适合对象有可视预览、图标或可抽象媒体区域的页面。若 共享需求简报 的主要任务是录入、审批、复杂表格分析或长文阅读，应选择其他视觉母体。

共享需求简报 决定内容结构，DESIGN.md 决定 UI 风格。不要为了套用分类、角标、收藏、评分或范围筛选而创造 共享需求简报 没有的功能。所有字段、标题、数值、分类名称和动作名称都必须来自 共享需求简报。
...
卡片角落微操作用于收藏、标记或短状态。它提升可操作感，但只在 共享需求简报 需要时出现。
...
| 筛选条件 | `filter_panel` | 分组垂直堆叠；只渲染 共享需求简报 存在的条件 |
| 搜索条件 | `filter_search` | 放在筛选面板顶部或工具条内，高度稳定 |
| 对象预览 | `media_stage` | 顶部大图/图标/摘要纹理；缺失时用中性占位 |
| 对象主标题 | `card_title` | 1-2 行，超长截断 |
| 首要数值或状态 | `card_value` | 靠右或靠左醒目展示，视 共享需求简报 信息优先级决定 |
| 角标/收藏/标记 | `corner_micro_actions` | 只在 共享需求简报 需要时渲染，不占位 |
...
禁止把 共享需求简报 没有的分类、筛选项、收藏、角标、评分或范围控件强行加入页面。
...
先读 共享需求简报，确认对象是否适合视觉卡片、是否需要分类、筛选、排序和角落动作。再应用主题色，只替换小面积强调色，不改变浅灰画布、白底柔和面板、大图媒体区和等高卡片。页面落地时优先保证分类胶囊、筛选分组、媒体区、卡片高度、角落动作和移动端抽屉的完整状态。
