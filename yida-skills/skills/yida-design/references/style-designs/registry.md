...
`_design-md-template.md` 只规定 design.md 的结构，不参与视觉风格选择。实现页面时读取当前项目的 PRD/design.md，不再读取本目录。
...
6. 每个自定义页面通过薄 page-spec 找到当前项目 PRD/design.md 章节，并使用项目脚手架；页面技能不回读本目录。
...
最终 `design.md` 只记录可交接给页面技能的精简选择摘要：
