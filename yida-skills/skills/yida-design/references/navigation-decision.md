...
前台自定义页面默认交付隐藏平台导航后的独立页面地址；Fast 按已确认业务用途补齐 `entryMode=standalone`，Plan 对绑定到页面的 `role=service` 自动推导缺失值。页面配置持久化并回读后取 CLI 的 `standaloneUrl`，不让用户额外传开关，也不追加 `isRenderNav=false` 等 URL 参数。没有自绘菜单的单步前台同样适用；后台内容页不因此隐藏导航，用户明确的保留导航要求优先。
...
导航作为业务方案的一部分说明：例如“客户在前台预约，员工通过后台菜单维护商品、资讯和处理预约”。默认方案明确时不另设“平台还是自绘”的强制技术选择题；用户明确选择时，保留其要求并说明影响范围。

- Plan：在整体方案中展示各入口的导航归属与管理页只实现业务内容的边界，随整体方案确认；已展示方案的导航归属发生实质变更时，按现有修订与确认流程处理。
- Fast：说明并实施已记录的选择，不新增导航审批环节；保留既有明确要求和局部修改范围。
- 实施中发现重复导航属于修正实现偏差，移除重复页面菜单即可，不自动改成应用级自绘导航，沿用已确认的业务范围。
