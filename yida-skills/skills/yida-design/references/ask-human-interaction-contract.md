...
`yida-design` 输出进度消息，或 Fast / Plan 流程需要用户选择、补充和确认时，执行本契约。内部执行保持技术字段精确，用户可见内容使用业务语言。
...
4. `title` 只写确认主题，`prompt` 只写用户当前需要回答的问题；存在合理默认值时标记“推荐”或“默认”。只有某个选择会产生重要且难以撤销的影响时，才在对应选项的 `description` 中说明影响。
...
| 内部表达 | 用户可见表达 |
| --- | --- |
| 加载 `yida-design`、读取子技能或交互契约 | 省略；需要进度时说明“我先梳理系统范围和搭建方式” |
...
| 场景             | 类型                                           | 必填内容                     | 回写位置                      |
| ---------------- | ---------------------------------------------- | ---------------------------- | ----------------------------- |
| Fast / Plan Gate | `single_choice`                                | 直接开始搭建、先确认整体方案 | 当前会话的 `designMode`       |
...
多模块或前后台复合应用的 Gate 使用以下逻辑问题。简单应用可以把“直接开始搭建”放在第一项并标记推荐，但不能替用户选择：
...
```json
{
  ...
  "title": "选择搭建方式",
  "prompt": "这个系统包含多个相互关联的功能。你希望先确认整体方案，还是直接开始搭建？",
  "options": [
    {
      "value": "plan",
      "label": "先确认整体方案（推荐）",
...
- 模式不明确 → 根据需求范围生成一句上下文说明，再展示 Gate；说明可以区分简单应用、多模块内部应用和前后台复合应用，但不能替用户选择。

可使用以下用户可见上下文文案：

- 多模块内部应用：这个系统包含多个相互关联的功能。建议先确认功能、流程和页面结构，再开始搭建。
- 简单应用：当前需求范围比较清晰，可以直接开始搭建；如果希望先查看整体结构，也可以先确认方案。
- 前后台复合应用：这个系统同时包含用户端和管理端。建议先确认两端边界、共享数据和主要流程，再开始搭建。
...
Plan 流程不得把 `ask_human` 做成固定问卷。AI 先判断现有输入是否足以确定应用范围，再决定一次结构化交互中包含一个还是两个逻辑问题：

1. 只有用户只给出宽泛应用名称或目标、没有模块/任务/流程/可读需求时，才询问一次“主要包含哪些业务模块”。
2. 应用范围已经清晰时不再询问范围；由 AI 自主推断对象、字段、审批、权限、首页、看板、页面和流程细节，并写进最终计划供用户统一检查。
3. 一般情况下生成恰好三套完整视觉方向供用户选择；用户已明确完整视觉约束，或明确采用推荐方案/不再提问时，可以直接写入。
4. 范围与视觉都需要确认时，合并在同一次结构化 `ask_human` 中。计划生成前最多一次结构化交互、最多两个逻辑问题。
5. 不对 AI 推断结果逐项设置中间确认，统一在最终搭建计划中检查和调整。

多个逻辑问题需要共同背景时，整组交互开头最多使用一句引导，例如：

> 请确认「{应用名称}」的应用范围和整体视觉方向。
