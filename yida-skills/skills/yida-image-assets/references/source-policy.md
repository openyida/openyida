...
| Pixabay API | API URL 用于临时展示搜索结果；缓存 API 响应并遵守限流 | 选用后下载并转存项目 CDN，以自托管 URL 交付 |
| unDraw | 人工选取和修改插画；自动搜索、抓取或下载集成先取得许可方同意 | 人工取得文件后按本地素材处理 |
...
- 外部 URL 通过 `openyida asset resolve` 验证可达性；需要自托管时以 CDN URL 作为 final 交付条件。
