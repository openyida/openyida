# 输出：prd.md

> 本技能输出 `prd/<项目名>/prd.md`。`prd.md` 记录业务语义、产品设计、页面结构、资源创建顺序、页面实现交付顺序和导航顺序。PRD 只保留共享需求简报中的品牌和色彩偏好，并引用 `design.md`，不写 UI 视觉设计规则。

## PRD 输出格式

```markdown
# <应用名> PRD

## 1. 应用基本信息

| 项目 | 内容 |
| --- | --- |
| 应用名称 | <业务名称> |
| 应用类型 | <企业管理 / 经营分析 / 流程审批 / 数据采集 / 客户服务 / 库存进销存 / 项目协作 / 资产设备 / 教育培训 / 知识内容 / 监控指挥 / 官网门户 / 活动报名 / 轻量工具> |
| 业务目标 | <一句话说明这个应用解决什么问题> |
| 核心用户 | <角色 1 / 角色 2 / 管理员等> |
| 使用场景 | <PC 办公 / 移动录入 / 大屏查看 / 外部官网等> |
| 核心对象 | <客户 / 订单 / 商品 / 工单 / 审批单等> |
| 明确范围 | <从 requirement-brief.json 的 explicitScope 读取；没有明确范围时写“按业务目标推导”> |
| 品牌和色彩偏好 | <从 requirement-brief.json 读取；用户未指定时写“由 yida-design 决定”> |
| 权限口径 | <谁能看、谁能填、谁能审批、谁能管理> |

## 2. 应用配置

| 配置项 | 值 |
| --- | --- |
| appType | <已有应用填真实 appType；从零创建时写“待创建后回填”> |
| corpId | <目标组织 corpId；未知时写“待登录态确认”> |
| baseUrl | <平台地址，如 https://www.aliwork.com 或私有化域名> |

## 3. 数据结构（业务语义，不含细节 ID）

### <表单 / 数据对象名称>

| 字段名 | 字段类型 | 必填 | 默认值 / 选项 | 关联关系 | 分组 | 说明 |
| --- | --- | --- | --- | --- | --- | --- |
| <字段> | <文本 / 数字 / 日期 / 单选 / 多选 / 子表单 / 关联表> | <是/否> | <默认值或选项> | <关联对象> | <基础信息 / 状态 / 明细> | <业务说明> |

### 初始示例数据计划

完整应用默认在表单字段映射完成后，为核心业务普通表单写入 1-3 条业务化示例记录。示例数据写入可以和自定义页面源码实现、编译并行；线上页面容器在源码编译成功后才创建。这里写业务语义，真实 fieldId 由页面技能通过 `openyida get-schema` 回填。

| 表单 | 默认写入数量 | 示例记录语义 | 是否允许跳过 | 跳过条件 |
| --- | --- | --- | --- | --- |
| <核心业务表单> | <1-3> | <记录 1：关键字段和值；记录 2：关键字段和值；记录 3：关键字段和值> | <否 / 是> | <用户明确不要造数 / 配置字典表 / 敏感数据 / 字段缺少可安全构造值> |

要求：示例记录必须贴合当前业务，不写“测试1/测试2”；日期字段由页面技能转成 13 位毫秒时间戳；普通表单逐条 `openyida data create form` 写入并 query 抽查。流程表单只有 processCode 已确认且页面需要流程实例时才发起。

流程表单需要补充：

| 节点 | 处理人 | 流转条件 | 表单动作 |
| --- | --- | --- | --- |
| 提交 | <角色> | <条件> | <填写/校验> |

## 4. 页面与功能设计

### <页面名>

- 页面类型：<display-page / form-page / process-form / report / detail-entry>
- 页面定位：<主入口页面 / 核心业务页 / 详情页 / 报表页 / 配置页；说明为什么需要这个页面>
- 页面目标：<这个页面帮助用户完成什么判断或操作>
- 页面关系：<从哪里进入、下一步去列表 / 看板 / 表单提交 / 详情 / 报表中的哪一个>
- 设计文件：<display-page 填 `prd/<项目名>/design.md`；普通表单 / 流程表单写“跟随应用主题与表单视觉引导”>
- 设计引用：<引用 design.md 中的章节 ID，例如 themeProfile、sceneRecipes.workbench、components.table、states.empty>
- 风格理由：<一句话说明该页面为什么采用 design.md 中的对应场景规则>
- 主题关系：<跟随应用主题色 / 页面级独立色盘原因；只写摘要，具体 token 与 UI 规则见 design.md>
- 关联资源：
  - 表单：<表单名称；用于新增、编辑、查询或作为列表数据来源>
  - 流程：<流程名称；用于提交、审批、处理或状态流转>
  - 报表 / 数据源：<报表或数据来源；用于指标、图表或大屏>
  - 详情页：<原生 formDetail / 自定义详情页 / 抽屉详情>
- 需要设计的区块：
  - <区块名称>：<区块目的；数据来源；主操作；状态>
  - 区块数量按当前页面任务、用户明确范围和数据来源决定；工作台、首页、门户、看板、展示页和业务入口页要覆盖首屏判断、操作入口、记录承接和状态反馈，但不能为了凑数增加 PRD 没有的功能。
  - 自定义页面需要逐个写清首屏、筛选、列表/卡片、图表、表单入口、详情抽屉、空态等区块。
- 布局骨架：<顶部概览 / 筛选区 / 表格 / 卡片列表 / 图表区 / 右侧详情等>
- 核心组件：<KPI / 快捷入口 / 表格 / 图表 / 表单入口 / 状态标签等>
- 主操作：<新增 / 提交 / 查看 / 审批 / 编辑 / 跳转>
- PC / 移动端差异：<PC 侧边抽屉 / 移动端全屏抽屉 / 卡片堆叠 / 隐藏低频列>
- pageSpecHandoff：
  - pageKey：<当前页面 resourceKey>
  - prdRefs：<pages.<pageKey> / businessObjects.<resourceKey> / interactions.<pageKey> / acceptance.<pageKey>>
  - designFile：<prd/<项目名>/design.md>
  - designRefs：<themeProfile / sceneRecipes.<scene> / components.<name> / states.<name>>

## 5. 设计需求与引用

| 项目 | 内容 |
| --- | --- |
| 设计文件 | `prd/<项目名>/design.md` |
| 品牌和色彩偏好 | <从共享需求简报读取用户指定品牌色、已有应用主题、偏好色和避用色> |
| 视觉目标 | <从共享需求简报读取业务气质、参考案例和素材要求> |
| 页面设计引用 | <逐页列出 designRefs，不复制 design.md 内容> |
| 一致性要求 | <两份文件完成后由 yida-app 校验页面场景和 designRefs；PRD 保留业务事实，design.md 保留视觉事实> |

## 6. 业务逻辑与交互状态

| 类型 | 规则 |
| --- | --- |
| 表单提交后 | <刷新列表 / 回到当前工作台 / 触发流程 / 更新状态> |
| 新增/提交入口 | <写清入口位置、打开方式、PC/移动端差异和提交后回到哪里；具体 URL 与容器实现由页面技能处理> |
| 详情查看 | <写清从哪类真实记录进入详情、PC/移动端打开方式和关闭后是否刷新；具体实例 ID 校验与 URL 构造由页面技能处理> |
| 数据变更 | <自动计算、状态流转、通知或提醒> |
| 权限规则 | <角色能看、能改、能审批的边界> |
| 空状态 | <无数据时的说明、主操作入口和下一步> |
| 加载状态 | <骨架屏 / 表格 loading / 图表 loading> |
| 错误状态 | <错误提示、重试、回退入口> |

## 7. 资源蓝图

| resourceKey | 资源 | 类型 | 用途 | 关键字段 / 功能 | 创建策略 |
| --- | --- | --- | --- | --- | --- |
| `home` | <主页面> | display-page / main | <入口和概览> | <contentBlocks：标题上下文、筛选、摘要、主操作、待办、最近记录、动态、提醒、右侧上下文、空态行动等；按 PRD 需要取舍> | <复用 / 创建> |
| `<business_form>` | <业务表单> | normal-form | <数据录入和管理> | <核心字段> | <复用 / 创建 / 更新> |
| `<approval_process>` | <审批表单> | process-form | <流程闭环> | <节点和条件> | <复用 / 创建 / 更新> |
| `<report>` | <报表> | report | <汇总分析> | <指标口径> | <复用 / 创建 / 更新> |

## 8. 资源创建顺序

资源创建顺序服务于依赖关系；表单/流程在自定义页面之前创建，便于页面消费表单 URL、字段语义和数据来源。

| 顺序 | 资源 | 依赖原因 | 创建后记录 |
| --- | --- | --- | --- |
| 1 | 应用 | 承载所有页面、表单、流程和导航 | `appType` |
| 2 | 普通表单 / 流程表单 | 自定义页面需要表单 URL、字段语义和数据来源 | `formUuid`、字段映射 |
| 3 | 初始示例数据 | 页面需要读取真实表单记录，完整应用默认写入 1-3 条核心业务记录；可与页面实现并行 | 写入数量、抽查结果 |
| 4 | 主自定义页面 / 业务自定义页面 | 页面消费表单入口、表单数据和主题色配置 | `displayPageFormUuid` |
| 5 | 报表 / 数据看板数据源 | 看板或大屏需要汇总指标时创建 | `reportId` 或数据源信息 |
| 6 | 发布与导航排序 | 页面发布成功后再调整导航展示 | 发布 URL、导航状态 |

## 9. 页面实现交付顺序

页面实现交付顺序是页面开发和验收顺序，不等同于资源创建顺序，也不等同于导航顺序。

| 顺序 | 页面 | 本轮交付 | 实现重点 | 依赖资源 | 验收点 |
| --- | --- | --- | --- | --- | --- |
| 1 | <主页面 / 工作台 / 官网首页> | 是 | <首屏、核心入口、主题风格> | <表单入口 / 空态 / 指标口径> | <打开后能完成核心判断> |
| 2 | <核心列表 / 管理页> | <是 / 否> | <筛选、表格、详情抽屉、主操作> | <核心业务表单> | <能查看或进入新增/详情> |
| 3 | <详情 / 看板 / 大屏> | <是 / 否> | <对象叙事、图表、状态、洞察> | <表单数据 / 报表> | <能解释业务状态> |
| 4 | <辅助页面> | <是 / 否> | <配置、说明、低频入口> | <相关表单或链接> | <不阻断主链路> |

完整应用默认把用户明确要求的核心页面标为“是”。用户明确页面清单时，本轮交付顺序只列这些页面和必要支撑资源；不为默认完整系统补页面。标为“是”的页面必须先完成源码和编译，再逐页创建、发布和回读；标为“否”的页面不得创建线上容器或加入本轮导航。

## 10. 导航顺序

导航顺序是用户在平台导航中看到的顺序。导航顺序只引用资源蓝图中的 `resourceKey`，不写真实 ID。

| 分组 | resourceKey 顺序 | 导航呈现 | 放置原则 |
| --- | --- | --- | --- |
| 根导航 | `home` | 平台导航 | 第一入口放最前 |
| 业务办理 | `<approval_process>` | 平台导航分组 | 高频动作靠前 |
| 数据管理 | `<business_form>` | 平台导航分组 | 数据录入、查询和维护集中 |
| 经营分析 | `<dashboard> / <report>` | 平台导航分组 | 管理者查看，放在业务操作之后 |
| 系统配置 | `<config_form>` | 平台导航靠后分组 | 低频维护靠后 |

后续才实现的页面保留在资源蓝图和页面交付顺序中，不放入本轮导航执行计划。

## 11. 验收标准

| 场景 | 验收标准 |
| --- | --- |
| 主页面访问 | <页面发布成功，打开后首屏能完成核心判断> |
| 数据录入 | <表单能提交，提交后页面能刷新或回到正确入口> |
| 初始示例数据 | <完整应用默认已为核心普通表单写入 1-3 条业务化示例记录并 query 抽查；跳过时说明原因> |
| 数据查看 | <列表 / 看板 / 详情能看到真实表单数据或清晰空态> |
| 权限 / 流程 | <权限规则或流程节点生效> |
| 视觉一致性 | <主题色、布局密度、状态、图标和当前业务一致> |
| 导航可用 | <导航顺序符合 PRD，主入口靠前，配置/低频资源靠后> |
```

## 交给页面技能

- `yida-app` 对齐 `prd/<项目名>/prd.md` 和 `prd/<项目名>/design.md`，再生成本轮页面规格和项目脚手架。
- 真实 ID 写入 `.cache/<项目名>-schema.json`。
- 页面实现阶段把 `pageSpecHandoff` 的引用和真实资源写入薄 page-spec；随后按 `prdRefs`、`designRefs` 读取原文章节，并使用项目 Canvas 脚手架。
