...
按目标用户的主任务分别规划：服务前台默认一个 coding 页面完成浏览、填写和结果等已选视图；业务后台或不分前后台的系统按操作效率选择原生或自定义页面，不以“后台”强制原生。计划采用原生能力时，办理任务用提交页，查询维护用数据管理页。同一表单可对应“活动报名”和“报名管理”等不同入口；将用途写入页面 `purpose` 和交互说明，供实现阶段选择路由。
