...
本技能负责生成 `prd/<项目名>/prd.md`。`design.md` 由 `yida-design` 生成，页面源码由页面技能实现。

## 何时使用

| 用户诉求 | 动作 |
| --- | --- |
| 从零搭完整应用、系统、平台 | 先用本技能生成 `prd.md`，再加载 `yida-design` 生成 `design.md` |
...
1. 写入 `prd/<项目名>/prd.md`。
2. PRD 只写业务事实、角色、数据对象、表单/流程/页面资源、页面结构、创建顺序、交付顺序、导航顺序和验收。
3. 每个 display 页面写薄 `pageSpecHandoff`，只包含页面结构、场景、区块、数据来源、主操作、`designFile` 和 `designRefs`。
4. 主题只写应用主题色和风格摘要；完整视觉规则交给 `yida-design`。
...
| 步骤 | 读取文件 | 产出 |
| --- | --- | --- |
| 1. 分析需求和资源 | [分析需求和资源](workflow/step-1-positioning.md) | 应用类型、核心用户、业务对象、资源蓝图 |
