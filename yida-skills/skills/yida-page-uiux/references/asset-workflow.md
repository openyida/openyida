...
3. **区分生产交付与官方 Sample**：生产页面没有 CDN 时只能用已校验外链或降级为**标注缺口的草稿**，不能假装“已上传”；官方 Sample / 离线展示为了原样发布稳定，可使用受控压缩的 JPEG/WebP data URI，但必须遵守本文的体积上限，且不能把这种方式当作生产资源方案。
...
├───────────────────────────────────────────────┤
│ Step 4  生成页面时回填 + 定级：                  │
│   openyida generate-page ... --resolve-assets   │
│   （有 CDN 时加 --upload-assets 转存稳定 URL）   │
└───────────────────────────────────────────────┘
        │
        ▼
...
> `generate-page` 也内置了同一套回填：`--resolve-assets` 触发校验/回填，`--upload-assets` 额外转存 CDN（隐含 `--resolve-assets`），`--offline-assets` 只做非网络的轻量定级。
...
openyida generate-page spec.json --resolve-assets --upload-assets    # 校验+上传+回填→final
```

### 2. 无 CDN，用已校验外链交付（可接受，但非"已上传"）
```bash
openyida asset verify-url "https://images.unsplash.com/photo-xxxx"
openyida generate-page spec.json --resolve-assets                    # 外链校验通过→final（未上传）
```

### 3. 暂无素材，诚实降级为草稿
```bash
openyida generate-page spec.json                 # 官网无 hero → materialStatus=none/draft
...
真实图片也不能只解决 Hero：强视觉官网至少需要“场景 Hero + 产品/服务 + 过程/空间”三类素材组成一条可理解的品牌故事。完整方法见 [真实品牌官网 Playbook](landing/realistic-brand-homepage.md)。

## 官方 Sample 的无 CDN 兜底

官方 Sample 或离线展示应用没有 CDN、但需要保证源码原样发布后图片稳定时，可内嵌压缩后的 JPEG/WebP data URI：
