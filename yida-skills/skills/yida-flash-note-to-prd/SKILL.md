...
- 生成完成后必须提示用户确认需求稿；完整应用设计和 `design.md` 由 `yida-design` 承接
...
后续如果要从会议需求稿继续做完整应用，加载 `yida-design`。`yida-design` 负责把需求稿完善为完整 `prd.md`，并生成 `prd/<项目名>/design.md`。
...
本技能的 PRD 文件结构和质量要求统一引用 [references/flash-note-prd-template.md](references/flash-note-prd-template.md)。完整应用 PRD 和 `design.md` 的产物生成、补齐和验收由 `yida-design` 承接。
...
本技能只把会议内容整理成 `prd/<项目名>/prd.md` 的需求稿，并保留「待确认事项」。完整应用继续开发时，`yida-design` 读取这份需求稿，补齐完整应用 PRD，并生成 `prd/<项目名>/design.md`。
