...
- 页面整体按 PRD 和 design.md 的 `contentBlocks` 落地有业务目的的区块；这些区块应通过密度、主次、分栏和列表节奏形成丰富度，不通过重复大卡片形成面积。计数按区块组算，KPI 子项、快捷入口子项和列表行不能分别计数，也不能为了数量新增 PRD 没有的功能。
...
6. `acceptanceChecks`：逐项检查 contentBlocks 来源与 PRD/design.md 一致、无大空白卡、主色跟随应用主题、KPI/快捷入口子项不计数、移动端不挤压。
