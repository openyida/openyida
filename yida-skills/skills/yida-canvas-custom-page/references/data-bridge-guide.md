...
Canvas 分析数据接入按 [原生报表集成流程](../../yida-canvas-data-binding/references/report-integration.md) 完成数据量确认和报表绑定。`mode=report` 页面使用 `window.__OPENYIDA_YIDA_API__.createReportDataBridge`；运行 `openyida sample yida-canvas-data-binding report-data` 可生成可编译示例。
