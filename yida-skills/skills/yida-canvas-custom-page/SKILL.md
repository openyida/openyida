...
18. **表单打开入口统一容器**：页面内新增、提交和详情操作，执行 `openyida sample openyida-page-template form-open-container --output .cache/samples/form-open-container.jsx` 复制组件，合并到当前 Canvas，接入真实表单、实例 ID 和刷新函数。调用方式见 [容器接入示例](references/navigation-and-entry-guide.md#接入示例)。应用级报名、申请等导航入口按 [入口用途](../yida-nav-shell/references/nav-shell-patterns.md#入口用途与嵌入页面) 在主内容区嵌入提交页。
