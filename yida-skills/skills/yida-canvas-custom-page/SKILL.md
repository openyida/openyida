...
10. **新版主题走应用主题文件**：使用 `yida-design/sub_skill/yida-design-fast/references/theme/app-custom-theme-template.css` 生成 CSS，并通过 `create-app/update-app --theme-file/--nav-theme/--logo-source/--layout` 联合保存；CLI 从 `--color-brand1-6` 自动提取 `themeColor`。运行容器在页面和表单 iframe 中加载同一文件，确保主题变量一致。
