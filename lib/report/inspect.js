...
function summarizeReportSchema(value, context = {}) {
  ...
  pages.forEach(page => (page.componentsTree || []).forEach(root => visitNodes(root, (node) => {
    if (node.componentName === 'RootContent' && node.props && Array.isArray(node.props.layout)) {
      layout = layout.concat(node.props.layout.map(item => ({
        i: item.i,
        x: item.x,
        y: item.y,
        w: item.w,
...
async function run(args = []) {
  ...
  summary.canvasBindings = runtime.probes.filter(probe => probe.success).map(probe => ({
    mode: 'report', appType, reportUuid: reportId, cid: probe.cid,
    componentClassName: probe.className, dataSetKey: probe.dataSetKey,
    filterKeys: probe.filterKeys || [], aliases: probe.aliases || [],
  }));
  summary.components = summary.components.map(component => {
    const probes = runtime.probes.filter(probe => probe.cid === component.cid);
    const probe = probes.find(item => !item.success) || probes[0];
    return {
      ...component,
      queryProbes: probes.map(item => ({ dataSetKey: item.dataSetKey, status: item.status, success: item.success, errorCode: item.errorCode })),
      queryProbe: probe
        ? {
          status: probe.status,
