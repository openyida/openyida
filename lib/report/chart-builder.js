...
function buildCalendarHeatmapSettings() {
  return {
    container: { height: 300 },
    partition: false,
    size: 10,
    colorRange: '#F2F3F5,#98caff,#518ff9,#005cbe',
    tooltip: { showTooltip: true },
  };
}
...
function buildMapSettings() {
  return {
    container: { zoom: 2, height: 500 },
    color: 'SCHEMA_COLOR',
    tooltip: { showTooltip: true },
    colorMode: 'GeekBlue',
    mappingChannel: ['size'],
    label: { fontSize: 12, showLabel: true, content: 'name' },
    attach: false,
    type: 'choropleth',
  };
}
...
function buildUserConfig(chartType) {
  ...
  if (chartType === 'map') {
    return { chartType: 'map', dataConfig: { name: [], value: [] } };
  }
  if (chartType === 'combo') {
...
function buildUserConfigWithFields(chartType) {
  ...
  if (chartType === 'map') {
    return [{
      name: 'chartData', title: '配置数据',
      items: [
        { setterName: 'ColumnFieldSetter', name: 'name', title: '地域字段',
          setterProps: { showFormatTab: true, showEditTab: true, showDrillTab: true } },
        { setterName: 'ColumnFieldSetter', name: 'value', title: '数值字段',
          setterProps: { single: true, showFormatTab: true, showEditTab: true } },
      ],
    }];
  }
...
function extractTopLevelFieldProps(chartType, dataSetModelMap) {
  ...
  if (chartType === 'map') {
    const ds = dataSetModelMap.chartData;
    if (ds) {
      if (ds.name) { result.name = ds.name; }
