...
// 引入经过验证的参考脚本中的构建函数，确保 fieldDefinitionList 生成正确
const {
  buildFieldDefinition: refBuildFieldDefinition,
  buildDataSetEntry: refBuildDataSetEntry,
} = require(path.resolve(__dirname, '../../yida-skills/skills/yida-create-report/build-yida-report-schema'));
 ...
 */
const SELECT_LIKE_PREFIXES = ['selectField_', 'radioField_', 'checkboxField_', 'multiSelectField_', 'employeeField_', 'departmentSelectField_'];
 ...
 *
 * 推断规则：
 * - 日期字段（dataType === 'DATE' 或 fieldCode 以 dateField_/cascadeDateField_ 开头）→ 维度
 * - 单选/多选字段（fieldCode 以 selectField_/radioField_/multiSelectField_/checkboxField_ 开头）→ 维度
 * - 成员字段（fieldCode 以 employeeField_ 开头）→ 维度
 * - 字符串类型且没有聚合函数 → 维度
 * - 数值字段 + 有聚合函数（SUM/COUNT/AVG 等）→ 度量
 *
 * @param {string} fieldCode 字段代码
 * @param {string} dataType 数据类型
 * @param {string} aggType 聚合类型
 * @returns {string} 'true' 或 'false'
 */
function inferIsDimension(fieldCode, dataType, aggType) {
  const code = fieldCode || '';

  // 1. 日期字段 → 维度
  if (dataType === 'DATE' || code.startsWith('dateField_') || code.startsWith('cascadeDateField_')) {
    return 'true';
  }

  // 2. 单选/多选/复选框/单选组字段 → 维度
  if (code.startsWith('selectField_') || code.startsWith('radioField_') ||
      code.startsWith('multiSelectField_') || code.startsWith('checkboxField_')) {
    return 'true';
  }

  // 3. 成员字段 → 维度
  if (code.startsWith('employeeField_')) {
    return 'true';
  }

  // 4. 有聚合函数的数值字段 → 度量
  const numericAggTypesForDim = ['COUNT', 'SUM', 'AVG', 'MAX', 'MIN', 'COUNT_DISTINCT'];
  if (numericAggTypesForDim.includes(aggType)) {
    return 'false';
  }

  // 5. 字符串类型且没有聚合函数 → 维度（可用于分组）
  if (dataType === 'STRING' && aggType === 'NONE') {
    return 'true';
  }
...
/**
 * 使用参考脚本的 buildFieldDefinition 构建 fieldDefinitionList 中的字段定义。
 * 这是一个统一的辅助函数，确保所有 fieldDefinitionList 的构建逻辑与经过验证的参考脚本一致。
 *
 * @param {object} options
 * @param {string} options.cubeCode - 数据集 cubeCode
 * @param {string} options.alias - 字段别名
 * @param {string} options.aliasName - 字段显示名称
 * @param {string} options.fieldCode - 字段编码（会自动 normalizeFieldCode）
 * @param {string} options.dataType - 数据类型
 * @param {string} options.aggregateType - 聚合类型
 * @param {boolean} options.isDim - 是否为维度字段
 * @param {string} options.timeGranularityType - 时间粒度
 * @param {string} options.cubeTenantId - 租户 ID
 */
function makeFieldDef(options) {
  // 提取纯字符串的 aliasName（兼容 i18n 对象和纯字符串两种格式）
  const rawAliasName = options.aliasName || options.alias || '';
  const aliasNameStr = (typeof rawAliasName === 'object' && rawAliasName.zh_CN) ? rawAliasName.zh_CN : rawAliasName;

  const fd = refBuildFieldDefinition({
    cubeCode: options.cubeCode || '',
    isDim: options.isDim !== undefined ? options.isDim : false,
    alias: options.alias || '',
    aliasName: aliasNameStr,
    classifiedCode: options.cubeCode || '',
    fieldCode: normalizeFieldCode(options.fieldCode || ''),
    dataType: options.dataType || 'STRING',
    aggregateType: options.aggregateType || 'NONE',
    timeGranularityType: options.timeGranularityType || null,
  });
  // 参考脚本的 aliasName 是字符串，但报表引擎需要 i18n 对象格式
  fd.aliasName = { type: 'i18n', zh_CN: aliasNameStr };
  // 添加 cubeTenantId（参考脚本不包含此字段，但报表引擎需要）
  if (options.cubeTenantId) {
    fd.cubeTenantId = options.cubeTenantId;
  }
  return fd;
}
 ...
 *
 * 关键规则（从宜搭报表实际保存接口学习）：
 * 1. isDimension 根据字段类型自动推断，也可通过参数显式指定
...
function buildFieldObj(cubeCode, fieldCode, aliasName, alias, dataType, aggregateType, orderType, _isDimension) {
  ...
  // SUM/AVG/MAX/MIN 等聚合函数的结果是数值，强制使用 DOUBLE；COUNT 保持原字段类型
  const doubleAggTypes = ['SUM', 'AVG', 'MAX', 'MIN', 'COUNT_DISTINCT'];
  const effectiveDataType = doubleAggTypes.includes(aggType) ? 'DOUBLE' : (dataType || 'STRING');

  // 宜搭报表引擎中所有字段的 isDimension 统一为 'false'
  const isDimensionValue = 'false';
...
function buildDataViewQueryModel(chart, cubeTenantId) {
  ...
  if (Array.isArray(chart.fields) && chart.fields.length > 0 && !chart.xField && !chart.yField) {
    ...
    // 注意：宜搭报表引擎要求 fieldDefinitionList 中度量（yField）排在维度（xField）之前
  ...
  allFields.forEach((f) => {
    ...
    // SUM/AVG/MAX/MIN 等聚合函数的结果是数值，dataType 需要用 DOUBLE；COUNT 保持原字段类型
    const doubleAggTypes = ['SUM', 'AVG', 'MAX', 'MIN', 'COUNT_DISTINCT'];
    const effectiveDataType = doubleAggTypes.includes(aggType) ? 'DOUBLE' : (f.dataType || 'STRING');

    fieldDefinitionList.push(makeFieldDef({
      cubeCode: cubeCode,
      alias: alias,
...
function inferDataType(fieldCode) {
  ...
  if (fieldCode.startsWith('numberField_') || fieldCode.startsWith('rateField_')) { return 'NUMBER'; }
  if (fieldCode.startsWith('dateField_')) { return 'DATE'; }
  if (fieldCode.startsWith('employeeField_') || fieldCode.startsWith('departmentSelectField_')) { return 'ARRAY'; }
  return 'STRING';
}
...
function normalizeField(field, defaultAggregateType) {
  ...
  return {
    ...
    aliasName: field.aliasName || field.label || field.title || field.fieldCode || '',
...
function buildDataSetModelMap(chart, cubeTenantId) {
  ...
  if (chartType === 'table') {
    // 支持两种输入格式：
    // 格式1：columnFields/columns/fields 数组（原有格式）
    // 格式2：xField（维度）+ yField（度量）组合（与 bar/pie 保持一致）
    let allFields;
    if ((chart.xField || chart.yField) && !chart.columnFields && !chart.columns && !chart.fields) {
      const xFields = normalizeFieldArray(chart.xField, 'NONE').map((f) => ({ ...f, role: 'col' }));
      const yFields = normalizeFieldArray(chart.yField, 'COUNT').map((f) => ({ ...f, role: 'col' }));
      allFields = [...xFields, ...yFields];
    } else {
      const rawColumns = chart.columnFields || chart.columns || chart.fields || [];
      allFields = normalizeFieldArray(rawColumns, 'NONE').map((f) => ({ ...f, role: 'col' }));
    }
...
function buildFilterFieldListItem(cubeCode, alias, aliasNameZh, fieldCode, dataType) {
  return {
    ...
    beUsedTimes: 1,
    isVisible: 'y',
    id: normalizeFieldCode(fieldCode),
    text: aliasNameZh,
    measureType: 'MEASURE_ATTRIBUTE',
  };
}
...
function buildSelectFilter(filterDef, filterNodeId, filterFieldId) {
  const cubeCode = normalizeCubeCode(filterDef.cubeCode || '');
  ...
  const dataSetModelMap = {
    selectFilter: {
      dataViewQueryModel: {
        cubeCode: cubeCode,
        fieldDefinitionList: [
          makeFieldDef({ cubeCode, alias: aliasValue, aliasName: valueFieldDef.aliasName || '查询字段', fieldCode: valueFieldDef.fieldCode || '', dataType: valueFieldDef.dataType || 'STRING', isDim: false, cubeTenantId }),
          makeFieldDef({ cubeCode, alias: aliasLabel, aliasName: labelFieldDef.aliasName || '显示字段', fieldCode: labelFieldDef.fieldCode || valueFieldDef.fieldCode || '', dataType: labelFieldDef.dataType || 'STRING', isDim: false, cubeTenantId }),
        ],
...
function injectFilterLinkage(dataSetModelMap, filterMeta, filterFieldCode, cubeCode, cubeTenantId) {
  ...
  if (ds.dataViewQueryModel && ds.dataViewQueryModel.fieldDefinitionList) {
    ds.dataViewQueryModel.fieldDefinitionList.push(makeFieldDef({
      cubeCode: cubeCode,
