...
function buildFieldObj(cubeCode, fieldCode, aliasName, alias, dataType, aggregateType, orderType, _isDimension) {
  ...
  // 计算 isDimension 值：显式传入优先，否则自动推断
  let isDimensionValue;
  if (_isDimension !== undefined && _isDimension !== null) {
    // 显式传入的值优先，支持 boolean 和 string
    isDimensionValue = String(_isDimension) === 'true' ? 'true' : 'false';
  } else {
    // 自动推断
    isDimensionValue = inferIsDimension(fieldCode, dataType, aggType);
  }
