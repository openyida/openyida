...
function stripPlatformOmittedChartDefaults(
  value,
  insideYoushuComponent = false,
  youshuComponentName = null,
  parentKey = null
) {
  if (Array.isArray(value)) {
    value.forEach((item) => stripPlatformOmittedChartDefaults(
      item,
      insideYoushuComponent,
      youshuComponentName,
      parentKey
    ));
    return value;
  ...
  ) ? value.componentName : youshuComponentName;
  const insideYoushu = insideYoushuComponent || !!currentComponentName;
  ...
  if (
    currentComponentName === 'YoushuPieChart'
    && parentKey === 'legend'
    && value.cardWidth === null
  ) {
    delete value.cardWidth;
  }
  ...
  Object.entries(value).forEach(([key, child]) => stripPlatformOmittedChartDefaults(
    child,
    insideYoushu,
