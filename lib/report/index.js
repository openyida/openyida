...
function readReportConfig(chartsJsonOrFile) {
    ...
    if (fs_1.default.existsSync(chartsJsonOrFile)) {
        try {
            raw = JSON.parse(fs_1.default.readFileSync(chartsJsonOrFile, 'utf-8'));
        }
        catch (e) {
...
async function main() {
    ...
    // Step 3: 创建空白报表
    console.error('\n[Step 3] 创建空白报表...');
    const createResult = await (0, utils_1.requestWithAutoLogin)(function (auth) {
        return (0, http_1.createBlankReport)(auth.baseUrl, auth.csrfToken, auth.cookies, appType, reportTitle);
    }, authRef);
    ...
    // Step 4: 构建报表 Schema
    console.error('\n[Step 4] 构建报表 Schema...');
    const schema = (0, chart_builder_1.buildReportSchema)(reportTitle, charts, reportId, corpId);
    schema.id = reportId;
    // Step 4.1: 注入筛选器（如果有）
    if (filters.length > 0) {
        ...
        if (pageHeaderContent) {
            ...
            const builtFilters = filters.map((filterDef) => {
                const filterNodeId = (0, constants_1.genNodeId)();
                const filterFieldId = (0, constants_1.genFieldId)('YoushuSelectFilter');
                return (0, chart_builder_1.buildSelectFilter)({ ...filterDef, cubeTenantId: filterDef.cubeTenantId || corpId }, filterNodeId, filterFieldId);
            });
            // 构建筛选器容器并注入到 PageHeaderContent
            const containerFieldId = (0, constants_1.genFieldId)('filter');
            const filterContainer = (0, chart_builder_1.buildFilterContainer)(builtFilters, containerFieldId);
            if (!pageHeaderContent.children) {
            ...
            builtFilters.forEach((builtFilter, fi) => {
                ...
                targetCharts.forEach((target) => {
                    ...
                    if (!chartNode || !chartNode.props || !chartNode.props.dataSetModelMap) {
                        return;
                    }
                    ...
                    chartNode.props.dataSetModelMap = (0, chart_builder_1.injectFilterLinkage)(chartNode.props.dataSetModelMap, filterMeta, filterFieldCode, cubeCode, corpId);
    ...
    const saveResult = await (0, utils_1.requestWithAutoLogin)(function (auth) {
        return (0, http_1.saveReportSchema)(auth.baseUrl, auth.csrfToken, auth.cookies, appType, reportId, schema);
    }, authRef);
