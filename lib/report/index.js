...
async function main() {
  ...
  // DEBUG: 输出 Schema 到文件
  try {
    const debugPath = path.resolve(process.cwd(), ".cache/debug-full-schema.json");
    fs.writeFileSync(debugPath, JSON.stringify(schema, null, 2));
    console.error("[DEBUG] Schema 已写入:", debugPath);
  } catch (e) {
    console.error("[DEBUG] 写入失败:", e.message);
  }
