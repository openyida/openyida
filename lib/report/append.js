...
async function getReportSchema(baseUrl, cookies, appType, reportId) {
  ...
  return new Promise((resolve, reject) => {
    const qs = querystring.stringify({ formUuid: reportId, schemaVersion: "V5" });
    const options = {
      hostname: parsedUrl.hostname,
      port: parsedUrl.port || (isHttps ? 443 : 80),
      // GET 用 /alibaba/web/ 路径
      path: `/alibaba/web/${appType}/_view/query/formdesign/getFormSchema.json?${qs}`,
      method: "GET",
      headers: {
        Origin: baseUrl,
        Referer: baseUrl + "/",
    ...
    const req = requestModule.request(options, (res) => {
      ...
      res.on("end", () => {
        console.error("[HTTP] GET Schema 状态码:", res.statusCode);
        try {
...
async function saveSchema(baseUrl, csrfToken, cookies, appType, reportId, schema) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = requestModule.request(options, (res) => {
      ...
      res.on("data", (chunk) => { data += chunk; });
      res.on("end", () => {
        console.error("[HTTP] SAVE Schema 状态码:", res.statusCode);
        try {
          resolve(JSON.parse(data));
        } catch {
          resolve({ success: false, errorMsg: "HTTP " + res.statusCode + ": 响应非 JSON" });
...
function parseArgs() {
  ...
  const [appType, reportId, chartsJsonOrFile] = argv;
...
async function main() {
  const { appType, reportId, chartsJsonOrFile } = parseArgs();
