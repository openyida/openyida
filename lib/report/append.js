...
async function getReportSchema(baseUrl, cookies, appType, reportId) {
    return (0, utils_1.httpGet)(baseUrl, `/alibaba/web/${appType}/_view/query/formdesign/getFormSchema.json`, { formUuid: reportId, schemaVersion: 'V5' }, cookies);
}
// ── HTTP：保存报表 Schema ─────────────────────────────
...
function readChartsDefinition(chartsJsonOrFile) {
    let charts;
    if (fs_1.default.existsSync(chartsJsonOrFile)) {
        try {
            charts = JSON.parse(fs_1.default.readFileSync(chartsJsonOrFile, 'utf-8'));
        }
        catch (e) {
...
function buildChartNode(chart, cubeTenantId) {
    ...
    const componentName = constants_1.CHART_COMPONENT_MAP[chartType] || constants_1.CHART_COMPONENT_MAP.bar;
    const fieldId = (0, constants_1.genFieldId)(componentName);
    const nodeId = (0, constants_1.genNodeId)();
    const chartTitle = chart.title || (componentName + '_' + (0, constants_1.randomId)().slice(0, 4));
    const dataSetModelMap = (0, chart_builder_1.buildDataSetModelMap)(chart, cubeTenantId);
    const userConfig = (0, chart_builder_1.buildUserConfigWithFields)(chartType);
    return {
        fieldId,
        nodeId,
        componentName,
        ...
        node: {
            ...
            props: {
                ...
                headerSize: 'medium',
                link: (0, chart_builder_1.buildLink)(),
                exportData: (0, chart_builder_1.buildExportData)(),
                openRefresh: true,
                enabledCache: true,
                auth: [],
                fieldId: fieldId,
                afterFetch: (0, chart_builder_1.buildAfterFetch)(),
                __style__: {},
                mockData: (0, chart_builder_1.buildMockData)(chartType),
                dataSetModelMap: dataSetModelMap,
                userConfig: userConfig,
                settings: (0, chart_builder_1.getChartSettings)(chartType),
                titleTip: false,
                hasFullscreen: false,
                copyAsImg: false,
                height: null,
...
async function main(args) {
    ...
    charts.forEach((chart, i) => {
        const componentName = constants_1.CHART_COMPONENT_MAP[chart.type] || constants_1.CHART_COMPONENT_MAP.bar;
        console.error(`  ${i + 1}. [${chart.type}] ${chart.title || componentName}`);
    });
    // Step 3: 获取已有报表 Schema
    console.error('\n[Step 3] 获取已有报表 Schema...');
    const getResult = await (0, utils_1.requestWithAutoLogin)(function (auth) {
    ...
    const rootContent = page.componentsTree[0].children.find((c) => c.componentName === 'RootContent');
    if (!rootContent) {
    ...
    charts.forEach((chart, index) => {
        ...
        const alreadyInMap = page.componentsMap.some((c) => c.componentName === componentName);
        if (!alreadyInMap) {
    ...
    // Step 6: 保存 Schema
    console.error('\n[Step 5] 保存 Schema...');
    const saveResult = await (0, utils_1.requestWithAutoLogin)(function (auth) {
