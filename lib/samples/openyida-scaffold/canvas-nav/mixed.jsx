...
function CanvasNav({ items = [], activeKey, onSelect, title, logo, actions, children }) {
  ...
  return <div className="oy-canvas-nav oy-nav-mixed">
    <style>{CANVAS_NAV_CSS + `
      ...
      .oy-nav-mixed .oy-nav-body { display: grid; flex: 1; grid-template-columns: var(--shell-dark-aside-width, 216px) minmax(0, 1fr); }
      .oy-nav-mixed .oy-nav-menu { display: flex; flex-direction: column; gap: var(--pod-nav-slide-aside-gap, 12px); padding: var(--pod-nav-l-aside-padding, 8px); border-right: 1px solid var(--pod-nav-sub-divider-color, var(--color-line1-1, #ddd)); }
