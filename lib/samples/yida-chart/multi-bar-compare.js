...
var PALETTE = {primary: '#D97706', primaryLight: '#FBBF24', accent: '#0EA5E9', success: '#10B981', warning: '#D97706', danger: '#DC2626', neutral: '#64748B', bg: '#F8FAFC', cardBg: '#FFFFFF', border: '#E2E8F0', textPrimary: '#1D2939', textSecondary: '#475569', textMuted: '#94A3B8'};
...
export function loadBarData() {
  ...
  Promise.all([
    ...
    .then(function(results) {
      ...
      var barData = this.buildMultiBarData(statusData, priorityData);
...
var _parseTableData = function(content) {
  ...
  return data.map(function(row) {
    return { name: String(row[dimField] || '未分类'), value: Number(row[measureField]) || 0 };
  });
};
 ...
 */
export function buildMultiBarData(statusData, priorityData) {
  var mainCategories = ['状态分布', '优先级分布'];
  var subCatSet = {};
  var matrix = { 状态分布: {}, 优先级分布: {} };

  (statusData || []).forEach(function(item) {
    subCatSet[item.name] = true;
    matrix['状态分布'][item.name] = item.value || 0;
  });
  (priorityData || []).forEach(function(item) {
    subCatSet[item.name] = true;
    matrix['优先级分布'][item.name] = item.value || 0;
  });
