...
const BACKGROUND_LOG_NAME = 'agent-run.log';
const ENV_KEYS = [
...
function runtimeEnvironment(env = process.env) {
  ...
  // npm CLI shebangs must find the same Node that successfully started OpenYida,
  // even when OpenYida itself was launched through an absolute Node path.
  result.PATH = [path.dirname(process.execPath), env.PATH || env.Path || env.path || ''].filter(Boolean).join(path.delimiter);
    ...
    function emitLine(line) {
      ...
      try {
        ...
        if (event.type === 'error') {
          ...
          diagnostic('error', 'runtime_action_failed', {
            command: config.command, component: 'node', errorCode: code, retryable: event.retryable === true,
          });
...
// The per-connection background daemon writes its structured runtime events and
// untrusted provider stderr to this file so `openyida agent logs` can surface
// them after the launcher has already returned the terminal.
function backgroundLogPath(config) {
  return path.join(config.stateDir, 'logs', BACKGROUND_LOG_NAME);
}

// Launch the Go runtime as a detached background daemon. Unlike launchRuntime,
// this deliberately does NOT bind the runtime lifetime to the launcher: the
// daemon survives terminal/desktop close and is governed only by its own owner
// lock and the authenticated loopback control socket, so `disconnect` remains
// the way to stop it. No parent-lifetime pipe is attached. Output is redirected
// to a per-connection log file. Resolves once the child is spawned and released,
// without awaiting its exit.
function launchBackgroundRuntime(runtime, config, options = {}) {
  if (config.command !== 'run') {
    throw new CliError(t('agent.input_invalid'), { code: 'AGENT_INPUT_INVALID' });
  }
  const daemonConfig = { ...config, background: true };
  delete daemonConfig.parentWatchFD;
  delete daemonConfig.parentPID;
  const payload = JSON.stringify(daemonConfig);
  if (Buffer.byteLength(payload) > MAX_CONFIG_BYTES) {
    throw new CliError(t('agent.input_invalid'), { code: 'AGENT_INPUT_INVALID' });
  }
  const diagnostic = typeof options.diagnostic === 'function' ? options.diagnostic : () => {};
  const logPath = backgroundLogPath(daemonConfig);
  return new Promise((resolve, reject) => {
    let logFd;
    try {
    ...
    let child;
    try {
      child = (options.spawn || spawn)(runtime.executable, [daemonConfig.command, '--stdio-config'], {
        cwd: daemonConfig.node.packageRoot,
        env: runtimeEnvironment(options.env),
        // stdout and stderr are redirected to the daemon log; stdin carries the
        // one launch-config JSON and then reaches EOF, which the daemon ignores.
      ...
      diagnostic('error', 'runtime_spawn_failed', { command: daemonConfig.command, component: 'node', errorCode: 'AGENT_RUNTIME_START_FAILED' });
      reject(new CliError(t('agent.runtime_failed'), { code: 'AGENT_RUNTIME_START_FAILED' }));
      return;
    }
    // The child holds its own duplicated descriptor after spawn; releasing the
    // launcher's copy keeps no handle that would tie the daemon to this process.
    ...
    diagnostic('info', 'runtime_backgrounded', { command: daemonConfig.command, component: 'node' });
    resolve({ pid: child.pid, logPath });
  });
}

module.exports = { MAX_CONFIG_BYTES, BACKGROUND_LOG_NAME, backgroundLogPath, runtimeEnvironment, readStdinJson, launchRuntime, launchBackgroundRuntime };
