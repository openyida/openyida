...
const { launchRuntime, launchBackgroundRuntime, BACKGROUND_LOG_NAME } = require('./stdio');
...
const COMMANDS = ['doctor', 'diagnose', 'status', 'connect', 'run', 'disconnect', 'logout', 'logs'];
const VALUE_OPTIONS = {
...
function buildLaunchConfig(args, options = {}) {
  ...
  const config = {
    ...
    stateDir,
    sessionStateDir: path.join(stateDir, 'sessions'),
  ...
  const endpoint = args.endpoint || env.OPENYIDA_AGENT_CONTROL_ENDPOINT ||
    (options.env === undefined ? openyidaAgent.endpoint : undefined);
  const endpointId = args.endpointId || env.OPENYIDA_AGENT_ENDPOINT_ID ||
    (options.env === undefined ? openyidaAgent.endpointId : undefined);
  ...
  if (['connect', 'run', 'doctor', 'diagnose'].includes(args.command) && config.providers.length === 0) {
    const discovered = discoverProviders(env, options.platform || process.platform, options.homedir).filter(p => !args.provider || p.provider === args.provider);
...
function readLogTail(logPath, maxBytes = 64 * 1024) {
  let fd;
  try {fd = fs.openSync(logPath, 'r');} catch {return null;}
  try {
...
// `logs` is a Node-only inspection command. It never launches the Go runtime;
// it surfaces the per-connection background daemon log so the user can review a
// connection that is running detached after the terminal already returned.
function emitLogs(args, options, stdout) {
  const baseStateDir = args.stateDir || path.join(options.homedir || os.homedir(), '.openyida', 'local-agent');
  if (!path.isAbsolute(baseStateDir)) {
    throw new CliError(t('agent.absolute_path_required'), { code: 'AGENT_STATE_PATH_INVALID' });
  }
  const connections = options.disableConnectionManager === true ? [] : listConnections(baseStateDir);
  if (connections.length === 0) {
    stdout.write(`${JSON.stringify({ type: 'logs', connections: [] })}\n`);
    return;
  }
  for (const connection of connections) {
    const logPath = path.join(connection.stateDir, 'logs', BACKGROUND_LOG_NAME);
    const content = readLogTail(logPath);
    stdout.write(`${JSON.stringify({ type: 'logs', connectionId: connection.connectionId, endpointId: connection.endpointId, logPath, available: content !== null })}\n`);
    if (content) {stdout.write(content.endsWith('\n') ? content : `${content}\n`);}
  }
}

async function run(argv = [], options = {}) {
  if (argv[0] === 'ask-human') {
    return require('./ask-human').run(argv.slice(1), options);
  }
  const args = parseArgs(argv);
  const stdout = options.stdout || process.stdout;
  if (args.command === 'help') {stdout.write(`${t('agent.usage')}\n`); return;}
  if (args.command === 'logs') {return emitLogs(args, options, stdout);}
  let providerCheck;
  // A normal `run` must resolve the same usable executable that connect/doctor
  // validated. A stale earlier PATH entry must not replace a working bundled
  // CLI after restart. An explicit absolute provider path is verified by Go.
  if (['connect', 'doctor'].includes(args.command) || (args.command === 'run' && !args.providerPath)) {
    ...
    if (args.command === 'run' && !providerCheck.ready) {
      throw new CliError(t('agent.preflight_blocked'), { code: 'AGENT_PREFLIGHT_BLOCKED', details: providerCheck });
    }
  }
  ...
  if (managedConnections && ['run', 'diagnose', 'status', 'disconnect', 'logout'].includes(args.command)) {
    ...
    if (args.command === 'run') {
      // Re-online mirrors connect: each Connection's serving loop runs as its own
      // detached background daemon and the terminal returns immediately, persisting
      // until `disconnect`. Organization Connections are independent security and
      // execution domains, so one failing to launch must not take the others
      // offline; only an all-Connection failure is fatal.
      const results = await Promise.allSettled(preparedConnections.map((prepared, index) => {
        const runConfig = { ...prepared.config, command: 'run' };
        return launchBackgroundRuntime(prepared.runtime, runConfig, { ...options, stdout }).then((background) => {
          stdout.write(`${JSON.stringify({
            type: 'connected', background: true,
            connectionId: connections[index].connectionId, endpointId: connections[index].endpointId,
            stateDir: runConfig.stateDir, logPath: background.logPath,
          })}\n`);
        });
      }));
      ...
      if (failures.length === preparedConnections.length) {throw failures[0].result.reason;}
      return;
    }
    // status/disconnect/logout/diagnose stay in the foreground: they are short
    // queries or graceful stops that must report their result to this terminal.
  ...
  if (args.command === 'connect') {
    // Pairing has completed in the foreground above. The serving loop now runs
    // as a detached background daemon so the terminal returns immediately and
    // the connection persists across terminal/desktop close until `disconnect`.
    const runConfig = { ...config, command: 'run' };
    delete runConfig.enrollmentToken;
    const background = await launchBackgroundRuntime(runtime, runConfig, { ...options, stdout });
    stdout.write(`${JSON.stringify({ type: 'connected', background: true, stateDir: runConfig.stateDir, logPath: background.logPath })}\n`);
  }
}
