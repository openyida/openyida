...
function assertManagedInvocation(command, args = [], options = {}) {
  ...
  if (args.some(value => IDENTITY_OPTIONS.has(String(value).split('=')[0]))) {
  ...
  if (args[entry.appPosition] !== context.appType || args.slice(0, entry.appPosition).some(arg => arg.startsWith('-'))) {
