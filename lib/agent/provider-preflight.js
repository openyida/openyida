...
// Match the Go Runtime's npm launcher handling without cmd.exe interpolation.
function probeCommand(executable, args, platform = process.platform, env = process.env) {
  ...
  const command = `& ${[script, ...args].map(powerShellLiteral).join(' ')}`;
...
async function preflight(args, options = {}) {
  ...
  const providers = await Promise.all(entries.map(async entry => {
    ...
    // Windows npm shims start PowerShell and Node for each of the three checks.
    // A healthy cold CLI can take more than 15 seconds for a single handshake.
    const timeoutMs = options.timeoutMs || (platform === 'win32' ? 45000 : 15000);
    const deadline = Date.now() + (options.budgetMs || (platform === 'win32' ? 135000 : 30000));
    const boundedProbe = async (exe, flags, rpc, opts) => {
      const remaining = deadline - Date.now();
      if (remaining <= 0) { return { status: 'probe_timeout' }; }
      return (options.probe || probe)(exe, flags, rpc, { ...opts, timeoutMs: Math.min(timeoutMs, remaining) });
    };
