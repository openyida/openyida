...
const USAGE = '用法：openyida agent ask-human --request-key <稳定业务键> --questions-file <相对路径> --json';
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (!key || result[key] !== undefined || !argv[i + 1] || argv[i + 1].startsWith('--')) {
      fail('AGENT_ASK_HUMAN_INVALID', USAGE);
    }
    result[key] = argv[++i];
  }
...
function run(argv = [], options = {}) {
  ...
  if (args.help) {
    (options.stdout || process.stdout).write(`${USAGE}\n问题文件格式：{ "title": "...", "questions": [{ "id": "...", "header": "...", "question": "...", "type": "text|single_select|multi_select", "options": [{ "label": "...", "description": "..." }] }] }\n`);
