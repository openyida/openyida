...
const { MAX_MANIFEST_BYTES, MAX_BINARY_BYTES, SHA256, PRODUCTION_RUNTIME_PLATFORMS, decodeManifestV2, verifyManifestSignature, parseStrictJson, exactKeys } = require('./runtime-manifest');
...
function platformPackageName(platform, arch) {
  if (!PRODUCTION_RUNTIME_PLATFORMS.includes(platform) || !['arm64', 'x64'].includes(arch)) {
    throw runtimeError('AGENT_RUNTIME_PLATFORM_UNSUPPORTED', 'platform_unsupported');
  }
