  ...
  return new Promise((resolve, reject) => {
    ...
    const onEnd = () => {
      ...
      if (!data || typeof data !== 'object' || Array.isArray(data)
          ...
          || typeof data.corpId !== 'string' || !data.corpId || data.corpId.length > 256
...
async function syncDws(options = {}) {
  ...
  if (options.corpId && options.corpId !== handoff.corpId) { throw failure('ORG_CONFLICT'); }
  let response;
  try {
    response = await requestJson('POST', `${target.origin}/openapi/cli/v1/auth/dws/token`, {
      corpId: handoff.corpId, environment: handoff.environment,
  ...
  if (typeof data.access_token !== 'string' || !data.access_token
      ...
      || data.client_id !== DINGTALK_OAUTH_CLIENT_ID || data.corp_id !== handoff.corpId
...
async function run(args, options = {}) {
  if (args.length === 2 && args.includes('--capabilities') && args.includes('--json')) {
    const capabilities = { protocol_version: 1, private_stdin: true, independent_profile: true };
