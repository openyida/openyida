...
function failure(reason, extra = {}) {
  return new CliError(`DWS sync failed: ${reason}`, {
    code: 'DWS_SYNC_FAILED', details: { reason, ...extra },
  });
}

function rateLimitFailure(response, headers = {}) {
  const data = response && (response.data || response.content || response);
  if (!data || !['dws_rate_limited', 'dws_rate_limit_unavailable'].includes(data.status)) { return null; }
  const delay = Number(data.retry_after ?? headers['retry-after']);
  return failure(data.status, { retry_after: Number.isInteger(delay) && delay > 0 && delay <= 60 ? delay : 1 });
}
...
async function syncDws(options = {}) {
  ...
  try {
    ...
    throw rateLimitFailure(error && error.payload, (error && error.headers) || {}) || failure('EXCHANGE_REQUEST_FAILED');
  } finally {
    delete handoff.accessToken;
  }
  const limited = rateLimitFailure(response);
