...
async function switchOrganization(targetCorpId, options = {}) {
  ...
  const originalPointer = loadAuthProfilePointer(options);
  ...
  if (targetResolution.session && targetResolution.session.corp_id === targetCorpId) {
    if (targetResolution.session.auth_profile) {
      saveAuthProfilePointer(targetResolution.session, options);
    }
  ...
  const loginResult = await login(loginOptions);
  if (!loginResult || !loginResult.access_token) {
    restoreAuthProfilePointer(originalPointer, options);
    return {
      ...(loginResult || {}),
      ok: false,
      auth_mode: 'token',
