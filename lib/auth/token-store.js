...
function normalizeTokenSession(session = {}) {
  ...
  const normalized = {
    ...
    auth_base_url: session.auth_base_url,
...
function saveDwsTokenSession(session, options = {}) {
  const normalized = normalizeTokenSession(session);
  const identity = buildAuthProfileIdentity(normalized);
  if (!identity || normalized.credential_source !== 'dws' || !normalized.session_id
      || !normalized.auth_base_url || !normalized.refresh_token) {
    throw new Error('dws_profile_incomplete');
  }
  // Isolate independent login and each server-side DWS session, including same user/corp.
  const authProfile = crypto.createHash('sha256').update(JSON.stringify({
    ...identity, credential_source: 'dws', session_id: normalized.session_id,
    auth_base_url: normalized.auth_base_url,
  })).digest('hex').slice(0, 32);
  const saved = normalizeTokenSession({ ...normalized, auth_profile: authProfile,
    auth_source: 'user_profile', auth_store: 'user', persistence_scope: 'user' });
  const file = getUserProfileFilePath(authProfile, options);
  const previous = fs.existsSync(file) ? JSON.parse(fs.readFileSync(file, 'utf8')) : null;
  const previousPointer = loadAuthProfilePointer(options);
  try {
    writeJsonFileAtomic(file, saved);
    const readback = loadUserProfileFile(authProfile, options);
    if (!readback || readback.access_token !== saved.access_token || readback.refresh_token !== saved.refresh_token) {
      throw new Error('dws_profile_readback_failed');
    }
    // Refresh a pinned running command's session without switching a newer project binding.
    if (options.bindProject !== false) {
      saveAuthProfilePointer(saved, options);
      const effective = loadTokenSession(options);
      if (!effective || effective.auth_profile !== authProfile || effective.access_token !== saved.access_token) {
        throw new Error('dws_binding_readback_failed');
      }
    }
    return saved;
  } catch (error) {
    const current = loadAuthProfilePointer(options);
    if (current && current.auth_profile === authProfile) {
      if (previousPointer) { writeJsonFileAtomic(getAuthProfilePointerFilePath(options), previousPointer); }
      else { clearAuthProfilePointer(options); }
    }
    if (previous) { writeJsonFileAtomic(file, previous); }
