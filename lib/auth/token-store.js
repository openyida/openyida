...
function deleteUserAuthProfile(authProfile, options = {}) {
  ...
  let deleted = false;
  // Persist invalidation before removal so an in-flight refresh cannot
  // resurrect a deleted profile, including a later login with the same ID.
  writeJsonFileAtomic(path.join(resolveUserAuthDir(options), 'revocations', `${normalizedAuthProfile}.json`), {
...
function saveUserTokenSession(session, options = {}) {
  ...
  const profileSession = normalizeTokenSession({
    ...
    auth_generation: crypto.randomBytes(16).toString('hex'),
  ...
  let pointerWarning;
  try {
    if (options.persistProfilePointer !== false) {saveAuthProfilePointer(profileSession, options);}
  } catch (error) {
    pointerWarning = `project_auth_pointer_unavailable: ${error.message}`;
  }
  if (options.persistProfilePointer !== false) {clearProjectLegacyTokenSession(options);}
  if (pointerWarning) {
...
function authProfileRevision(authProfile, options = {}) {
  const session = loadUserProfileFile(authProfile, options);
  if (!session) {return null;}
  let revocation;
  try {
    revocation = fs.readFileSync(path.join(resolveUserAuthDir(options), 'revocations', `${sanitizeProfileName(authProfile)}.json`), 'utf8');
  } catch (error) {
    if (error.code !== 'ENOENT') {throw error;}
    revocation = '';
  }
  return crypto.createHash('sha256').update(JSON.stringify([
    session.auth_generation, session.access_token, session.refresh_token, revocation,
  ])).digest('hex');
}
