...
const { getLanguage, t } = require('../core/i18n');
...
function responseHtml(title, body, options = {}) {
  const {
    ...
    fallbackDelayMs = 900,
    closingHint = t('login.oauth_closing'),
    staticHint = t('login.oauth_manual_close'),
  ...
  const escapeHtml = (value) => String(value)
    ...
    .replace(/"/g, '&quot;')
    .replace(/'/g, '&#39;');
  const safeLanguage = /^[a-z]{2}(?:-[A-Z]{2})?$/.test(language) ? language : 'en';
  const direction = safeLanguage === 'ar' ? 'rtl' : 'ltr';
  const delay = Math.max(0, Number(autoCloseDelayMs) || 0);
  const fallbackDelay = Math.max(0, Number(fallbackDelayMs) || 0);

  const closeScript = autoClose ? `
    <script>
      (function() {
        const delay = ${delay};
        const fallbackDelay = ${fallbackDelay};
        ...
        function showManualClose() {
          if (closingStatus) {
            closingStatus.hidden = true;
          }
          if (manualClose) {
            manualClose.hidden = false;
          }
        }
        ...
        setTimeout(showManualClose, delay + fallbackDelay);
      })();
    </script>
  ` : '';

  const statusContent = autoClose
    ? `<div id="closing-status" class="status-line" role="status">
        <span class="spinner" aria-hidden="true"></span>
        <span>${escapeHtml(closingHint)}</span>
      </div>
      <div id="manual-close" class="manual-close" role="status" hidden>
        ${escapeHtml(staticHint)}
      </div>
      <noscript><div class="manual-close">${escapeHtml(staticHint)}</div></noscript>`
    : `<div class="manual-close" role="status">${escapeHtml(staticHint)}</div>`;
  const tone = autoClose ? 'success' : 'error';
  const statusIcon = autoClose
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    body {
      ...
      color: #172033;
      background:
        radial-gradient(circle at 15% 10%, rgba(37, 99, 235, .12), transparent 34%),
        radial-gradient(circle at 85% 90%, rgba(14, 165, 233, .10), transparent 32%),
        #f5f8fc;
    }
    .card {
      width: min(100%, 520px);
    ...
    .brand { margin: 0 0 28px; color: #526079; font-size: 14px; font-weight: 700; letter-spacing: .08em; }
    .icon {
      width: 72px;
      height: 72px;
      margin: 0 auto 24px;
    ...
    .error .icon { background: linear-gradient(135deg, #dc2626, #f97316); box-shadow: 0 12px 28px rgba(220, 38, 38, .20); }
    .icon svg { width: 36px; height: 36px; fill: none; stroke: currentColor; stroke-width: 2.2; stroke-linecap: round; stroke-linejoin: round; }
    h1 { margin: 0; color: #111827; font-size: clamp(28px, 6vw, 36px); line-height: 1.2; letter-spacing: -.025em; }
    .message { margin: 14px auto 0; max-width: 400px; color: #64748b; font-size: 16px; line-height: 1.7; }
    .status-area { min-height: 68px; margin-top: 28px; display: grid; align-items: center; }
    .status-line { display: flex; align-items: center; justify-content: center; gap: 10px; color: #475569; font-size: 14px; }
    .spinner { width: 16px; height: 16px; border: 2px solid #bfdbfe; border-top-color: #2563eb; border-radius: 50%; animation: spin .8s linear infinite; }
    .manual-close { padding: 14px 16px; color: #334155; font-size: 14px; line-height: 1.6; background: #f1f5f9; border-radius: 12px; }
    [hidden] { display: none !important; }
    @keyframes spin { to { transform: rotate(360deg); } }
    @media (max-width: 520px) { .card { padding: 36px 24px 28px; border-radius: 20px; } }
    @media (prefers-reduced-motion: reduce) { .spinner { animation: none; } }
  </style>
</head>
<body>
  <main class="card ${tone}" aria-labelledby="page-title">
    <p class="brand">OpenYida</p>
    <div class="icon" aria-hidden="true">
      <svg viewBox="0 0 24 24">${statusIcon}</svg>
    </div>
    <h1 id="page-title">${escapeHtml(title)}</h1>
    <p class="message">${escapeHtml(body)}</p>
    <div class="status-area">${statusContent}</div>
  </main>
  ${closeScript}
</body>
</html>`;
...
function runDingtalkLoopback(options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const server = http.createServer((req, res) => {
      ...
      // The loopback server is single-use. Do not keep the callback socket alive
      // after sending the page, so the listener can shut down immediately.
      ...
      if (!receivedState || receivedState !== state) {
        ...
        res.end(responseHtml(
          t('login.oauth_failure_title'),
          t('login.oauth_invalid_state')
        ));
      ...
      res.setHeader('content-type', 'text/html; charset=utf-8');
      res.end(responseHtml(
        t('login.oauth_success_title'),
        t('login.oauth_success_message'),
        {
          autoClose: true,
        }
      ));
