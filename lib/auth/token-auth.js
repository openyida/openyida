...
function requestJson(method, url, body, headers = {}, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (options.timeoutMs > 0) {
      const timer = setTimeout(() => request.destroy(new Error('request_timeout')), options.timeoutMs);
...
function normalizeTokenResponse(response, baseUrl, clientId) {
  ...
  return {
    ...
    corp_name: normalizeCorpName(data),
...
function tokenStatus(options = {}) {
  const resolution = resolveTokenSession(options);
  ...
  if (session.credential_source === 'dws'
      && (!Number.isInteger(session.session_expires_at) || session.session_expires_at <= Date.now() / 1000)) {
    return {
      ok: false, status: 'expired', can_auto_use: false, auth_mode: 'token',
      auth_source: session.auth_source, auth_store: session.auth_store, auth_profile: session.auth_profile,
      base_url: session.base_url, credential_source: 'dws', environment: session.environment,
      session_expires_at: session.session_expires_at, corp_id: session.corp_id, user_id: session.user_id,
      failure_reason: 'dws_session_expired',
    };
  }
...
async function tokenRefresh(options = {}) {
  ...
  const authBaseUrlValue = (session.credential_source === 'dws' && session.auth_base_url) || resolveTokenBaseUrl({
  ...
  if (session.credential_source === 'dws') {
    try {
      response = await requestJson('POST', appendPath(authBaseUrl, '/refresh'), {
        refreshToken: session.refresh_token, clientId,
      }, {}, { timeoutMs: 30000, maxResponseBytes: 65536 });
      requireOkResponse(response);
    } catch (error) {
      if (isRefreshAuthRequired(error)) {
        throw createTokenAuthRequiredError('invalid_refresh_token');
      }
      throw createTokenRefreshFailedError('refresh_request_failed');
    }
  } else {
    response = await requestJson('POST', appendPath(authBaseUrl, '/refresh'), {
      refreshToken: session.refresh_token, clientId,
  ...
  const normalized = normalizeTokenResponse({
    ...
    corp_name: data.corp_name || data.corpName || data.name || session.corp_name,
  ...
  if (!normalized.access_token) {
    if (session.credential_source === 'dws') {
      throw createTokenRefreshFailedError('refresh_response_invalid');
    }
    return {
      ...normalized,
      ok: false,
  ...
  if (session.credential_source === 'dws') {
    if (normalized.corp_id !== session.corp_id || normalized.user_id !== session.user_id
        || data.client_id !== session.client_id || data.credential_source !== 'dws'
        || data.session_id !== session.session_id || data.session_expires_at !== session.session_expires_at
        || data.environment !== session.environment || !data.refresh_token
        || normalized.base_url !== session.base_url
        || !Number.isInteger(data.expires_in) || data.expires_in <= 0
        || data.expires_in > session.session_expires_at - Date.now() / 1000 + 2) {
      throw createTokenAuthRequiredError('dws_refresh_identity_mismatch');
    }
    delete normalized.raw;
    delete normalized.message;
    return saveDwsTokenSession({ ...normalized, credential_source: 'dws',
      auth_base_url: session.auth_base_url, session_id: session.session_id,
      session_expires_at: session.session_expires_at, environment: session.environment,
    }, { ...options, bindProject: false });
  }
...
async function revokeTokenSession(session) {
  ...
  try {
    await requestJson('POST', appendPath(appendPath(session.auth_base_url || session.base_url, DEFAULT_AUTH_PATH_PREFIX), '/logout'), {
