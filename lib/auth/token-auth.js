...
function requestJson(method, url, body, headers = {}, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const socketTimeoutMs = positiveNumber(options.timeoutMs, 0);
    let timeoutId = null;
    let settled = false;
    ...
    if (socketTimeoutMs > 0) {
      // Node's socket timeout is reset by network activity, so it cannot enforce
      // the caller's wall-clock budget. Use an independent timer that also
      // covers response bodies which trickle data without ever completing.
