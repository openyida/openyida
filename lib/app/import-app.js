...
async function readSchemaRevision(appType, formUuid, authRef) {
  const result = await requestWithAutoLogin((auth) => httpGet(
    auth.baseUrl,
  ...
  if (!result || result.success === false) {
    throw new Error(t('import.schema_empty_msg'));
  }
  return requireSchemaServerRevision(result);
}

async function saveFormSchema(appType, formUuid, schema, authRef, formType = 'receipt', serverRevision) {
  assertSchemaWriteReady(authRef);
  const payload = {
    _csrf_token: authRef.csrfToken,
    appType,
    formUuid,
    ...
    schemaVersion: 'V5',
    gmtModified: requireSchemaServerRevision({ gmtModified: serverRevision }),
  };
...
function assertSchemaWriteReady(authRef) {
  if (
    !authRef ||
    ...
    const error = new Error('Import Schema write authentication is not ready.');
    error.code = 'IMPORT_SCHEMA_WRITE_PRECHECK_FAILED';
    throw error;
  }
}
...
async function run(args) {
  ...
  for (const form of forms) {
    ...
    // 4.3 保存 Schema
    const serverRevision = await readSchemaRevision(newAppType, newFormUuid, authRef);
    const saveResult = await saveFormSchema(
      newAppType,
      newFormUuid,
      adaptedSchema,
      authRef,
