...
const { warn, info, step, success } = require('../core/chalk');
...
async function run(args) {
  ...
  const filteredArgs = args.filter(arg => !['--skip-lint', '--compat', '--modern', '--json', '--canvas'].includes(arg));
  ...
  const isCanvas = resolvePageCompilerMode(sourcePath, { forceCanvas: canvas }) === PAGE_COMPILER_MODE_CANVAS;
  if (isCanvas) {
    const result = compileCanvasLocal(initialSourceCode, { sourcePath });
    const output = {
      success: true,
      mode: 'canvas',
      sourcePath,
      runtimeCodeBytes: Buffer.byteLength(result.runtimeCode, 'utf8'),
      importedModules: JSON.parse(result.importedModules),
    };
    if (json) {
      console.log(JSON.stringify(output, null, 2));
    } else {
      success(t('publish.canvas_compile_done'));
      info(`  importedModules: ${output.importedModules.join(', ') || '(none)'}`);
    }
    return output;
  }
