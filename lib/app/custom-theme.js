...
const { resolveBearerAuthHeaders } = require('../core/utils');
const { readManagedContext, managedError } = require('../agent/managed-context');
...
function validateThemeCssContent(cssText) {
  ...
  // Check the original text before stripping comments or rewriting token scopes.
  const issue = findCssStructureIssue(source);
  if (issue) {
    const line = source.slice(0, issue.index).split('\n').length;
    throw new CliError(t('design_document.theme_css_invalid', line), {
      code: 'THEME_CSS_STRUCTURE_INVALID', details: { line, ...issue },
    });
  }
...
async function uploadCustomThemeFile(appType, filePath, authRef, options = {}) {
  const managed = readManagedContext(process.env);
  if (managed && appType !== managed.appType) {
    throw managedError('MANAGED_APP_MISMATCH', 'managed_app_mismatch');
  }
  if (managed && new URL(authRef.baseUrl).origin !== new URL(managed.baseUrl).origin) {
  ...
  const auth = await resolveBearerAuthHeaders({ projectRoot: authRef && authRef.projectRoot });
  ...
  const response = await fetchImpl(endpoint, {
    method: 'POST',
    // Never forward a multipart request carrying credentials to another URL.
    redirect: 'error',
    headers: {
