 ...
 * 3. 读取本地 .cache/cookies.json，在写前确认登录态
 * 4. 通过单次 HTTP POST 调用 saveFormSchema；发送后不自动登录或重试
 */
...
async function main(argv) {
  const { appType, formUuid, sourceFile, skipLint, healthCheck, compat, force, canvas: canvasFlag, browserOpenMode, legacyGuardOptions } = parseArgs(argv);
  assertLegacyDirectWriteAllowed({
    command: 'publish',
    resourceType: 'page',
    action: 'update',
  ...
  let csrfToken = authRef.csrfToken;
  ...
  if (configResponse && configResponse.__csrfExpired) {
    const cookieData = refreshCsrfToken();
    csrfToken = cookieData.csrf_token;
    cookies = cookieData.cookies;
    baseUrl = authRef.baseUrl;
    authRef.csrfToken = csrfToken;
    authRef.cookies = cookies;
    authRef.cookieData = cookieData;
