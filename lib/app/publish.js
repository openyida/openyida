...
const { buildDisplayPageEntryUrls } = require('./application-entry-urls');
...
function parseArgs(argv = process.argv.slice(2)) {
  ...
  const strictTheme = args.includes('--strict-theme');
  const autoNavOrder = args.includes('--auto-nav-order');
  const help = args.includes('--help') || args.includes('-h');
  const filteredArgs = args.filter(arg => arg !== '--skip-lint' && arg !== '--health-check' && arg !== '--check' && arg !== '--compat' && arg !== '--modern' && arg !== '--force' && arg !== '--canvas' && arg !== '--allow-fixed-brand' && arg !== '--fix-theme' && arg !== '--strict-theme' && arg !== '--auto-nav-order' && arg !== '--json' && arg !== '--help' && arg !== '-h');
  ...
  if (filteredArgs.length !== 3 || filteredArgs.some(arg => arg.startsWith('--')) ||
      (allowFixedBrand && (fixTheme || strictTheme))) {
    throw new CliError(t('publish.usage'), { code: 'PUBLISH_INVALID_ARGUMENTS' });
  }
...
// Migration is opt-in; publishing never waits for stdin, including agent PTYs.
function prepareFixedBrandThemeMigration(sourcePath, source) {
  const { transformFixedBrandTheme } = require('./fix-theme');
  const migration = transformFixedBrandTheme(source);
  if (migration.changed) {
    // Removing a local override must leave an assembled, mounted theme provider.
    require('./canvas-theme-guard').assertCanvasThemeStructure(source, {
      sourcePath, requireThemeProvider: true, allowFixedBrand: true, onThemeWarning() {},
    });
  }
  return migration;
}

function saveThemeMigration(sourcePath, original, output) {
  if (loadPublishSource(sourcePath).source !== original) {
    throw new CliError(t('publish.fix_theme_source_changed'), {
      code: 'OPENYIDA_CANVAS_THEME_SOURCE_CHANGED', details: { sourcePath, sideEffectState: 'none', retryable: false },
    });
  }
  const temporary = `${sourcePath}.${require('crypto').randomUUID()}.tmp`;
  try {
    fs.writeFileSync(temporary, output, { flag: 'wx', mode: fs.statSync(sourcePath).mode });
...
async function runMain(argv) {
  ...
  const { appType, formUuid, sourceFile, skipLint, healthCheck, compat, force, canvas: canvasFlag, allowFixedBrand, fixTheme, strictTheme, autoNavOrder, browserOpenMode } = options;
  ...
  if (!isCanvas && (fixTheme || strictTheme || allowFixedBrand)) {
    throw new CliError(t('publish.theme_canvas_only'), { code: 'PUBLISH_INVALID_ARGUMENTS' });
  }
  ...
  if (isCanvas) {
    ...
    let migration;
    try {
      migration = fixTheme ? prepareFixedBrandThemeMigration(sourcePath, initialSourceCode) : null;
      if (migration?.changed) { sourceCode = migration.output; }
      canvasResult = await compileCanvas(sourceCode, { sourcePath, allowFixedBrand, strictBrand: strictTheme });
    } catch (canvasCompileError) {
      throw require('../core/source-repair').withSourceRepair(
        buildCanvasCompileCliError(canvasCompileError, sourcePath), [sourcePath], argv);
    }
    if (migration?.changed) { saveThemeMigration(sourcePath, initialSourceCode, sourceCode); }
  ...
  try {
    const configResponse = await fetchFormSchemaInfo(authRef, appType, savedFormUuid);
    if (!configResponse || configResponse.success === false || configResponse.__needLogin || configResponse.__csrfExpired) {
      throw new Error(configResponse?.errorMsg || t('common.request_failed'));
    }
    pageConfig = configResponse.content || configResponse.result || configResponse.data || {};
  } catch (configError) {
    navigationWarning = configError.message || t('common.request_failed');
  }
  const entryUrls = buildDisplayPageEntryUrls({
    appType, formUuid: savedFormUuid, baseUrl: authRef.baseUrl || baseUrl,
    config: pageConfig, formType: 'display',
  });
  const pageUrl = entryUrls.url || `${baseUrl}/${appType}/workbench/${savedFormUuid}`;
