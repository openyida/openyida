...
function shouldAutoOrderNavigation(isCanvas, requestedAutoNavOrder) {
  return isCanvas || requestedAutoNavOrder === true;
}
...
async function ensurePublishTargetOrExit(appType, formUuid, authRef, options = {}) {
  if (options.force) {
    ...
    try {
      const forms = await fetchFormPageList(appType, authRef);
      return { ok: true, skipped: true, forms };
    } catch (targetError) {
      return { ok: true, skipped: true, forms: [], error: targetError };
    }
  }
...
async function verifyCanvasSourceBindings(sourceCode, appType, forms, authRef) {
  const contract = extractCanvasBindingContract(sourceCode);
  if (!contract.present) {
    return { valid: true, skipped: true, reason: 'no_binding_contract' };
  }
  const formUuids = contract.formUuids && typeof contract.formUuids === 'object' ? contract.formUuids : {};
  const knownFormUuids = new Set((forms || []).map(function (form) {return String(form && form.formUuid || '');}));
  const schemasByFormUuid = {};
  const schemaTargets = Array.from(new Set(Object.keys(formUuids)
    .map(function (formKey) {return String(formUuids[formKey] || '');})
    .filter(function (formUuid) {return formUuid && knownFormUuids.has(formUuid);})));
  await Promise.all(schemaTargets.map(async function (bindingFormUuid) {
    const existing = await fetchExistingSchemaContent(appType, bindingFormUuid, authRef);
    schemasByFormUuid[bindingFormUuid] = existing.schema;
  }));
  const verification = validateCanvasBindingContract(contract, {
    appType,
    forms,
    schemasByFormUuid,
  });
  if (!verification.valid) {
    throw new CliError('Code Canvas 资源绑定校验失败；formUuid 和 fieldId 必须来自当前应用的真实 Schema。', {
      code: 'CANVAS_BINDING_VALIDATION_FAILED',
...
async function runMain(argv) {
  ...
  const { appType, formUuid, sourceFile, skipLint, healthCheck, compat, force, canvas: canvasFlag, autoNavOrder: requestedAutoNavOrder, browserOpenMode } = options;
  ...
  const isCanvas = resolvePageCompilerMode(sourcePath, {
    forceCanvas: canvasFlag,
  }) === PAGE_COMPILER_MODE_CANVAS;
  const autoNavOrder = shouldAutoOrderNavigation(isCanvas, requestedAutoNavOrder);
  ...
  const targetCheck = await ensurePublishTargetOrExit(appType, formUuid, authRef, { force });
  cookies = authRef.cookies;
  baseUrl = authRef.baseUrl;

  let schemaContent;
  let serverRevision;
  let canvasBindingEvidence = null;
  if (isCanvas) {
    try {
      canvasBindingEvidence = await verifyCanvasSourceBindings(sourceCode, appType, targetCheck && targetCheck.forms || [], authRef);
  ...
  if (healthCheck) {
    ...
    } else if (healthCheckResult.skipped) {
      info(t('publish.health_check_skipped', healthCheckResult.reason || t('common.unknown_error')));
    } else {
  ...
  if (autoNavOrder) {
    ...
    try {
      ...
      if (isCanvas) {
        throw new CliError(t('publish.nav_order_required_failed', navOrderWarning), {
  ...
  if (configResponse && configResponse.success) {
    ...
    console.log(JSON.stringify(withBrowserHandoff(
      { success: true, appType, formUuid: savedFormUuid, version, url: pageUrl, canvasBindingEvidence, healthCheck: healthCheckResult, navOrder: navOrderResult, navOrderWarning },
      pageUrl,
    ...
    console.log(JSON.stringify(withBrowserHandoff(
      { success: true, appType, formUuid: savedFormUuid, version, url: pageUrl, configWarning: configErrorMsg, canvasBindingEvidence, healthCheck: healthCheckResult, navOrder: navOrderResult, navOrderWarning },
      pageUrl,
      { stage: 'publish_page_success', title: savedFormUuid },
      browserOpenMode
    )));
...
if (require.main === module) {
  ...
  module.exports.shouldAutoOrderNavigation = shouldAutoOrderNavigation;
  ...
  module.exports.ensurePublishTargetOrExit = ensurePublishTargetOrExit;
  module.exports.verifyCanvasSourceBindings = verifyCanvasSourceBindings;
