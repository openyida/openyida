...
const { redactSensitive } = require('../../core/redact');
...
function execute(args) {
  return new Promise((resolve, reject) => {
    const flags = ['--json', '--quiet'].filter(flag => !args.includes(flag));
    execFile(process.execPath, [path.resolve(__dirname, '../../../bin/yida.js'), ...args, ...flags], {
      ...
      if (error || !output || typeof output !== 'object' || output.success === false) {
        const failure = parseOutput(stderr);
        const source = failure?.success === false ? failure : output;
        // Child errors are written to stderr. Keep only bounded, redacted
        // diagnostics and a known created ID, never Node's argv-bearing error.
        let message = String(source?.errorMsg || source?.error || stderr || 'Missing command result');
        const grant = process.env.OPENYIDA_AGENT_TASK_GRANT;
        if (grant) { message = message.split(grant).join('***'); }
        // Diagnostics may contain embedded or incomplete JSON, not a plain
        // object. Reuse the shared sensitive-key rules, then discard the tail
        // rather than attempting to recover a possibly nested secret value.
        for (const match of message.matchAll(/["']([^"'\r\n]{1,128})["']\s*:/g)) {
          let key;
          try { key = JSON.parse(`"${match[1]}"`); } catch { key = match[1].replace(/\\/g, ''); }
          if (redactSensitive({ [key]: 'sensitive-value' })[key] !== 'sensitive-value') {
            message = `${message.slice(0, match.index)}[REDACTED]`;
            break;
          }
        }
        message = redactSensitive(message).slice(0, 2048);
        const errorCode = /^[A-Z][A-Z0-9_]{0,127}$/.test(source?.errorCode || error?.code || '')
          ? source?.errorCode || error.code : undefined;
        const formUuid = [source?.formUuid, source?.details?.formUuid, output?.formUuid, output?.details?.formUuid]
          .find(value => typeof value === 'string' && /^FORM[a-zA-Z0-9_-]{1,128}$/.test(value));
        const safeOutput = { success: false, errorMsg: message,
          ...(errorCode ? { errorCode } : {}), ...(formUuid ? { formUuid } : {}) };
        reject(Object.assign(new Error(message), { output: safeOutput }));
      } else { resolve(output); }
    });
  });
}
      ...
      for (const form of forms) {
        ...
        const task = Promise.resolve().then(() => worker(form)).then(output => {
          ...
          results[form.key] = { ...results[form.key], status: 'failed', error: error.message,
            ...(error.output?.errorCode ? { errorCode: error.output.errorCode } : {}),
...
module.exports = { run, parseArgs, loadPlan, mapReferences, schedule, parseOutput, execute };
