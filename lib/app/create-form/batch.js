...
function execute(args, dependencies = {}) {
  ...
  return new Promise((resolve, reject) => {
    runFile(process.execPath, [path.resolve(__dirname, '../../../bin/yida.js'), ...args, '--quiet'], {
      env: { ...process.env, YIDA_QUIET: '1' }, timeout: 180000, maxBuffer: 8 * 1024 * 1024,
    }, (error, stdout, stderr) => {
      const output = parseOutput(stdout) || parseOutput(stderr);
      if (error || !output || typeof output !== 'object' || output.success === false) {
        const stderrText = String(stderr || '').trim();
        reject(Object.assign(new Error(
          output?.errorMsg || output?.error || output?.message || stderrText || error?.message || 'Missing command result',
        ), { output }));
      } else { resolve(output); }
    });
  });
