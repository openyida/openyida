...
function execute(args, { execFile: execFileImpl = execFile } = {}) {
  return new Promise((resolve, reject) => {
    ...
    execFileImpl(process.execPath, [path.resolve(__dirname, '../../../bin/yida.js'), ...args, ...flags], {
      ...
      if (error || !output || typeof output !== 'object' || output.success === false) {
        ...
        let message = String(source?.errorMsg || source?.error || source?.message || stderr || 'Missing command result');
