...
function isPlaceholderFieldId(value) {
  const text = String(value || '').trim();
  return /(?:xxx|todo|placeholder|your_|字段ID|fieldid)/i.test(text);
}
...
function validateCanvasBindingContract(contract, context) {
  ...
  Object.keys(normalized.fieldsByForm).forEach(function (formKey) {
    ...
    Object.keys(normalized.fieldsByForm[formKey] || {}).forEach(function (fieldKey) {
      ...
      } else if (isPlaceholderFieldId(fieldId)) {
        errors.push({
...
module.exports = {
  ...
  _private: { isPlaceholderFieldId, levenshtein, nearestCandidates, normalizeFieldsByForm, readStaticValue },
};
