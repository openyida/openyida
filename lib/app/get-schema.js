    ...
    if (FIELD_COMPONENT_NAMES.has(node.componentName)) {
      ...
      if (fieldId) {
        ...
        const complexDefault = props.complexValue && typeof props.complexValue === 'object'
          ...
          : undefined;
        // Custom text defaults are rendered from props.value; complexValue may
        // retain an obsolete empty value after a targeted field update.
        const customText = ['TextField', 'TextareaField'].includes(node.componentName) && props.valueType === 'custom';
        const defaultValue = firstPresent(customText
          ? [props.value, complexDefault, props.defaultValue, props.default]
          : [complexDefault, props.defaultValue, props.value, props.default]);
