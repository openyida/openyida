...
function normalizeDividerType(type) {
  ...
  if (!DIVIDER_TYPES.includes(normalized)) {
    throwCreateFormError(t('create_form.divider_type_invalid', normalized, DIVIDER_TYPES.join(', ')), 'CREATE_FORM_INVALID_DIVIDER_TYPE', {
      actual: type, expected: DIVIDER_TYPES,
    });
  }
  return normalized;
}
