 ...
 *     { "type": "SelectField", "label": "部门", "dataSource": ["技术部", "产品部"] },
...
const formSchemaBuilder = require('./scaffolds/form/form-schema-builder');
...
function mapFieldValidationCode(code) {
  const cliErrorCodesByValidationCode = {
    ...
    DIVIDER_TITLE_MISSING: 'CREATE_FORM_DIVIDER_TITLE_MISSING',
    ROOT_DIVIDER_REQUIRED: 'CREATE_FORM_ROOT_DIVIDER_REQUIRED',
    SEPARATE_DIVIDERS_UNSUPPORTED: 'CREATE_FORM_SEPARATE_DIVIDERS_UNSUPPORTED',
    TRAILING_DIVIDER_UNSUPPORTED: 'CREATE_FORM_TRAILING_DIVIDER_UNSUPPORTED',
    ADJACENT_DIVIDERS_UNSUPPORTED: 'CREATE_FORM_ADJACENT_DIVIDERS_UNSUPPORTED',
  };
...
function validateFormFieldDefinitions(fields, rootPath, options) {
  const diagnostics = collectFormFieldValidationDiagnostics(fields, Object.assign({}, options, {
...
function collectFieldCandidates(fields, tableAncestors, query) {
  ...
  const refs = collectFieldReferences(fields).map(function (ref) {
...
function extractSchemaFromReadback(schemaResult) {
  let content = schemaResult && schemaResult.content;
  if (typeof content === 'string') {
    try {
      content = JSON.parse(content);
    } catch (error) {
      throwCreateFormError('Saved form schema readback returned invalid JSON.', 'CREATE_FORM_SCAFFOLD_READBACK_INVALID', {
        parseError: error.message,
      });
    }
  }
  if (content && content.schema) {
    content = typeof content.schema === 'string' ? JSON.parse(content.schema) : content.schema;
  }
  ...
  throwCreateFormError('Saved form schema readback did not include a complete schema.', 'CREATE_FORM_SCAFFOLD_READBACK_INVALID', {
    responseKeys: schemaResult && typeof schemaResult === 'object' ? Object.keys(schemaResult) : [],
    contentKeys: content && typeof content === 'object' ? Object.keys(content) : [],
  });
}

async function readBackSavedFormScaffold(authRef, appType, formUuid) {
  ...
  if (!schemaResult || schemaResult.success === false || schemaResult.__needLogin || schemaResult.__csrfExpired) {
    throwCreateFormError('Saved form schema readback failed.', 'CREATE_FORM_SCAFFOLD_READBACK_FAILED', {
      appType,
      formUuid,
      result: sanitizeFailureResult(schemaResult),
    });
  }
  const revision = requireSchemaServerRevision(extractSchemaServerRevision(schemaResult), { appType, formUuid, stage: 'postSaveReadback' });
  const schema = extractSchemaFromReadback(schemaResult);
  let scaffold;
  try {
    scaffold = formSchemaBuilder.validateNativeFormScaffold(schema);
  } catch (error) {
    throwCreateFormError('Saved form schema is missing required scaffold capabilities.', 'CREATE_FORM_SCAFFOLD_READBACK_INCOMPLETE', {
      appType,
      formUuid,
      revision,
  ...
  return Object.assign({ confirmed: true, revision }, scaffold);
}
...
function warnPostSaveUpdateConfigFailure(configResult) {
  ...
  if (isRetryablePostSaveUpdateConfigResult(configResult)) {
    warn(t('create_form.config_pending', warningPayload.configWarning));
    hint(t('create_form.schema_ok_config_pending'));
  } else {
...
// 返回 { saveResult, configResult, scaffoldReadback }。
...
async function saveSchemaAndUpdateConfig(authRef, appType, formUuid, schema, version, stepOffset, failureContext) {
  ...
  formSchemaBuilder.ensureDefaultPresentation(schema);
  ...
  const scaffoldReadback = await readBackSavedFormScaffold(authRef, appType, formUuid);

  return { saveResult: saveResult, configResult: configResult, scaffoldReadback: scaffoldReadback };
}
...
async function mainValidateFields(parsedArgs) {
  ...
  const { definition, fields } = readFieldsDefinition(fieldsJsonOrFile);
  ...
  const output = {
    ...
    fieldCount: countDataFieldDefinitions(fields),
    diagnostics,
  };
...
async function mainCreate(parsedArgs, authRef) {
  ...
  assertNoEmojiInDefinitionFileName(fieldsJsonOrFile);
  const { definition, fields: inputFields, columns, validations } = readFieldsDefinition(fieldsJsonOrFile);
  validateFormFieldDefinitions(inputFields);
  const fields = ensureSemanticDividerFields(inputFields);
  assertNoEmojiInFormDefinition(formTitle, fields, validations);
  validateFormFieldDefinitions(fields, 'fields', {
    definition,
    requireSemanticDividers: true,
  });
  ...
  try {
    ...
    const schema = formSchemaBuilder.buildFormSchema(
      formTitle,
      fields,
      formUuid,
      corpId,
      ...
      definition.labelAlign || labelAlign,
      {
        themeTokens: definition.themeTokens || {},
        formDetailPreset: definition.formDetailPreset,
        validations,
        rules: definition.rules || [],
        dataSources: definition.dataSources || [],
      }
    );
    const createValidationRules = collectSmartValidationRulesFromFields(fields);
    ...
    scaffoldReadback = saveAndConfigResult.scaffoldReadback;
  } catch (err) {
  ...
  if (configResult && configResult.success) {
    ...
    console.log(JSON.stringify(withBrowserHandoff(
      { success: true, formUuid, formTitle, appType, fieldCount, scaffoldReadback, url: formUrl },
      formUrl,
...
async function createFormForLegacyProcessQuiet(context, input) {
  ...
  const { appType, formTitle, fieldsJsonOrFile, layout, theme, labelAlign } = parsedArgs;
  assertNoEmojiInDefinitionFileName(fieldsJsonOrFile);
  const { definition, fields: inputFields, validations } = readFieldsDefinition(fieldsJsonOrFile);
  validateFormFieldDefinitions(inputFields);
  const fields = ensureSemanticDividerFields(inputFields);
  ...
  const schema = formSchemaBuilder.buildFormSchema(formTitle, fields, formUuid, corpId, appType, layout, theme, labelAlign);
  ...
  const { saveResult, configResult, scaffoldReadback } = await saveSchemaAndUpdateConfig(authRef, appType, formUuid, schema, serverRevision, 4);
  return {
    success: true,
    appType,
    formUuid,
...
async function mainAddOption(parsedArgs, authRef) {
  ...
  // Step 4 & 5: 保存 Schema
  const { configResult, scaffoldReadback } = await saveSchemaAndUpdateConfig(authRef, appType, formUuid, schema, version, 4);
...
async function mainPatch(parsedArgs, authRef) {
  ...
  console.log(JSON.stringify(withBrowserHandoff(
    Object.assign(
      { success: true, formUuid, appType, scaffoldReadback, operationsApplied: appliedOperations.length, operations: appliedOperations, url: formUrl },
      configWarningPayload || {}
    ),
    formUrl,
...
async function mainUpdate(parsedArgs, authRef) {
  ...
  if (!schema.pages || !Array.isArray(schema.pages) || schema.pages.length === 0) {
    ...
    schema = formSchemaBuilder.buildEmptyFormSchema();
  }
  ...
  const { configResult, scaffoldReadback } = await saveSchemaAndUpdateConfig(authRef, appType, formUuid, schema, version, 6);
  ...
  if (configResult && configResult.success) {
    ...
    console.log(JSON.stringify(withBrowserHandoff(
      { success: true, formUuid, appType, scaffoldReadback, changesApplied: appliedChanges.length, changes: appliedChanges, url: formUrl },
      formUrl,
...
module.exports = {
  ...
  parseArgs,
  _private: {
    buildFormSchema: formSchemaBuilder.buildFormSchema,
    ...
    applyDefaultFormDetailStyle: formSchemaBuilder.ensureDefaultPresentation,
    applyFormRules,
