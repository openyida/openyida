...
function buildFieldEvidence(component, tableAncestors) {
  ...
  if (ancestors.length > 0) {
    ...
    evidence.labelPath = ancestors.map(function (item) { return item.label; }).concat(labelText);
  }
...
function collectFieldMatchesByLabelDeep(fields, label, tableAncestors, matches) {
  ...
  fields.forEach(function (field, fieldIndex) {
    ...
    const nextAncestors = field && field.componentName === 'TableField'
      ? ancestors.concat(buildFieldEvidence(field, ancestors))
      : ancestors;
...
function collectFieldCandidates(fields, tableAncestors, query) {
  ...
  function visit(items, ancestors, filterByQuery) {
    if (!Array.isArray(items) || candidates.length >= 8) {
      return;
    }
    items.forEach(function (field) {
      ...
      const evidence = buildFieldEvidence(field, ancestors);
      if (evidence.label || evidence.fieldId) {
        const evidenceLabel = evidence.label || '';
        if (!filterByQuery || !normalizedQuery || evidenceLabel.indexOf(normalizedQuery) !== -1 || normalizedQuery.indexOf(evidenceLabel) !== -1) {
      ...
      const nextAncestors = field && field.componentName === 'TableField'
        ? ancestors.concat(buildFieldEvidence(field, ancestors))
        : ancestors;
      visit(field && field.children, nextAncestors, filterByQuery);
    });
  }
  visit(fields, Array.isArray(tableAncestors) ? tableAncestors : [], true);
  if (candidates.length > 0 || !normalizedQuery) {
    return candidates;
  }
  visit(fields, Array.isArray(tableAncestors) ? tableAncestors : [], false);
  return candidates;
}
...
function applyChangesToSchema(schema, changes) {
  ...
  changes.forEach(function (change, changeIndex) {
    ...
    if (change.action === 'add') {
      ...
      const deleteMatches = collectFieldMatchesByLabelDeep(formFields, change.label);
      if (deleteMatches.length === 1) {
        const deleteResult = deleteMatches[0];
        deleteResult.fields.splice(deleteResult.index, 1);
        success(actionDesc + t('create_form.delete_ok', change.label));
        appliedChanges.push({ action: 'delete', label: change.label, resolved: deleteResult.evidence });
      } else if (deleteMatches.length > 1) {
        warn(actionDesc + t('create_form.field_ambiguous', '', change.label));
        pushResolutionDiagnostic(diagnostics, {
          action: 'delete',
          code: 'CREATE_FORM_FIELD_AMBIGUOUS',
          label: change.label,
          candidates: deleteMatches.map(function (match) { return match.evidence; }),
        });
      } else {
        ...
        pushResolutionDiagnostic(diagnostics, {
          ...
          label: change.label,
          candidates: collectFieldCandidates(formFields, [], change.label),
        });
      }
      ...
      if (change.tableLabel) {
        const tableMatches = collectFieldMatchesByLabelDeep(formFields, change.tableLabel);
        if (tableMatches.length === 0) {
          warn(actionDesc + t('create_form.update_table_not_found', change.tableLabel));
          pushResolutionDiagnostic(diagnostics, {
            action: 'update',
            code: 'CREATE_FORM_TABLE_NOT_FOUND',
            tableLabel: change.tableLabel,
            label: change.label,
            candidates: collectFieldCandidates(formFields, [], change.tableLabel),
          });
          return;
        }
        if (tableMatches.length > 1) {
          warn(actionDesc + t('create_form.field_ambiguous', '', change.tableLabel));
          pushResolutionDiagnostic(diagnostics, {
            action: 'update',
            code: 'CREATE_FORM_TABLE_AMBIGUOUS',
            tableLabel: change.tableLabel,
            label: change.label,
            candidates: tableMatches.map(function (match) { return match.evidence; }),
          });
          return;
        }
        ...
        if (tableComponent.componentName !== 'TableField' || !tableComponent.children) {
          ...
          pushResolutionDiagnostic(diagnostics, {
            action: 'update',
            code: 'CREATE_FORM_TARGET_NOT_TABLE',
            tableLabel: change.tableLabel,
            label: change.label,
            candidates: [tableResult.evidence],
          });
          return;
        }
        searchFields = tableComponent.children;
        locationDesc = t('create_form.in_table', change.tableLabel);
        tableAncestors = [tableResult.evidence];
      }

      const updateMatches = collectFieldMatchesByLabelDeep(searchFields, change.label, tableAncestors);
      if (updateMatches.length === 1) {
        const updateResult = updateMatches[0];
        ...
        appliedChanges.push({
          ...
          resolved: buildFieldEvidence(updateResult.field, tableAncestors),
          updatedProps: summarizeUpdatedProps(updateResult.field, change.changes),
        });
      } else if (updateMatches.length > 1) {
        warn(actionDesc + t('create_form.field_ambiguous', locationDesc, change.label));
        pushResolutionDiagnostic(diagnostics, {
          action: 'update',
          code: 'CREATE_FORM_FIELD_AMBIGUOUS',
          label: change.label,
          tableLabel: change.tableLabel || null,
          candidates: updateMatches.map(function (match) { return match.evidence; }),
        });
      } else {
        ...
        pushResolutionDiagnostic(diagnostics, {
          ...
          label: change.label,
          tableLabel: change.tableLabel || null,
          candidates: collectFieldCandidates(searchFields, tableAncestors, change.label),
        });
      }
