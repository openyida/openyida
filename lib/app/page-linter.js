...
const { isCanvasSourcePath } = require('./page-compiler-routing');
...
function detectMissingCanvasYidaApiBridge(sourceCode, filePath, errors, disableMap) {
  if (!isCanvasSourcePath(filePath)) {
    return;
  }
  const strippedSource = stripCommentsOnly(sourceCode);
  ...
  if (/__OPENYIDA_RUNTIME__/.test(strippedSource) || /__OPENYIDA_YIDA_API__/.test(strippedSource)) {
    return;
  }
...
function isFormDataMemberExpression(expression) {
  if (!expression) {
    return false;
  }
  if (expression.type === 'Identifier' && expression.name === 'formData') {
    return true;
  }
  ...
  if (!expression.computed && property && property.type === 'Identifier') {
    return property.name === 'formData';
  }
  if (expression.computed && property && property.type === 'StringLiteral') {
    return property.value === 'formData';
  }
  return false;
}

function isYidaFieldIdLiteral(value) {
  return /^[a-z][A-Za-z0-9]*Field_[A-Za-z0-9_-]+$/.test(String(value || ''));
}
...
function collectAstLintIssues(sourceCode, errors, warnings, disableMap) {
  ...
  traverse(ast, {
    ...
    MemberExpression(pathRef) {
      ...
      if (!node.computed || !isFormDataMemberExpression(node.object)) {
        return;
      }
      const property = node.property;
      if (!property || property.type !== 'StringLiteral') {
        return;
      }
      if (isYidaFieldIdLiteral(property.value)) {
        return;
      }
      pushIssue(errors, getNodeLine(node), 'formdata-literal-key', t('publish.lint_formdata_literal_key', property.value), disableMap);
    },
    ...
    CallExpression(pathRef) {
      ...
      if (
        ...
        callee.name === 'setError' &&
        !pathRef.scope.hasBinding('setError')
      ) {
        pushIssue(errors, getNodeLine(pathRef.node), 'undefined-seterror', t('publish.lint_undefined_seterror'), disableMap);
      }
