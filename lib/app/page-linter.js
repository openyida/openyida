...
function urlHasExtension(rawUrl, extension) {
  const pathname = getUrlPathname(rawUrl).toLowerCase();
    ...
    CallExpression(pathRef) {
      ...
      if (isThisOrSelfUtilsCall(pathRef.node, 'loadScript')) {
        if (isPublicCdnUrl(url)) {
          pushIssue(warnings, getNodeLine(pathRef.node), 'external-script-public-cdn', t('publish.lint_external_script_public_cdn'), disableMap);
        }
      ...
      if (isThisOrSelfUtilsCall(pathRef.node, 'loadStyleSheet') && urlHasExtension(url, '.js')) {
