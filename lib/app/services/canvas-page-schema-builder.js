...
var OPENYIDA_YIDA_REPORT_API_METHODS = ['queryReportData', 'createReportDataBridge'];
...
var OPENYIDA_KNOWN_YIDA_API_METHODS = OPENYIDA_YIDA_FORM_API_METHODS
  ...
  .concat(OPENYIDA_YIDA_REPORT_API_METHODS);
...
function openyidaCreateYidaApiBridge(context) {
  ...
  function invoke(methodName, params, options) {
    if (methodName === 'createReportDataBridge') {
      return bridge.createReportDataBridge(params);
    }
    if (methodName === 'queryReportData') {
      return bridge.queryReportData(params, options);
    }
  ...
  var bridge = {
    ...
    apiGroups: {
      ...
      report: OPENYIDA_YIDA_REPORT_API_METHODS.slice()
    },
  ...
  // Report bridge creation is synchronous; keep the shared object and lifecycle intact.
  bridge.createReportDataBridge = function(binding) {
    if (!yida || typeof yida.createReportDataBridge !== 'function') {
      throw new Error('this.utils.yida.createReportDataBridge is not available');
    }
    return yida.createReportDataBridge.call(yida, binding);
  };
  // Forward both arguments so AbortSignal and timeout options reach the shared query.
  bridge.queryReportData = function(params, options) {
    if (!yida || typeof yida.queryReportData !== 'function') {
      return Promise.reject(new Error('this.utils.yida.queryReportData is not available'));
    }
    try {
      return Promise.resolve(yida.queryReportData.call(yida, params, options));
    } catch (err) {
      return Promise.reject(err);
    }
  };
...
function buildCanvasPageSchemaObject(sourceCode, runtimeCode, importedModules, formUuid, options = {}) {
  ...
  const schema = {
    ...
    actions: {
      module: {
        compiled: CANVAS_ACTIONS_COMPILED,
        source: CANVAS_ACTIONS_SOURCE,
      },
      type: 'FUNCTION',
