...
const { createReportQueryProtocol } = require('../../report/query-protocol');
const { createCanvasReportRuntime } = require('../../report/canvas-runtime');
const reportQueryErrors = require('../../report/query-errors');

// Only report-consuming pages receive this runtime. Each bridge owns its state and requests.
const CANVAS_REPORT_SOURCE = `var openyidaReportMessages = ${JSON.stringify(reportQueryErrors)};
var openyidaReportLanguage = (window.g_config || {}).locale || (window.navigator || {}).language || 'en';
var openyidaReportProtocol = (${createReportQueryProtocol.toString()})(/^zh/i.test(openyidaReportLanguage) ? openyidaReportMessages.zh : openyidaReportMessages.en);
window.__OPENYIDA_REPORT__ = (${createCanvasReportRuntime.toString()})(openyidaReportProtocol, window);`;
...
function buildCanvasPageSchemaObject(sourceCode, runtimeCode, importedModules, formUuid, options = {}) {
  ...
  const schema = {
    ...
    actions: {
      module: {
        compiled: (sourceCode.includes('__OPENYIDA_REPORT__') ? CANVAS_REPORT_SOURCE + '\n' : '') + CANVAS_ACTIONS_COMPILED,
        source: (sourceCode.includes('__OPENYIDA_REPORT__') ? CANVAS_REPORT_SOURCE + '\n' : '') + CANVAS_ACTIONS_SOURCE,
      },
      type: 'FUNCTION',
