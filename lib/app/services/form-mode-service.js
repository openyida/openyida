...
async function getProcessCodeFromFormBinding(authRef, appType, formUuid) {
  ...
  const binding = normalizeFormProcessBinding(result, appType);
...
function normalizeFormProcessBinding(result, expectedAppType, expectedFormUuid) {
  ...
  if (
    ...
    isNonEmptyString(expectedFormUuid) &&
    content.formUuid !== expectedFormUuid
  ) {
    return null;
  }
  if (
    !Object.prototype.hasOwnProperty.call(content, 'procCode') ||
    typeof content.procCode !== 'string'
  ) {
    return null;
  }
...
function modeError(code, operation) {
  return new FormModeServiceError(code, 'Form mode operation failed.', { operation });
}
