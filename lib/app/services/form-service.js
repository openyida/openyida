...
function sanitizeServiceResult(result) {
  ...
  const sanitized = {};
  ['success', 'errorMsg', 'errorCode', 'code', 'message'].forEach(function (key) {
...
function normalizeTitlePayload(input) {
  ...
  return JSON.stringify(i18n(input.formTitle || input.title || ''));
}
...
async function updateFormConfig(context, input) {
  ...
  if (input && input.strict) {
    assertConfigSuccess(result, { appType, formUuid });
  }
