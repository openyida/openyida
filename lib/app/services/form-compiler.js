...
const DIVIDER_FIELD_TYPE = 'Divider';
const DEFAULT_FORM_DETAIL_STYLE_PRESET = 'clean-card';
const DIVIDER_TYPE_PRIORITY = Object.freeze([
  'bold-with-thin',
...
const ALLOWED_DIVIDER_TYPES = new Set(DIVIDER_TYPE_PRIORITY.concat(LEGACY_STRONG_DIVIDER_TYPE));
...
function normalizeFormDefinition(definition, options) {
  ...
  if (Array.isArray(definition)) {
    return {
      ...
      themeTokens: compilerOptions.themeTokens || {},
      formDetailPreset: compilerOptions.formDetailPreset,
      validations: compilerOptions.validations || [],
      rules: compilerOptions.rules || [],
      dataSources: compilerOptions.dataSources || [],
    };
  }
  ...
  return {
    ...
    themeTokens: definition.themeTokens || compilerOptions.themeTokens || {},
    formDetailPreset: definition.formDetailPreset || compilerOptions.formDetailPreset,
    validations: definition.validations || compilerOptions.validations || [],
    rules: definition.rules || compilerOptions.rules || [],
    dataSources: definition.dataSources || compilerOptions.dataSources || [],
  };
}
...
function buildDividerComponent(field) {
  const fieldId = field.fieldId || field.id || generateFieldId(DIVIDER_FIELD_TYPE);
  const props = {
    fieldId,
    ...
    type: normalizeDividerType(field.dividerType || field.dividerStyle || field.styleType || field.typeStyle || field.type),
  ...
  if (field.props && typeof field.props === 'object' && !Array.isArray(field.props)) {
    ...
    props.type = normalizeDividerType(props.type);
  }
  ...
  return {
    componentName: DIVIDER_FIELD_TYPE,
    id: nextNodeId(),
...
function buildPageSectionComponent(field, options) {
  ...
  return buildSchemaNode(
    'PageSection',
    props,
    (field.children || []).map(function (child) { return buildFieldComponent(child, options); }),
    { fieldId }
  );
}

function countLayoutColumns(layout) {
  return String(layout || '12').split(':').filter(Boolean).length || 1;
}

function buildColumnComponent(columnDefinition, options) {
  if (Array.isArray(columnDefinition)) {
    return buildSchemaNode('Column', {}, columnDefinition.map(function (child) {
      return buildFieldComponent(child, options);
    }));
  }
  if (columnDefinition && typeof columnDefinition === 'object') {
    const componentName = normalizeFieldType(readFieldDefinitionType(columnDefinition));
    if (componentName === 'Column') {
      return buildSchemaNode(
        'Column',
        mergeExtraProps(Object.assign({}, columnDefinition.columnProps || {}), columnDefinition),
        (columnDefinition.children || []).map(function (child) { return buildFieldComponent(child, options); })
      );
    }
    return buildSchemaNode('Column', {}, [buildFieldComponent(columnDefinition, options)]);
  }
  return buildSchemaNode('Column', {}, []);
}

function buildColumnsLayoutComponent(field, options) {
  const fieldId = field.fieldId || field.id || generateFieldId('ColumnsLayout');
  const props = mergeExtraProps({
    fieldId,
    layout: field.layout || '6:6',
  ...
  const children = (Array.isArray(field.children) ? field.children : []).map(function (child) {
    return buildColumnComponent(child, options);
  });
  while (children.length < countLayoutColumns(props.layout)) {
    children.push(buildSchemaNode('Column', {}, []));
  }
  return buildSchemaNode('ColumnsLayout', props, children, { fieldId });
}
...
function buildFieldComponent(field, options) {
  ...
  if (componentName === DIVIDER_FIELD_TYPE) {
    return buildDividerComponent(field);
  }
  if (componentName === 'PageSection') {
    return buildPageSectionComponent(field, compilerOptions);
  }
  if (componentName === 'ColumnsLayout') {
    return buildColumnsLayoutComponent(field, compilerOptions);
  }
  if (componentName === 'Column') {
    return buildColumnComponent(field, compilerOptions);
  }
...
function applyDefaultFormDetailStyle(schema, options) {
  const styleOptions = {};
  if (options && options.themeTokens) {
    styleOptions.themeTokens = options.themeTokens;
  }
  return upsertFormDetailCss(
    schema,
    readFormDetailCss({ preset: DEFAULT_FORM_DETAIL_STYLE_PRESET }),
    styleOptions
  );
}
...
function buildFormSchemaCore(formTitle, fields, formUuid, corpId, appType, layout, theme, labelAlign, options) {
  const compilerOptions = createCompilerOptions(options);
  // 解析布局配置
  ...
  const constructorCode = buildFormConstructorCode();
  const actionsModule = buildFormActionsModule(options);
  ...
  const pageComponentsTree = [
    {
      ...
      lifeCycles: {
        ...
        componentDidMount: buildFormDidMountLifecycle(),
      },
      hidden: false,
      title: '',
      isLocked: false,
  ...
  const schema = {
    ...
    actions: {
      module: {
        compiled: actionsModule.compiled,
        source: actionsModule.source,
      },
      type: 'FUNCTION',
      list: [
        {
          ...buildFormActionListItem(nextNodeId),
        },
      ],
    },
    config: {
  ...
  if (!options || options.formDetailPreset !== 'none') {
    applyDefaultFormDetailStyle(schema, options);
  }
...
function buildEmptyFormSchemaCore(options) {
  const constructorCode = buildFormConstructorCode();
  const actionsModule = buildFormActionsModule(options);
  ...
  return {
    ...
    actions: {
      ...
      type: 'FUNCTION',
      list: [buildFormActionListItem(nextNodeId)],
    },
    pages: [{
      utils: [
...
function compileFormDefinition(definition, options) {
  return require('../scaffolds/form/form-schema-builder').compileFormDefinition(definition, options);
}
