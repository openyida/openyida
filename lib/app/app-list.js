 ...
 *   appName    - 应用名称（zh_CN）
 *   appType    - 应用唯一标识
 *   systemLink - 平台返回的应用访问地址
 *   appUrl / workbenchUrl - 应用工作台地址
 *   adminUrl   - 开发者管理后台地址（不表示已验证访问权限）
 */
...
const { buildApplicationEntryUrls } = require('./application-entry-urls');
 ...
 * 将应用原始数据提取为输出字段
 */
function formatApp(app, baseUrl) {
  return {
    appName: (app.appName && app.appName.zh_CN) || '',
    appType: app.appType || '',
    systemLink: app.systemLink || '',
    ...buildApplicationEntryUrls({ appType: app.appType, systemLink: app.systemLink, baseUrl }),
  };
}
...
async function run(args) {
  ...
  const formattedApps = apps.map(app => formatApp(app, authRef.baseUrl));
