...
function validateDivider(field, path, diagnostics) {
  const title = field.title !== undefined ? field.title : field.label;
  if (!hasFieldLabel(title)) {
    pushDiagnostic(
      diagnostics,
      'DIVIDER_TITLE_MISSING',
      path + '.title',
      'non-empty string or i18n object',
      title === undefined ? 'missing' : describeActual(title),
      'Divider must include a non-empty title.',
      'Use an inline item such as {"type":"Divider","title":"Basic information"}.'
    );
  }
  if (
...
function validateSemanticDividerLayout(fields, rootPath, diagnostics, options) {
  if (!options || options.requireSemanticDividers !== true) {
    return;
  }
  ...
  if (
    ...
    Object.prototype.hasOwnProperty.call(definition, 'dividers')
  ) {
    pushDiagnostic(
      diagnostics,
      'SEPARATE_DIVIDERS_UNSUPPORTED',
      'dividers',
      'Divider items inline in fields',
      describeActual(definition.dividers),
      'The dividers property is not part of the native form definition and would be ignored.',
      'Delete dividers and insert {"type":"Divider","title":"Section name"} directly into fields before each semantic group.'
    );
  }
  ...
  const firstType = normalizeDefinitionType(fields[0]);
  if (firstType !== 'Divider') {
    pushDiagnostic(
      diagnostics,
      ...
      'Divider',
      firstType || 'missing',
      'Native form fields must start with a Divider section title.',
      'Insert {"type":"Divider","title":"Basic information"} as the first fields item.'
    );
  }

  fields.forEach(function (field, index) {
    if (normalizeDefinitionType(field) !== 'Divider') {
      return;
    }
    if (index === fields.length - 1) {
      pushDiagnostic(
        ...
        rootPath + '[' + index + ']',
        'Divider followed by at least one field or layout component',
        'Divider at end of fields',
        'A Divider cannot be the last item because it would create an empty section.',
        'Remove the trailing Divider or add the section fields after it.'
      );
      return;
    }
    if (normalizeDefinitionType(fields[index + 1]) === 'Divider') {
      pushDiagnostic(
        diagnostics,
        'ADJACENT_DIVIDERS_UNSUPPORTED',
        rootPath + '[' + (index + 1) + ']',
        'business field or layout component after Divider',
        'Divider',
        'Two adjacent Divider items create an empty section.',
        'Keep one Divider and place the section fields after it.'
      );
    }
  });
}

function ensureSemanticDividerFields(fields) {
  if (!Array.isArray(fields) || fields.length === 0) {
    return fields;
  }
  if (normalizeDefinitionType(fields[0]) === 'Divider') {
    return fields;
  }
  return [
...
function validateFormFieldDefinitions(fields, options) {
  ...
  validateSemanticDividerLayout(fields, rootPath, diagnostics, validationOptions);
