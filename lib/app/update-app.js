...
async function run(args) {
  ...
  const response = params.themeFile ? await applyCustomThemeUpdate(params.appType, params, authRef)
    : shellUpdate ? await saveAppSettings(params, authRef) : await requestWithAutoLogin((auth) => {
      ...
      return httpPost(
        auth.baseUrl,
        `/${params.appType}/query/app/updateAppName.json?_api=Form.updateAppName&_mock=false&_stamp=${Date.now()}`,
        postData
      );
    }, authRef);

  // Verify the persisted resource before exposing success or a mutation receipt.
  let nameVerification;
  if (response && response.success && params.name) {
    const savedApp = await fetchCurrentApp(params.appType, authRef);
    const savedName = pickText(savedApp.appName, '');
    if (savedName !== params.name) {
      throwCommandError(t('update_app.name_not_persisted'), { code: 'APP_NAME_NOT_PERSISTED' });
    }
    nameVerification = { verified: true, name: savedName };
  }
