...
function parseArgs(args) {
  ...
  if (args.includes('--hide-app-nav') && args.includes('--show-app-nav')) {
    throwCommandError(t('cli_argument.invalid', '--hide-app-nav / --show-app-nav'), { code: 'CLI_INVALID_ARGUMENTS' });
  }
  ...
  for (let i = 1; i < args.length; i++) {
    ...
    if (!['--hide-app-nav', '--show-app-nav'].includes(arg) &&
        (!nextArg || nextArg.startsWith('--') || ['-n', '-d'].includes(nextArg))) {
      throwCommandError(t('cli_argument.invalid', arg), { code: 'CLI_INVALID_ARGUMENTS' });
    }
