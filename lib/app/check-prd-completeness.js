...
function isStructuredBuildFactSource(source) {
  return /^build_manifest\./.test(String(source || ''));
}
...
async function evaluatePrdCompleteness(markdown, options = {}, deps = {}) {
  ...
  checkableResources.forEach((resource) => {
    ...
    if (!ok) {
      ...
      } else if (!isStructuredBuildFactSource(resource.source)) {
        addItem('resource_missing', resource.source || 'prd.resourceBlueprint', 'needs_review', 'warning', `PRD hint 指向的资源未找到，需人工确认是否实际漏搭：${resource.name || resource.formUuid}`, {
          expected: { name: resource.name, formUuid: resource.formUuid, kind: resource.kind },
          actual: null,
        });
      } else {
