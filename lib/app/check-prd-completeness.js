...
const APP_TYPE_OPTIONS = Object.freeze(['--app-type', '--appType', '--app_type']);
...
function parseArgs(args = [], options = {}) {
  ...
  for (let index = 0; index < args.length; index++) {
    ...
    const key = APP_TYPE_OPTIONS.includes(arg) ? 'appType'
      : ['--build-manifest', '--manifest'].includes(arg) ? 'buildManifestPath' : arg;
    if (options.strict && arg.startsWith('-') && seen.has(key)) {
      throwUsage(t('check_prd_completeness.usage'), t('check_prd_completeness.example'));
    }
    if (options.strict && arg.startsWith('-')) {seen.add(key);}
    if (arg === '--help' || arg === '-h') {
      parsed.help = true;
    } else if (arg === '--json') {
      parsed.json = true;
    } else if (APP_TYPE_OPTIONS.includes(arg) && args[index + 1] && (!options.strict || !args[index + 1].startsWith('-'))) {
      parsed.appType = args[index + 1];
      index++;
    } else if ((arg === '--build-manifest' || arg === '--manifest') && args[index + 1] && (!options.strict || !args[index + 1].startsWith('-'))) {
      parsed.buildManifestPath = args[index + 1];
      index++;
    } else if (!arg.startsWith(options.strict ? '-' : '--') && !parsed.prdPath) {
      parsed.prdPath = arg;
