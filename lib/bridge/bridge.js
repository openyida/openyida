...
function normalizeAppListInput(body = {}) {
  const type = String(body.type ?? 'managed').trim();
  const page = Number(body.page ?? body.pageIndex ?? 1);
  const size = Number(body.size ?? body.pageSize ?? 16);
  if (!['managed', 'created'].includes(type)) {
    throw new Error('type must be managed or created');
  }
