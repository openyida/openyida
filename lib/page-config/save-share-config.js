...
function isUninitializedPublicClose(params, before) {
  return !params.openUrl && params.isOpen === 'n' && params.openAuth === undefined
    && isUninitializedPublicConfig(before);
}
...
function assertSavedConfig(params, before, after) {
  ...
  } else if (!isShareUpdate && !isUninitializedPublicClose(params, before)) {
    expected.openPageAuthConfig = buildExpectedOpenAuthConfig(params, before);
  } else if (Object.prototype.hasOwnProperty.call(before, 'openPageAuthConfig')) {
    expected.openPageAuthConfig = parseOpenAuthConfig(before.openPageAuthConfig);
  }
...
async function run(args) {
  ...
  if (needsPreservableAuth && !isUninitializedPublicClose(params, before)) {
