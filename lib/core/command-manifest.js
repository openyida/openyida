  ...
  return {
    ...
    core_workflows: {
      full_app_build: {
        ...
        default_nav_order_policy: 'The PRD defines navigation groups and order with stable resource keys. After current-run resources have real IDs, derive .cache/openyida/<project>/navigation-plan.json and call openyida nav-group order <appType> --plan <file>. The command preflights the full plan, applies groups and order, then reads back the tree; completion requires verification.matched=true. Do not use auto-order when the PRD has a navigation plan. Use auto-order only for a standalone task without a PRD plan.',
        ...
        completion_contract: 'Follow the yida-app stage table. Every PRD page marked for current delivery must be published and read back; container_created_unpublished is incomplete. Then apply the PRD navigation plan and return 2-3 business-summary sentences followed by one primary user-facing link.',
      },
    },
  };
}
