...
const COMMAND_SIDE_EFFECTS = new Map([
  ...
  ['agent-center', sideEffect('mixed', {
    mutates_yida: true,
    mutates_local: false,
    read_actions: ['list', 'range', 'search-user', 'is-leader', 'last-departure'],
  ...
  ['i18n', sideEffect('mixed', {
    mutates_yida: true,
    mutates_local: false,
    read_actions: ['overview', 'config get', 'languages', 'list', 'get'],
    mutating_actions: ['config set', 'upsert', 'batch-upsert', 'bind', 'delete', 'translate', 'translate-all', 'upgrade'],
  })],
...
const COMMAND_PERMISSIONS = new Map([
  ...
  ...permissionEntries([
    ...
    preauthorized_actions: ['config set', 'upsert', 'batch-upsert', 'bind', 'translate', 'translate-all', 'upgrade'],
    ask_actions: ['delete'],
  })),
...
const COMMAND_GROUPS = [
  ...
  },
  {
    ...
    commands: [
      ...
      command('i18n', ['i18n'], 'i18n <overview|config|get|languages|list|upsert|batch-upsert|bind|delete|translate|translate-all|upgrade> <appType> ...', 'help.cmd_i18n', {
        output: 'json',
      }),
  ...
  },
  {
    ...
    commands: [
      ...
      command('agent-center', ['agent-center'], 'agent-center <list|create|update|cancel|range|search-user|is-leader|last-departure> ...', 'help.cmd_agent_center', { output: 'json' }),
    ],
  },
  {
    id: 'process',
  ...
  },
  {
    ...
    commands: [
      command('commands', ['commands'], 'commands [--json|validate|build]', 'help.cmd_commands', {
        requiresLogin: false,
        output: 'json',
      }),
