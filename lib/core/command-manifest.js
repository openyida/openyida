...
const { buildScaffoldContracts } = require('../app/scaffold-contracts');
...
const FORBIDDEN_ALIASES = Object.freeze([
  ...
  Object.freeze({
    ...
    suggested_usage: 'openyida create-page <appType> "<name>" [--source <page.canvas.jsx>] [--mode dashboard] [--hide-nav] [--locale zh_CN|en_US|ja_JP] [--open|--no-open]',
...
const COMMAND_GROUPS = [
  ...
  },
  {
    ...
    commands: [
      command('create-form.create', ['create-form', 'create'], 'create-form create <appType> "<formTitle>" <fieldsJsonFile> [--layout single|double|card] [--theme default|compact|comfortable] [--label-align top|left|right] [--locale zh_CN|en_US|ja_JP] [--open|--no-open]', 'help.cmd_create_form', {
        args: [
          {
            name: 'appType',
            type: 'string',
      ...
      command('create-form.update', ['create-form', 'update'], 'create-form update <appType> <formUuid> <changesJsonOrFile> [--locale zh_CN|en_US|ja_JP] [--open|--no-open]', 'help.cmd_update_form'),
      ...
      command('create-page', ['create-page'], 'create-page <appType> "<name>" [--source <page.canvas.jsx>] [--mode dashboard] [--hide-nav] [--locale zh_CN|en_US|ja_JP] [--open|--no-open]', 'help.cmd_create_page'),
      command('build-page', ['build-page'], 'build-page <sourceFile> [--output file|--write]', 'help.cmd_build_page', { requiresLogin: false }),
      command('check-page', ['check-page'], 'check-page <src> [--compat] [--json]', 'help.cmd_check_page', { output: 'text|json' }),
      command('compile', ['compile'], 'compile <src> [--json] [--canvas]', 'help.cmd_compile', {
        requiresLogin: false,
        output: 'text|json',
      }),
  ...
  },
  {
    ...
    commands: [
      command('data', ['data'], 'data <query|get|create|update|execute> <form|process|subform|operation-records|task|tasks> [args]', 'help.cmd_data'),
  ...
  },
  {
    ...
    commands: [
      command('configure-process', ['configure-process'], 'configure-process <appType> <formUuid> <processDefinitionFile> [processCode]', 'help.cmd_configure_process'),
      command('create-process', ['create-process'], 'create-process <appType> "<formTitle>" <fieldsJsonFile> <processDefinitionFile> | create-process <appType> --formUuid <formUuid> <processDefinitionFile>', 'help.cmd_create_process'),
  ...
  },
  {
    ...
    commands: [
      command('create-report', ['create-report'], 'create-report <appType> "<name>" <configJsonFile> [--open|--no-open]', 'help.cmd_create_report'),
      command('append-chart', ['append-chart'], 'append-chart <appType> <reportId> <chartConfigJsonFile> [--open|--no-open]', 'help.cmd_append_chart'),
    ],
  },
  {
    id: 'connector',
  ...
  },
  {
    ...
    commands: [
      command('integration.create', ['integration', 'create'], 'integration create <appType> <formUuid> "<flowName>" [--events <list>] [--spec file.json] [--publish]', 'help.cmd_integration'),
  ...
  },
  {
    ...
    commands: [
      ...
      command('sample', ['sample'], 'sample [--list] | sample <skill> <name> [--output <file>] [--design-config <file>]', 'help.cmd_sample', { requiresLogin: false }),
      ...
      command('flash-to-prd', ['flash-to-prd'], 'flash-to-prd [--file <path>] --name "<project>" [--max-tokens N]', 'help.cmd_flash_to_prd', {
        requiresLogin: false,
      }),
      ...
      full_app_build: {
        ...
        artifact_generation: {
          ...
          tasks: [
            ...
            {
              skill_id: 'yida-design',
              output_path: 'prd/<project>/design.md',
              derived_paths: [
                '.cache/openyida/<project>/design-runtime.json',
                '.cache/openyida/<project>/scaffolds/form.form.json',
                '.cache/openyida/<project>/scaffolds/canvas.canvas.jsx',
              ],
            },
          ],
        ...
        scaffold_contracts: buildScaffoldContracts(),
        page_index_contract: {
          path_template: '.cache/openyida/<project>/page-specs/<pageKey>.json',
          purpose: 'Index the current page source sections and real resources without copying PRD or design content.',
          required_fields: ['version', 'pageKey', 'sourceOfTruth.prdFile', 'sourceOfTruth.designFile', 'sourceOfTruth.prdRefs', 'sourceOfTruth.designRefs', 'resources', 'dataBindings', 'sourceFile'],
          forbidden_fields: ['contentBlocks', 'themeSummary', 'visualImplementation', 'interactionProfile', 'insights'],
          read_policy: 'Read the indexed PRD/design sections before writing page source. The project scaffold supplies shared runtime and visual defaults.',
        },
        form_visual_guidance_policy: 'Native forms use the independent .form.json scaffold. The fields array starts with an inline Divider and each semantic group places its Divider before the group fields; do not use a separate dividers array. Use type for component types and dataSource for fixed options, then run create-form validate-fields before create. OpenYida generates and validates the native Schema through form-schema-builder.js and form-runtime.js.',
        page_skill_policy: 'New custom pages use Code Canvas. yida-canvas-custom-page owns Code Canvas page implementation rules. OpenYida provides one complete canvas.canvas.jsx scaffold with 13 Yida APIs, theme, form submission/detail/data-management drawer, URL builders, instance-id validation, iframe theme sync, and base state. Use yida-custom-page only when the user explicitly asks to modify an existing non-CodeCanvas Jsx/renderJsx page.',
        page_creation_policy: 'For every PRD page marked for current delivery, finish the Canvas source and compile it first. Then call create-page with --source and immediately execute delivery.requiredNextCommand. Never batch-create page containers. Do not create later-step pages. If publish fails after create, hide that navigation item and stop before creating another page.',
        form_entry_policy: 'Code Canvas submission, detail, and data-management actions use the scaffold FormOpenContainer drawer on all devices; mobile uses a full-screen drawer. Data management uses /{appType}/workbench/{formUuid}?hideLeftNav=true&corpid={corpId}. Use full-page or new-tab navigation only when the user explicitly requests it.',
        product_design_policy: 'Full app creation loads yida-app. yida-requirement-analysis writes one shared requirement brief. yida-prd and yida-design start in parallel; yida-design also derives design-runtime.json and project form/Canvas scaffolds. After both files are ready, yida-app validates the page references. Real resource IDs are resolved before page implementation.',
        ui_guidance_policy: 'prd.md stores complete business requirements and design.md stores complete visual requirements. design-runtime.json writes shared visual values into project scaffolds. page-spec.json is a thin index containing prdRefs, designRefs, real resources, data bindings, and source path; it must not copy or summarize PRD/design content. Page implementation reads the indexed PRD/design sections plus the project Canvas scaffold.',
        default_nav_order_policy: 'Navigation grouping and ordering are required for every full-app build. The PRD defines groups and order with stable resource keys. After current-run resources have real IDs, write .cache/openyida/<project>/navigation-plan.json and call openyida nav-group order <appType> --plan <file>. The plan file and command result are required; completion requires verification.matched=true. A warning, auto-order, or created group is not completion. Use auto-order only for a standalone task without a PRD plan.',
        navigation_contract: {
          required: true,
          canvas_publish_baseline: 'Every Code Canvas publish auto-orders navigation so custom pages precede process and normal forms, then verifies the server readback. A failed mutation or mismatched readback fails the publish command.',
          plan_path_template: '.cache/openyida/<project>/navigation-plan.json',
          command: 'openyida nav-group order <appType> --plan <file>',
        ...
        default_data_contract: 'Code Canvas pages use window.__OPENYIDA_RUNTIME__.yida or the compatible window.__OPENYIDA_YIDA_API__ bridge. Do not call ordinary-page this.utils.yida.* or this.dataSourceMap.* inside YidaComp. Existing ordinary JSX pages may keep this.dataSourceMap.* only when the target page already has that designer data source.',
        default_seed_data: {
          skill_id: 'yida-data-management',
          command_id: 'data',
          start_after: 'form field mappings are ready',
          run_in_parallel_with: ['yida-canvas-custom-page source implementation and local compile'],
          record_count: '1-3 records for each core normal form',
          skip_when: ['user opts out', 'configuration or permission table', 'sensitive data', 'no safely constructible values'],
        },
        ...
        completion_contract: 'Follow the yida-app stage table. Every PRD page marked for current delivery must be published and read back; container_created_unpublished is incomplete. The navigation plan file and command result with verification.matched=true are also required; warnings cannot replace navigation execution. Then return 2-3 business-summary sentences followed by one primary user-facing link.',
      },
    },
  };
}
