  ...
  return {
    ...
    core_workflows: {
      full_app_build: {
        ...
        form_visual_guidance_policy: 'Native forms use the independent .form.json scaffold. The agent only extends fields, Divider groups, validations, and rules in .form.json. OpenYida generates native form Schema, lifecycle, theme, and formDetail style through form-schema-builder.js and form-runtime.js. Do not write a native form as custom-page JSX.',
        page_skill_policy: 'New custom pages default to Code Canvas. yida-canvas-custom-page owns Code Canvas page implementation rules. OpenYida provides one complete canvas.canvas.jsx scaffold with 13 Yida APIs, theme, form submission/detail drawer, URL builders, instance-id validation, iframe theme sync, and base state. Use yida-custom-page when the user explicitly asks for ordinary JSX/Jsx, maintains an existing non-CodeCanvas Jsx/renderJsx page, or needs a page-instance capability that Canvas lacks.',
        product_design_policy: 'Full app creation loads yida-app. After target resources are confirmed, yida-requirement-analysis writes one shared requirement brief. yida-prd and yida-design then start in parallel from that brief; neither waits for the other. yida-app validates their references after both files are ready.',
