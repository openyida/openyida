  ...
  return {
    ...
    core_workflows: {
      full_app_build: {
        ...
        product_design_policy: 'Full app creation loads yida-app. yida-requirement-analysis reads the user request and existing-resource context, then writes one shared requirement brief. yida-prd and yida-design start in parallel from that brief; neither waits for the other. yida-app validates their references after both files are ready.',
        ui_guidance_policy: 'yida-prd writes prd/<project>/prd.md and yida-design writes prd/<project>/design.md from the same requirement brief. Page implementation reads both files. page-spec.json is only a derived implementation handoff for generators, not a third design source.',
