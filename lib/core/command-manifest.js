...
const COMMAND_GROUPS = [
  ...
  },
  {
    ...
    commands: [
      ...
      command('sample', ['sample'], 'sample [--list] [<skill> <name>] [--output <file>] [--design-file <design.md>]', 'help.cmd_sample', { requiresLogin: false }),
  ...
  return {
    ...
    core_workflows: {
      full_app_build: {
        ...
        design_mode_policy: 'yida-app Step 2 selects Fast or Plan once. Fast uses the parallel artifact workflow below. Plan uses yida-prd for business facts and yida-design for visual facts; yida-app materializes build-plan.json and requires confirmation of the current revision before resource creation; never run Fast afterward. Only build-plan.html is user-visible during Plan review.',
