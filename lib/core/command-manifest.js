  ...
  return {
    ...
    core_workflows: {
      full_app_build: {
        ...
        product_design_policy: 'Full app creation first resolves resource context, then calls yida-design once. The yida-design entry routes to Fast Design for direct product design or Plan Design for requirement alignment and a user-confirmed build plan. Plan Design must not return to resource creation before explicit confirmation, and after confirmation must proceed without running Fast Design again. Both branches output prd/<project>/prd.md with app information, app configuration, data structures, page functions, business rules, interaction states, resource blueprint, resource creation order, page implementation delivery order, navigation order, and acceptance criteria; they output prd/<project>/design.md with theme tokens, visual DNA, layout density, rounded rules, scene recipes, component rules, state rules, and visual acceptance checks.',
