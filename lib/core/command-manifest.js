  ...
  return {
    ...
    core_workflows: {
      full_app_build: {
        ...
        completion_contract: 'Resolve context, organize the user requirements, generate and validate prd.md and design.md in parallel, create or reuse app and core forms/processes, seed and query-check core normal-form samples or record an allowed skip reason, then implement the primary page in PRD resource order, publish it, apply PRD navigation order or lightweight fallback navigation order, and return one named application entry group. The group contains workbench, conditionally a verified standalone custom entry, and admin only for non-cloud agents. Business resources are summarized instead of emitted as individual delivery artifacts.',
      },
    },
  };
}
