...
const PERMISSION_SCHEMA = Object.freeze({
  ...
  effects: {
    ...
    unknown: 'Action-dependent; callers should inspect ask_actions before using the default mode.',
  },
});
...
function normalizePermission(metadata, sideEffectMetadata) {
  ...
  if (sideEffectMetadata && sideEffectMetadata.kind === 'mixed') {
    ...
    if (!Array.isArray(normalized.ask_actions)) {
      normalized.ask_actions = Array.isArray(sideEffectMetadata.mutating_actions)
        ? [...sideEffectMetadata.mutating_actions]
        : [];
    }
  }
  return normalized;
...
const ACTION_DEPENDENT_REASON = 'Action-dependent command: allow by default; ask only when the selected action matches ask_actions.';
...
const COMMAND_PERMISSIONS = new Map([
  ...
  ...permissionEntries([
    ...
    reason: ACTION_DEPENDENT_REASON,
    ask_actions: ['commands containing delete or remove'],
  })),
