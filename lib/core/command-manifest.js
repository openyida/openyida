...
const COMMAND_GROUPS = [
  ...
  },
  {
    ...
    commands: [
      ...
      command('design-plan.materialize', ['design-plan', 'materialize'], 'design-plan materialize <build-plan.json> [--output-dir <dir>] [--check] [--json]', 'help.cmd_design_plan_materialize', {
        requiresLogin: false,
        output: 'text|json',
      }),
  ...
  },
  {
    ...
    commands: [
      ...
      command('sample', ['sample'], 'sample [--list] [<skill> <name>] [--output <file>] [--var KEY=VALUE ...] [--design-file <design.md>]', 'help.cmd_sample', { requiresLogin: false }),
  ...
  return {
    ...
    core_workflows: {
      full_app_build: {
        ...
        form_page_policy: 'Use yida-create-form-page for form structure. Never inject CSS, JavaScript, HTML, or theme code into native forms or formDetail pages; the platform renders those pages.',
        ...
        plan_command_ids: ['design-plan.materialize', 'design-plan.patch'],
        ...
        navigation_policy: 'Resolve navigation before PRD planning: platform-l-shape, platform-top, platform-side, or custom. In non-Fast mode, ask when unclear; present platform-l-shape, platform-top, platform-side, and custom in that fixed order with neutral functional descriptions, no recommendation or default labels, and no preselection; record type, source, and reason in requirement-brief.json. Custom navigation requires update-app --hide-app-nav plus update-form-config <appType> <formUuid> false "<title>" and get-form-config --json readback of isRenderNav=false for every in-scope form, process form, and custom page. Platform layout and navTheme are separate decisions.',
        design_mode_policy: 'yida-app Step 2 selects Fast or Plan once per build. Offer exactly two modes: Plan (confirm the plan before building) and Fast (complete business and visual planning, then build without a separate plan confirmation). Present Plan then Fast with neutral descriptions, no recommendation or default labels, and no preselection. Never offer skills, tools, CLI paths, or skipping planning as mode options. First-time builds include a new app without appType and an existing empty app with appType but no pages. Unless the user has explicitly selected a mode for this same target and build, ask and wait before business or visual planning; app existence, login state, cached files, or simple requirements do not select a mode. Fast uses the parallel artifact workflow below. Plan prepares business facts and base visual facts concurrently, then binds visual applications to settled page contracts; yida-app merges the separate results into build-plan.json and materializes PRD, design and preset-template HTML in one call, then requires confirmation of the current revision before resource creation; never run Fast afterward. Only build-plan.html is user-visible during Plan review.',
        product_design_policy: 'Full app creation first resolves resource context. yida-requirement-analysis writes the organized user requirements to requirement-brief.json. In Fast, yida-prd and yida-design read that same file and run in parallel to write prd/<project>/prd.md and prd/<project>/design.md; neither waits for or overwrites the other. yida-app checks both files and their references, and only then creates or fills missing app/form/process/page resources.',
