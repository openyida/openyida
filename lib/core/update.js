...
function isAutoUpdateDisabled(env = process.env) {
  if (env.OPENYIDA_MANAGED_RUN === '1') {return true;}
  return !!(
    String(env.OPENYIDA_MANAGED_RUNTIME || '').trim().toLowerCase() === 'local' ||
...
function shouldSkipAutoUpdateCommand(command, args = []) {
  ...
  if (['commands', 'agent-capabilities', 'agent', 'mcp', 'a2a', 'bridge', 'update'].includes(command)) {
    return true;
  }
