...
function buildSkillsCandidate(destPath, source, scope, reason) {
  ...
  const entryPath = path.join(destPath, 'SKILL.md');
  let readable = false;
  try {
    readable = fs.statSync(entryPath).isFile();
  ...
  return {
    ...
    readable,
    usable: readable,
    installable: scope !== 'package' && writable,
...
function buildSkillsDiagnostics(options = {}) {
  ...
  // Bundled documentation is a read fallback, never an installation destination.
  addCandidate(
    options.bundledSkillsPath || path.resolve(__dirname, '../../yida-skills'),
    'package:yida-skills', 'package', 'bundled_skills_read_fallback'
  );
  const selected = candidates.find((candidate) => candidate.usable) || null;
  const installTarget = candidates.find((candidate) => candidate.installable) || null;
  const diagnostics = {
  ...
  if (result && result.__needLogin) {
    ...
    try {
      ...
      const refreshResult = await tokenRefresh({
        ...
        ...(currentAuthData && currentAuthData.credential_source === 'dws'
          ? { authProfile: currentAuthData.auth_profile } : {}),
...
function normalizeRefreshedTokenAuthData(refreshResult, fallbackBaseUrl) {
  ...
  return {
    ...
    credential_source: refreshResult.credential_source,
