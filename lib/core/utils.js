...
function buildCodeBundleResponseMetadata(initialUrl, currentUrl, response, firstPartyTraceId) {
  const traceId = firstPartyTraceId
    || response.headers.get('eagleeye-traceid')
    || '';
  const metadata = {
    baseUrl: initialUrl.origin,
    finalHost: currentUrl.host,
...
/**
 * 获取文本并显式跟随受控 302。鉴权头只发送给初始宜搭域名，不转发到 OSS。
 */
async function httpGetRedirectText(baseUrl, requestPath, queryParams, optionsOrLegacyCookies, maybeOptions) {
  if (typeof fetch !== 'function') {
    throw new Error('当前 Node.js 环境缺少 fetch，请使用 Node.js 18+');
  }
  const optionsOverride = resolveRequestOptions(optionsOrLegacyCookies, maybeOptions);
  const authHeaders = await resolveRequestAuthHeaders(optionsOverride);
  const initialUrl = buildRequestUrl(baseUrl, requestPath, queryParams);
  const maxRedirects = Number.isInteger(optionsOverride.maxRedirects)
    ? optionsOverride.maxRedirects
    : 3;
  const maxBytes = Number(optionsOverride.maxBytes) || 5 * 1024 * 1024;
  let currentUrl = initialUrl;
  let firstPartyTraceId = '';

  for (let redirectCount = 0; redirectCount <= maxRedirects; redirectCount++) {
    const firstPartyRequest = currentUrl.origin === initialUrl.origin;
    const response = await fetchWithTimeout(currentUrl, {
      method: 'GET',
      redirect: 'manual',
      headers: firstPartyRequest
        ? {
    ...
    if (firstPartyRequest && response.headers.get('eagleeye-traceid')) {
      firstPartyTraceId = response.headers.get('eagleeye-traceid');
    }
    const metadata = buildCodeBundleResponseMetadata(
      initialUrl,
      currentUrl,
      response,
      firstPartyTraceId
    ...
    if (isHttpRedirectStatus(response.status)) {
      ...
      if (!location || redirectCount >= maxRedirects) {
        throw new Error('CodeBundle 下载重定向无效或次数过多');
      }
      const nextUrl = new URL(location, currentUrl);
      if (!['http:', 'https:'].includes(nextUrl.protocol)) {
        throw new Error('CodeBundle 下载重定向协议不受支持');
      }
      if (currentUrl.protocol === 'https:' && nextUrl.protocol !== 'https:') {
        throw new Error('CodeBundle 下载禁止从 HTTPS 降级到 HTTP');
      }
      if (redirectCount === 0 && nextUrl.origin === initialUrl.origin) {
        return {
          __needLogin: true,
          __httpStatus: response.status,
          __location: location,
    ...
    if (firstPartyRequest && response.status === 401) {
  ...
  throw new Error('CodeBundle 下载重定向次数过多');
}
