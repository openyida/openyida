...
async function httpGetRedirectText(baseUrl, requestPath, queryParams, optionsOrLegacyCookies, maybeOptions) {
  ...
  for (let redirectCount = 0; redirectCount <= maxRedirects; redirectCount++) {
    ...
    if (response.status < 200 || response.status >= 300) {
      const errorBody = (await readFetchResponseBuffer(response, 4096, optionsOverride.timeout)).toString('utf8');
      throw new Error(`CodeBundle 下载失败：HTTP ${response.status}${errorBody ? ` ${errorBody.substring(0, 500)}` : ''}`);
    }
    ...
    try {
      ...
      throw new Error('CodeBundle 下载内容不是有效 UTF-8 文本');
    }
  }
