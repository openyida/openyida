...
async function resolveBearerAuthHeaders(options = {}) {
  ...
  if (managed) {
    const headers = {
      Authorization: `Bearer ${managed.taskGrant}`,
      'X-OpenYida-Local-Run-Id': managed.runId,
      'X-OpenYida-Local-Attempt-Id': managed.attemptId,
      'X-OpenYida-Local-App-Type': managed.appType,
    };
    return { tokenAuth: true, headers };
  }
  if (!isTokenAuthMode()) {
...
async function requestWithAutoLogin(requestFn, authRef) {
  ...
  if (result && result.__needLogin) {
    const { readManagedContext } = require('../agent/managed-context');
    if (readManagedContext(process.env)) {return tokenAuthRequiredResult();}
