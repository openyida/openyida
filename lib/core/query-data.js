...
const { assessAnalysisDataVolume } = require('../report/analysis-policy');
...
async function queryForm(positionals, options, session) {
  ...
  if (!options.inst_id && isSuccessfulDataResponse(result)) {
    const advice = assessAnalysisDataVolume(result, { appType, formUuid, searchFieldJson: searchJson });
    if (advice.requiresUserDecision) {
      result = { ...result, _openyidaAnalysis: advice };
    }
  }
