 ...
 *   openyida sample <skill> <name> --design-config <file>  应用项目视觉配置
 *
 * 示例：
 ...
 * @returns {{ outputPath: string, variables: Object, designConfigPath: string }}
 */
function parseOptions(rest, defaultOutputPath) {
  let outputPath = defaultOutputPath;
  let designConfigPath = '';
  const variables = {};
  ...
  for (let i = 0; i < rest.length; i++) {
    ...
    if (arg === '--design-config' && rest[i + 1]) {
      designConfigPath = rest[i + 1];
      i++;
      continue;
    }
  }

  return { outputPath, variables, designConfigPath };
}
...
async function run(args) {
  ...
  const { outputPath, variables, designConfigPath } = parseOptions(rest, defaultOutputPath);

  // 写入文件
  ensureDirectoryExists(outputPath);
  const sourceContent = fs.readFileSync(sourcePath, 'utf-8');
  let outputContent = applyTemplateVariables(sourceContent, variables);
  if (designConfigPath) {
    const { applyDesignRuntimeToSample, loadDesignRuntime } = require('./design-scaffold');
    outputContent = applyDesignRuntimeToSample(skill, name, outputContent, loadDesignRuntime(designConfigPath));
  }
  fs.writeFileSync(outputPath, outputContent, 'utf-8');

  chalkSuccess(`sample 已输出到：${c.cyan}${outputPath}${c.reset}`);
  chalkHint('可用 --design-config <design-runtime.json> 写入项目主题、圆角、密度和间距。');
}
