...
function printSampleList() {
  ...
  console.log(`  ${t('sample_options.theme_help')}`);
  console.log(`  ${t('sample_options.style_help')}`);
}
...
/**
 * 解析 sample 输出路径、设计文件、主题 ID 与模板变量；拒绝未声明参数。
 * @param {string[]} rest
 * @param {string} defaultOutputPath
 * @returns {{ outputPath: string, variables: Object, designFile?: string, styleId?: string }}
 */
function parseOptions(rest, defaultOutputPath) {
  ...
  for (let i = 0; i < rest.length; i++) {
    const arg = rest[i];
    const invalid = () => {throw new CliError(t('cli_argument.invalid', arg), { code: 'SAMPLE_ARGUMENT_INVALID' });};
    if (!['--output', '--var', '--design-file', '--style-id'].includes(arg)) {invalid();}
    if (!rest[i + 1] || rest[i + 1].startsWith('--')) {invalid();}
    ...
    if (arg === '--var' && rest[i + 1]) {
      ...
      if (eqIndex <= 0 || !pair.slice(0, eqIndex).trim()) {invalid();}
      const key = pair.slice(0, eqIndex).trim();
      variables[key] = pair.slice(eqIndex + 1);
      i++;
      continue;
    }
  }
...
async function run(args) {
  ...
  if (skill === 'yida-design' && name === 'application-style') {
    ...
    if (!options.styleId) {
      throw new CliError(t('cli_argument.invalid', '--style-id'), { code: 'APPLICATION_STYLE_REQUIRED' });
    }
  ...
  if (sampleKey === 'yida-design/app-theme') {
    if (Object.keys(variables).length) {
      throw new CliError(t('cli_argument.invalid', '--var (yida-design app-theme: --design-file)'), { code: 'SAMPLE_ARGUMENT_INVALID' });
    }
