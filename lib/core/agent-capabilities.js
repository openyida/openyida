...
function buildFullAppArtifactRoute(manifest) {
  ...
  return {
    ...
    divider_style_contract: workflow.divider_style_contract,
...
function buildApplicationEntryPolicy(builderPath, env = process.env) {
  ...
  return {
    ...
    frontend_delivery: 'Infer standalone for a scoped service frontend, persist page navigation hiding and return the verified publish/get-form-config standaloneUrl. No extra flag or navigation URL parameter. Missing verification is incomplete delivery; never silently omit the frontend or replace it with workbench/admin.',
    entries: {
