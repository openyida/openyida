...
function buildFullAppArtifactRoute(manifest) {
  ...
  const canvasContract = workflow.scaffold_contracts
    && workflow.scaffold_contracts.canvas;
  return {
    ...
    canvas_binding_contract: canvasContract && canvasContract.binding_contract,
  };
}
  ...
  return {
    ...
    sideEffects: {
      ...
      completion_contracts: {
        full_app: 'Default full-app build follows the yida-app stage table, publishes the primary page, applies lightweight navigation sorting when required, then returns 2-3 business-summary sentences followed by one primary user-facing link. Do not default to resource ID tables or long URL lists.',
      },
