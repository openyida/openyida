...
module.exports = {
  ...
  help: {
    ...
    cmd_flash_to_prd: '闪记 / 会议纪要生成会议需求稿',
...
命令：
  ...
  create-page <appType> "<页面名>" [--source <page.canvas.jsx>] [--mode dashboard] [--hide-nav] 创建自定义页面，输出 pageId
...
`,
  ...
  create_page: {
    ...
    usage: '用法: openyida create-page <appType> "<pageName>" [--source <page.canvas.jsx>] [--mode dashboard] [--hide-nav]',
  ...
  create_form: {
    ...
    config_pending: '  ⚠️  配置更新暂未生效: {0}',
    schema_ok_config_failed: '  Schema 已保存，但配置更新失败',
    schema_ok_config_pending: '  Schema 已保存；平台配置服务仍在同步，可稍后重试配置更新',
  ...
  publish: {
    ...
    lint_form_open_container: '自定义页内打开表单提交、详情或数据管理页必须使用 FormOpenContainer 抽屉。数据管理 URL 使用 workbench/{formUuid}?hideLeftNav=true&corpid={corpId}。按钮事件请调用 openForm(request)。',
    lint_form_detail_link: '表单详情页必须使用真实 formInstId：优先读取 row.formInstId，缺少实例 ID 时禁用详情入口或提示，不能打开空 formInstId 的 formDetail 链接。',
    lint_formdata_literal_key: '读取 formData 时不能用字段名/中文 label「{0}」作为 key；searchFormDatas 返回的 formData key 是真实 fieldId，请使用 get-schema --field-map-json 生成的 FIELDS 映射。',
    lint_undefined_seterror: '检测到调用 setError(...)，但当前作用域没有绑定 setError。请从 useState 或 useCanvasBaseState 返回值中解构 setError，避免 Canvas 页面运行时报 ReferenceError。',
    ...
    lint_canvas_yida_api_bridge_missing: 'Code Canvas 表单数据读取必须优先消费 window.__OPENYIDA_RUNTIME__.yida 或 window.__OPENYIDA_YIDA_API__（发布层由外层 didMount 注入 this.utils.yida 桥）；不要默认手写内部 searchFormDatas fetch',
