...
module.exports = {
  agent: {
    preflight_probe_cancelled: '预检已中止，尚未开始连接。准备好后重新执行连接命令。',
    preflight_continue: '至少一个本地 CLI 可用，将继续连接；其他 Agent 的提示可稍后处理。',
    preflight_ready: 'CLI 协议和登录检查通过，可以继续连接。',
    preflight_available: 'CLI 可启动；登录和协议能力将在使用时检查。',
    preflight_cli_not_found: '未找到可连接的 CLI。仅安装桌面版可能不包含 CLI，请安装或指定 CLI 路径。',
    preflight_login_required: '已找到 CLI，请在终端或浏览器中完成登录后重试。',
    preflight_launch_failed: 'CLI 无法启动。请检查路径与系统权限；命令脚本可尝试改用原生可执行文件。',
    preflight_protocol_unsupported: 'CLI 未通过所需协议握手，请检查或升级 CLI 版本。',
    preflight_probe_timeout: 'CLI 检查超时，请检查网络与程序能否正常启动；这不表示未安装。',
    preflight_probe_failed: 'CLI 检查失败，请直接运行 CLI 检查配置、网络或账号后重试。',
    preflight_desktop: '检测到桌面版',
    preflight_retry: '完成上述操作后，重新执行原连接命令；若接入凭证过期，请在宜搭重新获取命令。可运行 openyida agent doctor --json 复查。',
    preflight_blocked: '暂无可用的本地 Agent，请按预检提示修复；尚未开始宜搭连接授权。',
    private_directory_failed: '无法准备本地 Agent 私有目录。请检查目录权限及 Windows PowerShell 策略；现有目录权限未被修改。',
    managed_receipt_invalid: '任务回执目录不可用、回执超限或写入失败；若命令已执行，请先核对业务结果，不要直接重试。',
    usage: '用法：openyida agent <doctor|diagnose|status|connect|run|disconnect|logout> [--json]\n诊断：diagnose [--session <宜搭会话链接或 local_会话ID>] [--deep] [--output <目录>]\n开发构建：--development-runtime --runtime-path <绝对路径> --runtime-manifest <绝对路径>\n连接/运行：--endpoint <云端HTTPS地址> --endpoint-id <环境标识>\n网页发起接入：--enroll <短期令牌>\n可选：--provider <qoder|codex|opencode> [--provider-path <绝对路径>] --state-dir <绝对路径>\nconnect 接入成功后直接以前台方式接单；run 用于已接入设备重新上线。connect 授权前自动预检；doctor 复用预检。--provider 可单独指定类型，路径可选。不指定类型时检查 PATH 和常见安装位置，至少一个 CLI 可用即可连接，仅注册通过检查的 CLI。远程任务不会使用本地普通宜搭登录 profile；工作目录隔离不是操作系统沙箱。',
    diagnostic_session_invalid: '诊断参数必须是合法的 local_ 会话 ID，或包含该 sessionId 的 HTTPS 宜搭会话链接。',
    absolute_path_required: '必须提供明确的绝对文件路径。',
    runtime_missing: 'Runtime 或清单文件不存在；不会从 PATH 启动其他程序。',
    runtime_not_configured: '尚未安装已批准的平台 runtime 包。开发构建需显式提供 --development-runtime、--runtime-path 和 --runtime-manifest；平台 npm 包尚未发布。',
    development_required: '未签名的开发 runtime 必须显式提供 --development-runtime；生产模式不接受任意可执行文件路径。',
    signature_failed: 'Runtime 发布者签名缺失、不受信任、已撤销或无效，已拒绝执行。',
    bundle_invalid: '固定本地工具包不完整、已改变或目录不安全。请检查错误码；未启动 runtime，也未删除已有工具包。',
    unsafe_file: 'Runtime 必须是普通文件，不能是符号链接或目录。',
    manifest_invalid: 'Runtime 清单无效。',
    protocol_unsupported: 'Runtime 协议版本不兼容。',
    platform_unsupported: 'Runtime 与当前操作系统或处理器架构不匹配。',
    runtime_not_executable: 'Runtime 文件没有执行权限。',
    integrity_failed: 'Runtime 内容校验失败，已拒绝执行。',
    input_invalid: '必须通过受控 stdin 提供有效且不超过 64 KiB 的 JSON 对象。',
    runtime_failed: '本地 runtime 未正常完成；请根据错误代码检查状态，不要自动重跑业务操作。',
    identity_invalid: '本地 Agent 服务端点或接入身份无效、不匹配。',
    connection_options_required: '连接或运行需要 --endpoint 与 --endpoint-id（正式包通常会内置）。',
    provider_options_required: '指定 qoder/codex/opencode；若提供 --provider-path，必须同时指定类型并使用绝对路径。',
    provider_not_found: '未找到所选本地 Agent 的 CLI。请安装 CLI，或使用 --provider 指定类型并通过 --provider-path 提供绝对路径。',
    managed_context_invalid: '受管 CLI 执行必须绑定完整的任务凭据、用户、组织、执行轮次和已有应用。',
    managed_override_forbidden: '受管 CLI 执行不能覆盖身份、环境、凭据或应用绑定。',
    managed_command_unsupported: '受管本地任务尚不支持此命令。仅允许经过明确审查且目标应用可确定的命令；创建应用、切换身份和任意批处理均禁用。',
    managed_app_mismatch: '命令必须按规范的位置参数顺序，使用本次任务精确绑定的应用。',
  },
  asset: {
  ...
  help: {
    ...
    cmd_agent: '连接本地 Agent（Go runtime，开发预览）',
...
`,
  ...
  update_app: {
    ...
    name_not_persisted: '应用名称保存后回读不一致，尚未确认修改成功。请核对当前应用名称，不要重复创建应用。',
