...
module.exports = {
  agent: {
    ...
    usage: '用法：openyida agent <doctor|diagnose|status|connect|run|disconnect|logout|logs> [--json]\n诊断：diagnose [--session <宜搭会话链接或 local_会话ID>] [--deep] [--output <目录>]\n开发构建：--development-runtime --runtime-path <绝对路径> --runtime-manifest <绝对路径>\n连接/运行：--endpoint <云端HTTPS地址> --endpoint-id <环境标识>\n网页发起接入：--enroll <短期令牌>\n可选：--provider <qoder|codex|opencode> [--provider-path <绝对路径>] --state-dir <绝对路径>\nconnect 接入成功后转入后台常驻，终端立即返回，连接会持续到执行 disconnect；用 status 查看运行状态、logs 查看后台日志、disconnect 停止并断开。run 用于已接入设备重新上线，同样后台常驻并立即返回。connect 授权前自动预检；doctor 复用预检。--provider 可单独指定类型，路径可选。不指定类型时检查 PATH 和常见安装位置，至少一个 CLI 可用即可连接，仅注册通过检查的 CLI。远程任务不会使用本地普通宜搭登录 profile；工作目录隔离不是操作系统沙箱。',
    ...
    platform_unsupported: '本地 Agent Beta 当前仅支持 macOS 和 Windows；Linux 暂未开放。普通 OpenYida CLI 命令仍可在 Linux 使用。',
