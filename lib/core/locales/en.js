...
module.exports = {
  agent: {
    preflight_probe_cancelled: 'Preflight was interrupted before enrollment. Repeat the connect command when ready.',
    preflight_continue: 'At least one local CLI is usable. Continuing connection; other Agent notices can be addressed later.',
    preflight_ready: 'CLI protocol and login checks passed; ready to connect.',
    preflight_available: 'CLI starts; login and protocol will be checked during use.',
    preflight_cli_not_found: 'No connectable CLI found. A desktop installation alone may not provide a CLI. Install one or specify its path.',
    preflight_login_required: 'CLI found. Complete login in your terminal/browser, then retry.',
    preflight_launch_failed: 'CLI could not start. Check its path and OS permissions; for a command shim, try the native executable.',
    preflight_protocol_unsupported: 'CLI did not complete the required protocol handshake. Check or update this CLI version.',
    preflight_probe_timeout: 'CLI check timed out. Check the network and whether the program can start; this does not mean it is missing.',
    preflight_probe_failed: 'CLI check failed. Run the CLI directly to check its configuration, network or account, then retry.',
    preflight_desktop: 'desktop installation detected',
    preflight_retry: 'Complete the steps above, then repeat your original connect command. If its enrollment ticket expires, obtain a new command from Yida. Verify with openyida agent doctor --json.',
    preflight_blocked: 'No usable local Agent is ready. Follow the preflight instructions; Yida enrollment has not started.',
    private_directory_failed: 'Cannot prepare a private local Agent directory. Check directory permissions and Windows PowerShell policy; existing permissions were not changed.',
    managed_receipt_invalid: 'Task receipts are unavailable, oversized, or could not be saved. If execution started, verify the business result before retrying.',
    usage: 'Usage: openyida agent <doctor|diagnose|status|connect|run|disconnect|logout|logs> [--json]\nDiagnostics: diagnose [--session <Yida URL or local_ session ID>] [--deep] [--output <directory>]\nDevelopment build: --development-runtime --runtime-path <absolute-path> --runtime-manifest <absolute-path>\nConnect/run: --endpoint <cloud-HTTPS-URL> --endpoint-id <environment-id>\nWeb-first connect: --enroll <short-lived-token>\nOptional: --provider <qoder|codex|opencode> [--provider-path <absolute-path>] --state-dir <absolute-path>\nConnect enrolls and then keeps serving as a background daemon, returning the terminal immediately; the connection persists until disconnect. Use status to check it, logs to read background output, and disconnect to stop and disconnect. run brings an enrolled device online again, also as a background daemon that returns the terminal immediately. Connect runs preflight before enrollment; doctor shares the checks. --provider selects a type without requiring a path. With no type selected, PATH and known installation directories are checked; one usable CLI is sufficient, and only usable CLIs are registered. Remote tasks never use your ordinary local Yida profile; a workdir is not an OS sandbox.',
    diagnostic_session_invalid: 'Diagnostics require a valid local_ session ID or an HTTPS Yida session URL containing that sessionId.',
    absolute_path_required: 'An explicit absolute file path is required.',
    runtime_missing: 'Runtime or manifest file is missing; no PATH fallback will be executed.',
    runtime_not_configured: 'No approved platform runtime package is installed. Development builds require --development-runtime, --runtime-path and --runtime-manifest; platform npm packages are not published yet.',
    development_required: 'Unsigned development runtimes require the explicit --development-runtime flag; production cannot use arbitrary executable paths.',
    signature_failed: 'Runtime publisher signature is missing, untrusted, revoked or invalid; execution was refused.',
    bundle_invalid: 'The immutable local tool bundle is incomplete, modified, or unsafe. Check the error code; no runtime was started and no existing bundle was deleted.',
    unsafe_file: 'Runtime must be a regular file, not a symlink or directory.',
    manifest_invalid: 'The runtime manifest is invalid.',
    protocol_unsupported: 'The runtime protocol version is incompatible.',
    platform_unsupported: 'Local Agent Beta currently supports macOS and Windows only; Linux is not yet available. Regular OpenYida CLI commands remain available on Linux.',
    runtime_not_executable: 'The runtime file is not executable.',
    integrity_failed: 'Runtime integrity verification failed; execution was refused.',
    input_invalid: 'A valid JSON object of at most 64 KiB must be supplied through controlled stdin.',
    runtime_failed: 'The local runtime did not complete normally. Inspect the error code and status; do not automatically replay business operations.',
    identity_invalid: 'The local Agent endpoint or enrollment identity is invalid or mismatched.',
    connection_options_required: 'Connect/run requires --endpoint and --endpoint-id (normally supplied by the published package).',
    provider_options_required: 'Specify qoder/codex/opencode; --provider-path additionally requires a provider type and an absolute path.',
    provider_not_found: 'No CLI was found for the selected local Agent. Install a CLI, or select its type with --provider and an absolute --provider-path.',
    managed_context_invalid: 'Managed CLI execution requires a complete task grant, user, organization, run and existing application binding.',
    managed_override_forbidden: 'Managed CLI execution cannot override identity, environment, credentials or application binding.',
    managed_command_unsupported: 'This command is not supported in a managed local run. Only explicitly audited commands with a known application target are allowed; creating applications, switching identity and arbitrary batch commands are disabled.',
    managed_app_mismatch: 'The command must use the exact application bound to this run, in canonical positional order.',
  },
  design_document: {
  ...
  help: {
    ...
    cmd_agent: 'Connect a local Agent (Go runtime, development preview)',
...
`,
  ...
  update_app: {
    ...
    name_not_persisted: 'The saved app name did not match the requested name. Verify the current app name before retrying; do not create another app.',
