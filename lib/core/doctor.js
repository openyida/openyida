...
class EnvironmentChecker {
  ...
  checkLoginStatus() {
    ...
    if (status.auth_mode === 'cookie') {
      if (status.status !== 'ok') {
        return {
          id: 'env-login',
          label: '宜搭 Cookie 注入登录态',
          passed: false,
          severity: Severity.WARNING,
          message: `Cookie 注入不可用（${status.failure_reason || status.status}），请宿主重新注入 OPENYIDA_COOKIE_B64`,
          fixType: null,
          fixCommand: null,
        };
      }
