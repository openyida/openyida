...
const SENSITIVE_KEY_PATTERN = /(^|[_-])(authorization|auth|cookie|csrf|token|system[_-]?token|secret|password|passwd|api[_-]?key|access[_-]?key|signature|sign)([_-]|$)/i;
const SENSITIVE_CAMEL_KEY_PATTERN = /^(?:authorization|auth|cookie|csrf|token|systemToken|secret|password|passwd|apiKey|accessKey|signature|sign)(?:[A-Z]|$)|(?:Authorization|Auth|Cookie|Csrf|Token|SystemToken|Secret|Password|Passwd|ApiKey|AccessKey|Signature|Sign)$/;
...
function redactString(value) {
  return String(value)
    ...
    .replace(/(access[_-]?token|csrf[_-]?token|system[_-]?token|api[_-]?key|secret|password|token)=([^&\s]+)/gi, '$1=***')
    .replace(/(["']systemToken["']\s*:\s*["'])([^"']+)(["'])/gi, '$1***$3')
    .replace(/(Cookie:\s*)([^\r\n]+)/gi, '$1***')
...
function redactSensitive(value, options = {}) {
  ...
  try {
    ...
    // Only ancestors form a cycle; siblings may share the same diagnostic object.
    seen.delete(value);
  }
}
