...
function resolveDeclaredSourceFormType(node = {}) {
  const declarations = [
    ...
    && value !== null
    && String(value).trim() !== ''
    && !(field === 'originalType' && isSubTableOriginalType(value)))
    .map(([field, value]) => ({
      field,
      rawValue: value,
...
function normalizeDataConditions(conditions, context, sourceFormType) {
  return asArray(conditions).map((condition) => {
    ...
    const runtimeFieldId = toRuntimeDataQueryField(resolveNodeRefs(bFieldId, context), sourceFormType);
    return {
      bFieldId: runtimeFieldId,
      bFieldName: normalizeDataQueryFieldName(runtimeFieldId, sourceFormType, bFieldName),
...
function resolveRetrieveSourceLabel(node, context, sourceId) {
  ...
  return resolveDataSourceFormName(node, context)
    || node.formName
    || (context.formNamesByUuid && context.formNamesByUuid.get(sourceId))
    || sourceId;
}
...
function buildDataRetrieveProcessNode(node, nodeId, nextNodeId, context) {
  ...
  const dataSourceFormType = isSubTableRetrieve(node)
    ? ''
    : resolveDataSourceFormType(node, context);
  ...
  return {
    ...
    props: {
      ...
      sourceId,
      appType: context.appType,
      originalType: isSubTableRetrieve(node)
        ? 'sub_table'
        : resolveDataOriginalType(dataSourceFormType),
...
function buildDataRetrieveViewNode(node, nodeId, context) {
  ...
  const dataSourceFormType = isSubTableRetrieve(node)
    ? ''
    : resolveDataSourceFormType(node, context);
  const dataSourceFormName = isSubTableRetrieve(node)
    ? ''
    : resolveDataSourceFormName(node, context);
  ...
  } else if (isSubTableRetrieve(node)) {
    viewCondition = props.condition;
  } else {
    viewCondition = {
      ...props.condition,
...
function resolveCreateTarget(node, context) {
  if (isSubTableInsert(node)) {
    ...
    const parentFormUuid = resolveSubTableParentFormUuidFromContext(node, context);
    const parentFormName = resolveDataSourceFormName(sourceNode, context)
    ...
    return {
      ...
      parentFormUuid,
      parentFormName,
      parentFormType: resolveDataSourceFormType(sourceNode, context),
      formName,
...
function buildDataCreateViewNode(node, nodeId, context) {
  ...
  if (target.isSubTable) {
    ...
    const parentFormName = target.parentFormName || target.formName;
    const targetItem = {
      deep: 0,
    ...
    if (target.parentFormUuid) {
      targetItem.formItem = {
        formType: resolveDataFormItemType(target.parentFormType),
        advanceProc: 'n',
...
function collectAddDataFormUuids(spec, triggerFormUuid, options = {}) {
