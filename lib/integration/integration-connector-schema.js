...
function collectInputNames(inputs, prefix = '', result = new Map()) {
  for (const input of inputs || []) {
    if (!input || typeof input !== 'object' || !input.name) { continue; }
    const name = prefix ? `${prefix}.${input.name}` : input.name;
    if (!result.has(input.name)) { result.set(input.name, new Set()); }
    result.get(input.name).add(name);
    result.set(name, new Set([name]));
...
function validateConnectorAssignmentsAgainstSchema(assignments, inputs) {
  ...
  for (const assignment of assignments || []) {
    const matches = knownNames.get(assignment.column);
    if (!matches) {
    ...
    if (!String(assignment.column).includes('.') && matches.size > 1) {
      throw connectorSchemaError(
        'INTEGRATION_CONNECTOR_INPUT_AMBIGUOUS',
        `连接器输入字段 ${assignment.column} 在多个参数分组中存在，请使用完整路径。`,
        { column: assignment.column, candidates: Array.from(matches).sort() }
      );
    }
  }
}
