...
function projectConnectorAssignments(root) {
  const projected = [];
  const seen = new Set();

  function collectAssignments(rule, parentPath, assignments) {
    if (!rule || typeof rule !== 'object') { return; }
    const name = String(rule.name || '').trim();
    const path = parentPath && name ? `${parentPath}.${name}` : (name || parentPath);
    for (const appliedRule of Array.isArray(rule.rules) ? rule.rules : []) {
      if (!appliedRule || typeof appliedRule !== 'object') { continue; }
      const sensitive = path.split('.').pop().toLowerCase() === 'systemtoken';
      assignments.push({
        path,
        valueType: appliedRule.valueType || '',
        ...(sensitive
          ? { bound: appliedRule.value !== undefined && appliedRule.value !== null && String(appliedRule.value) !== '' }
          : { value: appliedRule.value }),
      });
    }
    for (const child of Array.isArray(rule.childList) ? rule.childList : []) {
      collectAssignments(child, path, assignments);
    }
  }
  ...
  function visit(value) {
    ...
    if (value.componentName === 'ConnectorNode') {
      const connectorRules = value.props && value.props.connectorRules;
      const assignments = [];
      for (const rule of connectorRules && Array.isArray(connectorRules.rules) ? connectorRules.rules : []) {
        collectAssignments(rule, '', assignments);
      }
      projected.push({
        nodeId: String(value.id || ''),
        connectorId: String(connectorRules && connectorRules.connectorId || ''),
        actionId: String(connectorRules && connectorRules.actionId || ''),
        connectionId: String(connectorRules && connectorRules.connectionId || ''),
        assignments: assignments.sort((left, right) => String(left.path).localeCompare(String(right.path))),
      });
    }
    Object.values(value).forEach(visit);
  }
...
async function verifyLogicflowFinalState(authRef, input, dependencies = {}) {
  ...
  let connectorAssignmentProjection = 'NOT_REQUESTED';
  if (Array.isArray(input.expectedConnectorAssignments) && input.expectedConnectorAssignments.length > 0) {
    const actualConnectorAssignments = projectConnectorAssignments(detail);
    if (!isDeepStrictEqual(actualConnectorAssignments, input.expectedConnectorAssignments)) {
      throw readbackError(
        'INTEGRATION_READBACK_CONNECTOR_ASSIGNMENTS_MISMATCH',
        t('integration.readback_connector_assignments_mismatch'),
        {
          processCode: input.processCode,
          formUuid: input.formUuid,
          expectedNodeCount: input.expectedConnectorAssignments.length,
          actualNodeCount: actualConnectorAssignments.length,
        }
      );
    }
    connectorAssignmentProjection = 'EXACT';
  }
