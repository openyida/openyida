 ...
 *   - listLogicflowDetailLogs：查询一次运行的节点日志详情
...
/**
 * 读取一次集成自动化运行的节点详情。此平台接口尚未公开，参数契约来自只读实测。
 */
async function listLogicflowDetailLogs(authRef, params) {
  const { appType, procInstId, pageIndex, pageSize } = params;
  if (!appType || !procInstId) {
    throw new Error(t('integration_detail_log.missing_args'));
  }
  const response = await createYidaClient({ authRef }).get(
    `/alibaba/web/${encodeURIComponent(appType)}/query/formLogicflowBinding/listDetailLog.json`,
    () => ({
      _api: 'Connector.listDetailLog',
      procInstId,
      pageIndex: String(pageIndex),
      pageSize: String(pageSize),
    }),
  ...
  if (!response || response.success !== true) {
    throw new Error(t('integration_detail_log.api_failed', response && response.errorMsg || 'unknown'));
  }
  const content = response.content;
  if (!content || !Array.isArray(content.data) || !Number.isSafeInteger(content.totalCount) || content.totalCount < 0) {
    throw new Error(t('integration_detail_log.invalid_response'));
  }
  return content;
}
...
async function getLogicflowDetail(authRef, params) {
  ...
  if (!response || !response.success) {
    const errorMsg = response ? response.errorMsg || JSON.stringify(response) : '请求失败';
    throw new Error(t('integration.detail_api_failed', errorMsg));
  }
  return response;
}
