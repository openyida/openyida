...
async function getFormSchema(authRef, params) {
  ...
  // V5 Schema 结构：pages[0].componentsTree[0].children
  // 递归提取所有字段组件（跳过布局容器，保留表单字段）
  const componentsTree = schemaContent
    && schemaContent.pages
    && schemaContent.pages[0]
    && schemaContent.pages[0].componentsTree
    && schemaContent.pages[0].componentsTree[0];

  if (!componentsTree || !Array.isArray(componentsTree.children)) {
    return [];
  }
  ...
  collectFields(componentsTree.children);
