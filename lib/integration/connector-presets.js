...
function buildConnectorRulesFromInputs(inputsSchema, assignments) {
  ...
  function countInputNames(items) {
    (items || []).forEach((item) => {
      if (!item || !item.name) { return; }
      inputNameCounts[item.name] = (inputNameCounts[item.name] || 0) + 1;
      countInputNames(item.childList);
    });
  }
  countInputNames(inputsSchema);
  ...
  function buildChildRules(input, parentPath) {
    ...
    const assign = assignMap[path]
      || (inputNameCounts[input.name] === 1 ? assignMap[input.name] : null);
    if (assign) {
