 ...
 *
 * integration-view-builder.js 构建 ConnectorNode 时只接受两类 schema：
 *   1. 平台只读 connector detail 对 action 的精确发现结果
 *   2. 按 `${connectorId}::${actionId}` 命中的固定已证 preset
 * 未命中时 fail-closed，不根据 assignments 猜字段类型。
 */
...
function inferInputLocation(input, inheritedLocation = '') {
  const explicit = String(input && input.paramLocation || '').trim().toLowerCase();
...
function buildConnectorInputIndex(inputsSchema) {
  const byPath = new Map();
  const byName = new Map();
  const leaves = [];

  function visit(items, parentPath = '', inheritedLocation = '') {
    for (const input of items || []) {
      if (!input || typeof input !== 'object' || !input.name) { continue; }
      const path = parentPath ? `${parentPath}.${input.name}` : input.name;
      const children = Array.isArray(input.childList) ? input.childList : [];
      const descriptor = {
        input,
        name: input.name,
        path,
        location: inferInputLocation(input, inheritedLocation),
        leaf: children.length === 0,
      };
      byPath.set(path, descriptor);
      if (!byName.has(input.name)) { byName.set(input.name, []); }
      byName.get(input.name).push(descriptor);
      if (descriptor.leaf) { leaves.push(descriptor); }
      visit(children, path, descriptor.location);
    }
  }

  visit(inputsSchema);
  return { byPath, byName, leaves };
}

function resolveConnectorAssignmentTarget(column, inputIndex) {
  const name = String(column || '').trim();
  if (!name) { return { state: 'unknown', column: name, candidates: [] }; }
  const exact = inputIndex.byPath.get(name);
  if (exact) { return { state: 'resolved', descriptor: exact }; }
  const candidates = inputIndex.byName.get(name) || [];
  if (candidates.length === 1) {
    return { state: 'resolved', descriptor: candidates[0] };
  }
  if (candidates.length > 1) {
    return {
      state: 'ambiguous',
      column: name,
      candidates: candidates.map(item => item.path).sort(),
    };
  }
  return { state: 'unknown', column: name, candidates: [] };
}
...
function buildConnectorRulesFromInputs(inputsSchema, assignments) {
  ...
  const inputIndex = buildConnectorInputIndex(inputsSchema);
  const assignMap = new Map();
  (assignments || []).forEach((a) => {
    if (a && a.column) {
      const resolved = resolveConnectorAssignmentTarget(a.column, inputIndex);
      if (resolved.state === 'resolved' && !assignMap.has(resolved.descriptor.path)) {
        assignMap.set(resolved.descriptor.path, a);
      }
    }
  });

  function buildChildRules(input, parentPath) {
    const cloned = cloneInputSchema(input);
    const path = parentPath ? `${parentPath}.${input.name}` : input.name;
    cloned.id = parentPath ? `${parentPath.replace(/\./g, '%')}%${input.name}` : input.name;
    cloned.parentId = parentPath ? parentPath.replace(/\./g, '%') : '';
    if (Array.isArray(cloned.childList)) {
      cloned.childList = cloned.childList.map(child => buildChildRules(child, path));
    }
    const assign = assignMap.get(path);
    if (assign) {
      const inner = cloneInputSchema(input);
      inner.id = cloned.id;
      inner.parentId = cloned.parentId;
      inner.valueType = assign.valueType || 'processVar';
      inner.value = normalizeTypedLiteral(assign.valueType, assign.value, input.componentName);
      inner.ruleId = assign.ruleId || generateConnectorRuleId();
      inner.valueLabel = assign.valueLabel || input.label || input.name;
      cloned.rules = [inner];
    } else {
      cloned.rules = [];
    }
  ...
  return inputsSchema.map((input) => {
    ...
    if (Array.isArray(base.childList)) {
      base.childList = base.childList.map(child => buildChildRules(child, input.name));
    }

    const assign = assignMap.get(input.name);
    if (assign) {
