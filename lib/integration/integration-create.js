...
async function findFormNavigationItem(authRef, appType, formUuid) {
  if (!formUuid) {
    return null;
  }
...
async function run(args) {
  ...
  if (addDataFormUuid) {
    try {
      addDataTargetForm = await findFormNavigationItem(authRef, appType, addDataFormUuid.toString());
    } catch (error) {
  ...
  if (initiateApprovalFormUuid) {
    try {
      initiateApprovalTargetForm = await findFormNavigationItem(authRef, appType, initiateApprovalFormUuid.toString());
    } catch (error) {
