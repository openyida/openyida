...
function buildPickerOption(value, label) {
  return {
    deep: 0,
    value,
    text: label,
...
function listTableFieldSummaries(components) {
  return (components || [])
    ...
    .map((component) => {
      ...
      return {
        ...buildPickerOption(fieldId, label),
        props: {
          fieldId,
          label: { zh_CN: label, en_US: label, type: 'i18n' },
        },
      };
    });
}
...
function buildSubTableRelativeItem(value, label) {
  return buildPickerOption(value, label);
}
...
function buildViewJson(options) {
  ...
  if (hasDataNode && dataNodeId) {
    ...
    const designerConditions = (rawConditions || []).map((condition) => {
      ...
      return designerFieldId === condition.bFieldId && designerFieldName === condition.bFieldName
        ? condition
        : {
          ...condition,
          bFieldId: designerFieldId,
          bFieldName: designerFieldName,
        };
    });
