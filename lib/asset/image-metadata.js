...
function svgDimensions(buffer) {
  ...
  const root = text.replace(/^\uFEFF/, '').replace(/^\s*(?:<\?xml[\s\S]*?\?>\s*)?(?:<!--[\s\S]*?-->\s*)*/, '')
    .match(/^<svg\b([^>]*)>/i);
  if (!root) {return null;}
  const attributes = root[1];
  const width = Number((attributes.match(/\swidth=["']([\d.]+)(?:px)?["']/i) || [])[1] || 0);
  const height = Number((attributes.match(/\sheight=["']([\d.]+)(?:px)?["']/i) || [])[1] || 0);
  if (width > 0 && height > 0) {return { type: 'svg', width, height };}
  const viewBox = (attributes.match(/\sviewBox=["']([^"']+)["']/i) || [])[1] || '';
