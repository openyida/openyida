 ...
 *   - http(s) 外链：由 resolve 流程确认可用后保留（source: external）；
 *     有 CDN 且 options.mirror=true 时镜像转存；不可用时记为缺口。
 *   - 空 / 占位：记为对应槽位缺口。
 *
 * materialStatus：
...
function getExternalDeliveryPolicy(rawUrl) {
  try {
    ...
    if (host === 'images.unsplash.com' || host.endsWith('.images.unsplash.com')) {
      return 'hotlink-only';
    }
    if (host === 'cdn.pixabay.com' || host.endsWith('.cdn.pixabay.com')) {
      return 'self-host-required';
    }
  } catch {
    return 'standard';
  }
...
async function resolveOne(candidate, ctx) {
  ...
  if (isHttpUrl(raw)) {
    ...
    if (v.ok) {
      const deliveryPolicy = getExternalDeliveryPolicy(v.finalUrl || raw);
      ...
      if (deliveryPolicy === 'self-host-required' && (!ctx.mirrorExternal || !ctx.canUpload)) {
        return {
          resolved: false,
          url: raw,
          source: 'external',
 ...
 * @param {boolean} [options.online] 是否可联网处理外链（默认 true）
 * @param {number}  [options.timeout] 单个外链处理超时
