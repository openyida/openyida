...
function deliveryRights(rawUrl, ctx = {}) {
  const provider = getCollectedImageProvider(rawUrl) || ctx.provider;
  const stock = (ctx.assetSource || 'search') === 'search';
  return {
    hotlinkAllowed: ctx.hotlinkAllowed ?? (isHttpUrl(rawUrl) && stock && !!ALLOWED_STOCK_IMAGE_PROVIDERS[provider]),
    rehostAllowed: ctx.rehostAllowed ?? !(stock && provider === 'unsplash'),
  };
}

function getExternalDeliveryPolicy(rawUrl, ctx = {}) {
  if (!isHttpUrl(rawUrl)) {return 'standard';}
  const rights = deliveryRights(rawUrl, ctx);
  if (rights.hotlinkAllowed && ctx.deliveryMode !== 'upload') {return 'original-url';}
  return rights.rehostAllowed ? 'yida-attachment' : 'blocked';
}
...
async function resolveOne(candidate, ctx = {}) {
  ...
  if (isHttpUrl(raw)) {
    ...
    const deliveryPolicy = getExternalDeliveryPolicy(raw, ctx);
    const rights = deliveryRights(raw, ctx);
    const common = { provider: inputProvider || ctx.provider || assetSource };
    const original = (metadata, reason = 'ASSET_TOO_LARGE') => {
      if (!rights.hotlinkAllowed) {return { ...common, resolved: false, reason };}
      ...
      return { ...common, ...metadata, imageType: metadata.type, resolved: true, url: raw,
        source: 'external', reason, deliveryPolicy: 'original-url' };
    };
    if (deliveryPolicy === 'blocked') {
      return { ...common, resolved: false, reason: 'REHOST_NOT_ALLOWED', deliveryPolicy };
    }
    if (deliveryPolicy === 'original-url') {
      const verified = await (ctx.verifyFn || verifyImageUrl)(raw, {
        timeout: ctx.timeout, minWidth: ctx.minWidth, minHeight: ctx.minHeight,
        allowUrl: assetSource === 'search' ? url => getCollectedImageProvider(url) === inputProvider : undefined,
      });
      if (!verified.ok) {return { ...common, resolved: false, reason: verified.reason };}
      return original({ ...verified, type: verified.imageType }, 'OK_DIRECT');
    }
    let tempPath = '';
    ...
    try {
      ...
      verifiedMetadata = { ...common, type: downloaded.type };
      const reused = await reuseDelivery(raw, common, ctx);
      if (reused) {return reused;}
      if (!ctx.canUpload) {return original(verifiedMetadata, 'YIDA_ATTACHMENT_UNAVAILABLE');}
      ...
      return original(verifiedMetadata, ((results || [])[0] || {}).error || 'UPLOAD_FAILED');
    } catch (error) {
      if (error.message === 'ASSET_TOO_LARGE') {return original(error.metadata || {});}
      if (verifiedMetadata) {return original(verifiedMetadata, error.code || error.message || 'UPLOAD_FAILED');}
  ...
  if (isLocalFile(raw)) {
    ...
    if (!deliveryRights(raw, ctx).rehostAllowed) {return { resolved: false, reason: 'REHOST_NOT_ALLOWED' };}
...
async function resolveAssets(assets = [], options = {}) {
  ...
  const actions = await Promise.all(entries.map(async (asset, index) => {
    ...
    try {
      result = await resolveOne(input, {
        ...
        hotlinkAllowed: asset.hotlinkAllowed,
        rehostAllowed: asset.rehostAllowed,
        deliveryMode: options.uploadAssets ? 'upload' : asset.deliveryMode,
        online,
        canUpload,
  ...
  actions.forEach(({ asset, input, source, result }, index) => {
    ...
    const output = {
      ...
      pageId: asset.pageId || '',
      input,
      deliveryMode: options.uploadAssets ? 'upload' : asset.deliveryMode || 'auto',
      ...Object.fromEntries(['hotlinkAllowed', 'rehostAllowed'].filter(key => asset[key] !== undefined).map(key => [key, asset[key]])),
      ...minimumSize(asset),
      ...
      licenseCheckedAt: asset.licenseCheckedAt || '',
      authorizationEvidence: [...(asset.authorizationEvidence || [])],
  ...
  return {
    ...
    capabilityEvidence: {
      ...
      deliveryPreference: 'verified-direct-url',
