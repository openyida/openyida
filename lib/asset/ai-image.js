...
const FREE_STOCK_LIBRARIES = [
  ...
  },
  {
    ...
    deliveryPolicy: 'API URL 只用于临时预览；选用后下载并托管到项目 CDN，禁止永久热链。',
  },
  {
    name: 'Undraw',
    ...
    deliveryPolicy: '许可不允许未经同意将 unDraw 做成自动搜索、抓取或下载集成；仅用于人工选取或取得授权的流程。',
  },
];
 ...
 * 检测是否存在「内置图片生成能力」。
 *
 * openyida 无内置文生图连接器；宿主未显式声明能力时返回 available:false，
 * 并由 agent 检查真实工具清单后接管素材来源（避免据产品名猜测或伪造 URL）。
 *
...
function detectImageGenerator(options = {}) {
  ...
  if (options.hostCapabilities && options.hostCapabilities.image_generation.available === true) {
    return {
      available: true,
      reason: '当前宿主显式声明提供图片生成工具；由宿主生成本地文件，再交给 asset resolve 处理。',
      delegateToAgent: true,
      source: 'host-capability-declaration',
    };
  }
  return {
    available: false,
