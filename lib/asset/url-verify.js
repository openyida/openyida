...
function requestOnce(rawUrl, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = client.request(parsed, {
      ...
      // Redirects and denied requests need no response-body download.
      ...
      res.on('end', finish);
      res.on('error', reject);
      res.on('aborted', () => reject(new Error('DOWNLOAD_ABORTED')));
    });
...
async function requestFollow(rawUrl, options, depth = 0) {
  if (options.allowUrl && !options.allowUrl(rawUrl)) {throw new Error('STOCK_PROVIDER_REDIRECT_NOT_ALLOWED');}
  const timeout = options.deadline - Date.now();
  if (timeout <= 0) {throw new Error('TIMEOUT');}
  const response = await requestOnce(rawUrl, { ...options, timeout });
  if (response.status >= 300 && response.status < 400 && response.headers.location) {
    if (depth >= MAX_REDIRECTS) {throw new Error('TOO_MANY_REDIRECTS');}
...
async function verifyImageUrl(rawUrl, options = {}) {
  ...
  let response;
  try {
    response = await requestFollow(trimmed, {
      method: 'GET', deadline: Date.now() + (options.timeout || DEFAULT_TIMEOUT), allowUrl: options.allowUrl,
    });
  } catch (error) {
  ...
  if (!metadata.type) {
    out.reason = 'INVALID_IMAGE_CONTENT';
    return out;
  }
...
function normalizeError(error) {
  ...
  if (['TIMEOUT', 'INVALID_URL', 'UNSUPPORTED_PROTOCOL', 'STOCK_PROVIDER_REDIRECT_NOT_ALLOWED', 'TOO_MANY_REDIRECTS', 'DOWNLOAD_ABORTED'].includes(message)) {return message;}
