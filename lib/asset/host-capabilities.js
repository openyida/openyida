...
const { getAssetExecutionContract, selectAssetExecutionMode } = require('./asset-execution');
...
const CAPABILITY_ENV = Object.freeze({
  ...
  backgroundAgent: 'OPENYIDA_AGENT_BACKGROUND_AGENT',
  backgroundShell: 'OPENYIDA_AGENT_BACKGROUND_SHELL',
});
...
function buildHostAssetCapabilities(options = {}) {
  ...
  const backgroundAgent = readDeclaredCapability(env[CAPABILITY_ENV.backgroundAgent]);
  const backgroundShell = readDeclaredCapability(env[CAPABILITY_ENV.backgroundShell]);
  const capabilities = [onlineSearch, imageSearch, imageGeneration, backgroundAgent, backgroundShell];
  ...
  return {
    ...
    execution: { selected_mode: selectAssetExecutionMode(backgroundAgent, backgroundShell), ...getAssetExecutionContract() },
    requires_host_tool_inventory_check: capabilities.some(item => item.status === 'unknown'),
    declaration_env: { ...CAPABILITY_ENV },
    detection_policy:
      'Explicit declarations override runtime defaults. QwenWork provides image generation, and verified non-cloud runtimes provide online and image search. Managed-cloud or unknown runtimes use the host tool inventory. Background agent and shell execution are separate capabilities; neither is inferred from host name. Declarations must match actual nonblocking tool semantics.',
  };
}
