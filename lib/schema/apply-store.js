...
function createOwnershipFile(lockPath, owner, options = {}) {
  ...
  try {
    ...
    if (!sameFileIdentity(finalIdentity, fileIdentity)) {
      throw new Error('lock file generation changed');
    }
    return captureFileIdentity(finalIdentity);
  } catch (error) {
    if (fd !== undefined) {
      try {
        fsImpl.closeSync(fd);
...
function cleanupCreatedOwnershipFile(lockPath, fileIdentity, fsImpl) {
  try {
    ...
    if (fileIdentity && sameFileIdentity(current, fileIdentity)) {
...
function removeOwnedLock(lockPath, owner, fsImpl, expectedIdentity) {
  if (expectedIdentity && !sameFileIdentity(fsImpl.lstatSync(lockPath), expectedIdentity)) {
    return false;
  }
  const current = readLockOwner(lockPath, fsImpl);
...
function createApplyLockAuthority(lockPath, options) {
  ...
  return Object.freeze({
    ...
    statePath,
    workspaceIdentity: captureFileIdentity(after),
    workspaceIdentityHash: createIdentityHash(after),
    workspaceRoot,
  });
}
...
function assertApplyLockAuthorityCurrent(authority, fsImpl) {
  ...
  if (
    ...
    !current.isDirectory() ||
    !sameFileIdentity(current, authority.workspaceIdentity)
  ) {
    throw new Error('workspace generation changed');
  }
}
...
function assertCurrentLockFileIdentity(state) {
  ...
  if (
    current.isSymbolicLink() ||
    !current.isFile() ||
    !sameFileIdentity(current, state.fileIdentity)
  ) {
    throw new Error('lock file generation changed');
  }
}
...
function createIdentityHash(stat) {
  return hashStable({ dev: String(stat.dev), ino: String(stat.ino) });
}
