...
const APP_CREATE_POST_WRITE_READBACK_MAX_ATTEMPTS = 6;
const APP_CREATE_POST_WRITE_READBACK_DELAY_MS = 500;
...
async function applySchema(input, options = {}) {
  ...
  try {
    ...
    const observed = deferredResourceIds.size > 0
      ? await readObservedForReviewedRecovery({
        deferredResourceIds,
        desiredResources,
        input,
        options,
      ...
      : await readInitialObserved(desiredResources, state, input, options, registry, lock);
...
async function resumeCreateIdentityOperation(operation, resource, state, context) {
  ...
  return { ...finalized, remoteWork: true };
}
...
async function finalizeOrdinaryResult(runtime) {
  ...
  try {
    ...
    postRead = await readPostWriteResource(runtime, proposedStateResource);
    await runtime.adapter.verify(postRead.projection, {
      ...runtime.adapterContext,
...
async function readPostWriteResource(runtime, proposedStateResource) {
  if (!shouldWaitForAppCreatePostWriteReadback(runtime, proposedStateResource)) {
  ...
  const retryOptions = appCreatePostWriteReadbackRetryOptions(runtime.context.options);
  let lastError;
  for (let attempt = 1; attempt <= retryOptions.maxAttempts; attempt++) {
    try {
      return await readAndProjectResource(
        runtime.context.resource,
        proposedStateResource,
        runtime.context.state,
        runtime.context.input,
      ...
      if (!isAppCreatePostWriteReadbackMissing(error, runtime) || attempt === retryOptions.maxAttempts) {
        throw error;
      }
      lastError = error;
      assertApplyLockHandleOwned(runtime.context.lock);
      await waitForAppCreatePostWriteReadback(retryOptions.delayMs);
      assertApplyLockHandleOwned(runtime.context.lock);
    }
  }
  throw lastError;
...
function shouldWaitForAppCreatePostWriteReadback(runtime, proposedStateResource) {
  const resource = runtime && runtime.context && runtime.context.resource;
  const appType = proposedStateResource &&
    proposedStateResource.bindings &&
    proposedStateResource.bindings.appType;
  return !!(
    runtime &&
...
function isAppCreatePostWriteReadbackMissing(error, runtime) {
  const resource = runtime && runtime.context && runtime.context.resource;
  return !!(
    error &&
    error.code === 'SCHEMA_APPLY_JIT_CONFLICT' &&
    error.message === 'Bound remote resource is missing.' &&
    error.details &&
    resource &&
    error.details.resourceType === 'app' &&
    error.details.key === resource.key
  );
}

function appCreatePostWriteReadbackRetryOptions(options = {}) {
  const config = options.appCreatePostWriteReadbackRetry || {};
  return {
    maxAttempts: positiveIntegerOrDefault(
      config.maxAttempts,
      APP_CREATE_POST_WRITE_READBACK_MAX_ATTEMPTS
    ),
    delayMs: nonNegativeIntegerOrDefault(
      config.delayMs,
      APP_CREATE_POST_WRITE_READBACK_DELAY_MS
    ),
  };
}
...
function waitForAppCreatePostWriteReadback(delayMs) {
  if (delayMs === 0) {
