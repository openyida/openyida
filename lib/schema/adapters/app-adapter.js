...
const appAdapter = {
  ...
  async resumeCreate(desired, createIdentity, context = {}) {
    ...
    const resource = context.resource || { desired, key: desired && desired.key };
    const observed = await this.readObserved({ bindings }, context);
    const projection = this.projectObserved(observed, { bindings }, {
      ...context,
      desired,
      resource,
    });
    this.verify(projection, {
      ...context,
      resource,
    });
