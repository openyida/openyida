...
const pageAdapter = {
  ...
  normalize(entry, context = {}) {
    ...
    const compiled = compileNativePageSource(loaded);
  ...
  classifyObservedConflict(input = {}) {
    ...
    if (
      ...
      observed.profile !== NATIVE_PAGE_PROFILE ||
      !observed.sourceHash ||
  ...
  async prepareOperation(input, context = {}) {
    ...
    const patch = buildNativePageSchemaPatch({
      ...
      desiredTitle: input.resource.desired.title,
      expectedRemoteSchemaHash: currentProjection.remoteSchemaHash,
      observedFormType: observed.observedFormType,
      observedTitle: observed.observedTitle,
      remoteSchema: observed.schema,
      shellProfile: profile,
    });
    return { compiled, patch, profile };
  },
...
async function continuePageCreate(desired, bindingInput, context) {
  ...
  const shellProfile = resolveInitialProfile(context);
  ...
  const patch = buildDataSourceOnlyNativePagePatch({
    compiledPage: compiled,
...
function compileResourceSource(resource, context) {
  ...
  return compileNativePageSource(loadPageSource(resource.source, {
    fsImpl: context.fsImpl,
...
function validateManagedPage(value, pointer) {
  ...
  if (
    ...
    value.profile !== NATIVE_PAGE_PROFILE ||
