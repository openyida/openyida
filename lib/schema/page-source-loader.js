...
function loadPageSource(sourcePath, options = {}) {
  ...
  assertNativeSourcePath(relativePath);
  ...
  try {
    ...
    if (!sameFileIdentity(initialPathStat, before)) {
...
function assertNativeSourcePath(relativePath) {
  if (CANVAS_SOURCE_PATTERN.test(relativePath) || !NATIVE_SOURCE_PATTERN.test(relativePath)) {
    sourceError('SCHEMA_PAGE_SOURCE_PROFILE_UNSUPPORTED', 'Page source must use the native/default profile.');
  }
}
...
function verifyPathAfterRead(fsImpl, rootRealPath, absolutePath, initialRealPath, fdStat) {
  ...
  if (currentPathStat.isSymbolicLink() || !sameFileIdentity(currentPathStat, fdStat)) {
    sourceError('SCHEMA_PAGE_SOURCE_READ_FAILED', 'Page source changed while it was being read.');
  }
}
