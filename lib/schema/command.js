...
function isSecurityFailure(code) {
  return /(?:FORBIDDEN|INTEGRITY|PATH_UNSAFE|ENVIRONMENT_MISMATCH|LOCKED|LOCK_FAILED|LOCK_LOST)/.test(code);
}

function isValidationFailure(code) {
  return /(?:INVALID|MANIFEST_|FIELD_|REFERENCE_|DEPENDENCY_|DUPLICATE_|UNKNOWN_PROPERTY)/.test(code);
}
