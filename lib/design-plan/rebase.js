...
function rebaseParts(source, input, businessFile, visualFile) {
  ...
  if (source.meta?.status !== 'draft' || source.meta?.planState?.presentedRevision
      || source.meta?.planState?.planConfirmed || !equal(source.meta, saved.plan.meta)) {
  ...
  return parts.map((part, index) => {
    const conflicts = [];
    const facts = Object.fromEntries(Object.keys(part.facts).map(key => [key,
