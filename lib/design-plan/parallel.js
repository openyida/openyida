...
function readPart(file, role, base, allowed, supplied) {
  const part = supplied || readJson(file, `${role} 规划片段`);
  ...
  if (part.base?.revision !== base.revision || part.base?.digest !== base.digest) {
    throw new CliError(`${role} 规划来自旧版本，请根据当前计划重新整理`, { code: 'DESIGN_PLAN_STALE_PART',
      details: { sourcePath: file, expectedBase: base, actualBase: part.base, nextStep: t('design_plan.rebase_required') } });
  }
...
function mergeParts(source, businessFile, visualFile, options = {}) {
  const base = planBase(source);
  const rebased = options.rebaseParts ? require('./rebase').rebaseParts(source, options.input, businessFile, visualFile) : [];
  const business = readPart(businessFile, 'business', base, ['overview', 'dataModels', 'businessFlows', 'pages', 'execution', 'meta'], rebased[0]);
  const visual = readPart(visualFile, 'visual', base, ['visualStyle'], rebased[1]);
