...
function readPart(file, base) {
  ...
  const unsupported = keys.filter(key => !MODULES.includes(key));
  if (!keys.length || unsupported.length) {
    invalid(`facts modules: ${keys.length ? unsupported.join(', ') : 'none'}; allowed: ${MODULES.join(', ')}`);
  }
...
function preview(inputPath, options = {}) {
  ...
  try {
    ...
    if (accepted.visualStyle && changed.some(key => ['visualStyle', 'pages'].includes(key))) {
      // Optional theme recipes do not determine whether a real page exists.
      // Preserve accepted page drafts; final materialization checks completeness.
      const design = renderDesign(plan, { outputDir: dir, draft: true });
...
function finalizePreview(source, stateFile) {
  ...
  if (require('./patch').invalidateConfirmation(merged, source) || merged.meta.status === 'draft') {
