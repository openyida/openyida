...
function readIndex(filePath, label) {
  try {
    return JSON.parse(fs.readFileSync(filePath, 'utf8'));
  } catch (error) {
    throw new CliError(`${label}不可读或不是有效 JSON：${filePath}`, {
      code: 'DESIGN_PLAN_INDEX_INVALID',
...
function pageApplicationDefaults(page, theme, color) {
...
function ensureVisual(plan, theme) {
  ...
  forUser.pageApplications = list((plan.pages || {}).customPageDetails).map(page => {
    ...
    const application = { ...pageApplicationDefaults(page, theme, color), ...source };
...
function normalizePlan(sourcePlan) {
  ...
  const themeIndex = readIndex(THEME_INDEX_PATH, '主题索引');
  const pagePatternIndex = readIndex(PAGE_PATTERN_INDEX_PATH, '页面模式索引');
  ensureOverview(plan);
