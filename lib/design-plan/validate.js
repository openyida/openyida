...
const { ENTRY_MODES } = require('./entry-navigation');
...
function collectIssues(plan) {
  ...
  if (recommendation !== undefined && check(object(recommendation), 'execution.entryRecommendation', '填写访问态入口方案')) {
    check(ENTRY_MODES.includes(recommendation.mode), 'execution.entryRecommendation.mode', t('cli.design_plan_entry_mode_required'));
    if (array(recommendation.entries, 'execution.entryRecommendation.entries', true)) {
      if (recommendation.mode === 'backend-only') {
        check(recommendation.entries.every(entry => entry?.role === 'management'), 'execution.entryRecommendation.entries', t('cli.design_plan_backend_only_roles'));
      }
...
function validateAuthoring(plan) {
  ...
  // Report every gap here: stricter downstream validators would surface it with a different error shape.
  if (issues.length) {
