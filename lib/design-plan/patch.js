...
function readPlan(inputPath) {
  ...
  try {
    ...
    throw new CliError(`build-plan.json 不可读：${inputPath}`, {
      code: 'DESIGN_PLAN_FILE_UNREADABLE',
      details: { inputPath, cause: error.message },
    });
  }
  try {
    return JSON.parse(source);
  } catch (error) {
    throw new CliError(`build-plan.json 不是有效 JSON：${inputPath}`, {
...
function parsePath(pathExpression) {
  ...
  if (PROTECTED_PATHS.some(protectedPath => expression === protectedPath || expression.startsWith(`${protectedPath}.`))) {
...
function setExistingPath(target, change) {
  ...
  if (current === null || current === undefined || !Object.prototype.hasOwnProperty.call(current, leaf)) {
    throw new CliError(`字段路径不存在：${change.path}`, {
      code: 'DESIGN_PLAN_PATH_NOT_FOUND',
    });
  }
...
function patchPlan(inputPath, setExpressions, options = {}) {
  ...
  const original = readPlan(resolvedInput);
  ...
  atomicWriteJson(resolvedInput, patched);
