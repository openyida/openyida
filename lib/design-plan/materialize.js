...
function renderPrd(plan) {
  ...
  const lines = [
    ...
    '## 2. 应用配置', '', mdTable(['配置项', '值'], [['导航类型', NAVIGATION_TYPES[handoff.appConfig.navigationType].label], ...Object.entries(handoff.appConfig)]), '',
    '### 页面导航配置', '', mdTable(['页面', '显示平台页面导航'], handoff.pageNavigation.map(page => [page.name, page.isRenderNav])), '',
    '## 3. 数据结构（业务语义，不含细节 ID）',
  ];
  for (const model of models) {
