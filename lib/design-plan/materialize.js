...
function deriveSurfacePalette(primaryColor, surfaceTone, template) {
  ...
  return {
    '--pod-page-bg-color': mixHex(primaryColor, [255, 255, 255], 0.94),
...
function renderDesign(plan) {
  ...
  if (Object.keys(palette).length) {
    output = injectAfterFrontmatter(output, [
      '## 项目配色适配（优先于模板默认配色）', '',
      `用户选择的 ${text(color.primaryColorName, primaryColor)} 用于整体浅色氛围。页面、填充和边界已按同一主色派生，项目显式 token 覆盖优先；最终取值以本文 tokens 为准。`,
