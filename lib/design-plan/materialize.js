...
function validateBusiness(plan, execution) {
  ...
  if (execution.appConfig !== undefined) {
    ...
    const fields = ['appType', 'corpId', 'baseUrl', 'hideAppNav', 'layoutDirection', 'navTheme', 'logoSource'];
...
function pageHandoff(page, plan) {
  ...
  const entryMode = supplied.entryMode || page.entryMode || 'platform-shell';
...
function buildExecution(plan) {
  ...
  const visualConfig = { layoutDirection: navigationStyle.structure, navTheme: navigationStyle.tone, logoSource: 'appIcon' };
  ...
  const handoff = {
    ...
    appConfig: { appType: '待创建后回填', corpId: '待登录态确认', baseUrl: '待目标环境确认', hideAppNav: 'n', ...execution.appConfig, ...visualConfig },
  ...
  const resources = [...models, ...pages];
  requireFact(handoff.resourceBlueprint.every(resource => object(resource) && nonEmpty(resource.name) && nonEmpty(resource.type)), '资源蓝图缺少名称或类型');
  requireFact(resources.every(resource => handoff.resourceBlueprint.some(item => item.name === resource.name)), '资源蓝图遗漏业务表单或页面');
...
function renderPrd(plan) {
  ...
  const lines = [
    ...
    '## 2. 应用配置', '', mdTable(['配置项', '值'], Object.entries(handoff.appConfig)), '',
    '## 3. 数据结构（业务语义，不含细节 ID）',
  ];
  for (const model of models) {
...
function renderDesign(plan) {
  ...
  output = output.replace(/^---\n/, () => [
    '---',
    `themeProfile: ${JSON.stringify({ themeColor: primaryColor, navTheme: navigationStyle.tone, layoutDirection: navigationStyle.structure, logoSource: 'appIcon', themeDelivery: 'app-custom-theme-file' })}`,
...
function prepareArtifacts(sourcePlan, outputDir) {
  ...
  applyDesignTokens(fs.readFileSync(path.join(PACKAGE_ROOT, 'yida-skills/skills/yida-design/references/theme/app-custom-theme-template.css'), 'utf8'), design);
  ...
  try {
    fs.writeFileSync(normalizedInput, `${JSON.stringify({ ...plan, execution: buildExecution(plan) }, null, 2)}\n`, 'utf8');
    ...
    return {
      revision: (plan.meta || {}).revision || null, outputs,
      files: [[outputs.prd, prd], [outputs.design, design], [outputs.html, html]],
    };
  } finally {
...
function materialize(inputPath, options = {}) {
  ...
  const artifacts = prepareArtifacts(readJson(input), outputDir);
  ...
  if (!options.check) {writeFiles(artifacts.files);}
  return { success: true, checked: options.check === true, input, outputDir, revision: artifacts.revision, outputs: artifacts.outputs };
}
