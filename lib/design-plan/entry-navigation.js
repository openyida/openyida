...
const assert = (condition, message) => {
  if (!condition) {throw new CliError(message, { code: 'DESIGN_PLAN_INVALID_ENTRY_NAVIGATION' });}
};
    ...
    const visit = items => {
      for (const item of items) {
        ...
        if (item.targetType === 'local') {
          assert(page && item.resource === page.name && filled(item.viewKey), `本地菜单 ${item.label} 必须关联承载页面和 viewKey`);
        }
