...
const ENTRY_MODES = Object.freeze(['unified', 'service-management', 'frontend-only', 'backend-only']);
const ENTRY_MODE_SUMMARIES = Object.freeze({
  unified: '不分前后台：使用同一套业务页面，按职责显示菜单、数据和操作。',
  'service-management': '分前后台：办事的人使用前台，处理业务的人使用后台，两边共享业务数据。',
  'frontend-only': '只有访问前台：本次提供办理、查询或展示页面；需要处理的业务由已有系统、人员入口或自动流程承接。',
  'backend-only': '只有访问后台：工作人员录入、受理、处理或维护业务，办事入口在别处或本次不需要。',
});
...
function resolvePageEntryMode(page, plan, { sceneKey, customNavigation }) {
  const supplied = page.pageSpecHandoff || {};
  const entries = plan.execution?.entryRecommendation?.entries;
  const servicePage = Array.isArray(entries) && entries.some(entry =>
    entry?.role === 'service' && filled(entry.sceneKey) && entry.sceneKey === sceneKey);
  return supplied.entryMode || page.entryMode ||
    (servicePage || customNavigation || supplied.navigation ? 'standalone' : 'platform-shell');
}
...
function buildEntryNavigation(plan, resources, pages) {
  ...
  assert(object(recommendation) && ENTRY_MODES.includes(recommendation.mode), t('cli.design_plan_entry_mode_required'));
  ...
  for (const entry of recommendation.entries) {
    ...
    const page = filled(entry.sceneKey) ? pages.find(item => item.sceneKey === entry.sceneKey) : undefined;
  ...
  assert(recommendation.mode !== 'backend-only' || roles.every(role => role === 'management'), t('cli.design_plan_backend_only_roles'));
  return { recommendation: JSON.parse(JSON.stringify(recommendation)), platformOrder: [...new Set(platformOrder)] };
}

module.exports = { buildEntryNavigation, resolvePageEntryMode, ENTRY_MODES, ENTRY_MODE_SUMMARIES };
