...
const DEFAULT_PROCESS_BINDING_READBACK_ATTEMPTS = 4;
const DEFAULT_PROCESS_BINDING_READBACK_DELAY_MS = 250;
...
async function readConvertedProcessMode(authRef, input, options = {}) {
  const attempts = Math.min(
    positiveInteger(options.attempts, DEFAULT_PROCESS_BINDING_READBACK_ATTEMPTS),
    10
  );
  const delayMs = Math.max(0, Number.isFinite(Number(options.delayMs))
    ? Number(options.delayMs)
    : DEFAULT_PROCESS_BINDING_READBACK_DELAY_MS);
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
      lastObservation = await readFormMode(authRef, input, { oneShot: true });
      lastError = null;
      if (
        lastObservation
        && lastObservation.mode === 'process'
        && lastObservation.processCode
      ) {
        return { mode: lastObservation, attempts: attempt, lastError: null };
      }
    } catch (error) {
      lastError = error;
    }
  ...
  return { mode: null, attempts, lastObservation, lastError };
}
...
async function run(args, options) {
  ...
  if (startedAsReceipt) {
    ...
    const conversionReadback = await readConvertedProcessMode(
      authRef,
      { appType, formUuid },
      {
        attempts: runOptions.processCodeReadbackAttempts,
        delayMs: runOptions.processCodeReadbackDelayMs,
        sleep: runOptions.sleep,
      }
    );
    if (!conversionReadback.mode) {
      failure(
        'NON_IDEMPOTENT_RESULT_UNKNOWN',
        t('common.non_idempotent_result_unknown'),
        'resolve_process_code',
        conversionReadback.lastError || conversionReadback.lastObservation,
        {
          ...
          readbackAttempts: conversionReadback.attempts,
          readbackTarget: { appType, formUuid, resourceType: 'form_process_binding' },
          nextStep: t('process_diagnostics.unknown_result'),
        }
      );
    }
    processCode = conversionReadback.mode.processCode;
