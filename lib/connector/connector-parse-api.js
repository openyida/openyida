...
function generateTableReport(curlData, docData, operationConfig) {
    ...
    let authType = '无身份验证';
    if (curlData) {
        const auth = (0, curl_parser_1.detectAuthType)(curlData.headers);
        authType = auth.type;
    }
...
async function run(args) {
    ...
    if (options.curl) {
        ...
        curlData = (0, curl_parser_1.parseCurl)(options.curl);
    }
    if (options.doc) {
        console.log(`🔍 正在解析接口文档: ${options.doc}`);
        try {
            docData = (0, doc_parser_1.parseAPIDoc)(options.doc);
            operationConfig = (0, doc_parser_1.convertToOperationConfig)(docData);
        }
        catch (error) {
    ...
    if (options.output && operationConfig) {
        fs_1.default.writeFileSync(options.output, JSON.stringify([operationConfig], null, 2));
        console.log(`💾 配置文件已保存到: ${options.output}`);
    }
}
//# sourceMappingURL=connector-parse-api.js.map