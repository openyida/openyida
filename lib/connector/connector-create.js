...
async function run(args) {
    ...
    if (options.isUpdateMode) {
        ...
        const connector = await (0, api_1.findConnectorById)(options.connectorId, authRef);
        if (!connector) {
        ...
        const detail = await (0, api_1.getConnectorDetail)(connector.connectorName, authRef);
        const currentOperations = JSON.parse(detail.operations || '[]');
        const newDesc = (0, api_1.buildConnectorDesc)(options.description || null, connector.connectorDesc || null, authRef, currentOperations);
        await (0, api_1.saveConnector)({
    ...
    let operations;
    try {
        const opsContent = fs_1.default.readFileSync(options.operationsFile, 'utf-8');
    ...
    const connectorDesc = (0, api_1.buildConnectorDesc)(options.description ?? null, null, authRef, operations);
    const saveResult = await (0, api_1.saveConnector)({
...
//# sourceMappingURL=connector-create.js.map