/**
 * connector create - 创建或更新宜搭 HTTP 连接器
 *
 * 用法：
 *   创建: openyida connector create "连接器名称" "https://api.example.com" --operations ./ops.json [选项]
 *   更新: openyida connector create --id <connector-id> [--desc "新描述"]
 *
 * 选项:
 *   --id <id>                  连接器 ID（用于更新现有连接器）
 *   --auth <type>              鉴权方式: 无身份验证, 基本身份验证, API密钥, 钉钉开放平台验证, 阿里云API网关, 钉钉零信任网关
 *   --username <username>      Basic Auth 用户名
 *   --password <password>      Basic Auth 密码
 *   --api-key-label <label>    API Key 显示标签
 *   --api-key-name <name>      API Key 参数名
 *   --api-key-location <loc>   API Key 位置: HEADER 或 QUERY
 *   --desc <description>       连接器描述
 *   --icon <icon>              图标代码
 *   --operations <file>        执行动作配置文件 (JSON 格式，创建时必填)
 */

'use strict';
const { fail } = require('../core/chalk');

const fs = require('fs');
const {
  getAuthRef,
  buildConnectorDesc,
  findConnectorById,
  getConnectorDetail,
  saveConnector,
} = require('./api');
const { normalizeOperations } = require('./operation-normalizer');
const { safeJsonStringify } = require('../core/redact');
const { CliError } = require('../core/cli-error');
const { t } = require('../core/i18n');
const { HTTP_CONNECTOR_MODE } = require('./contract');
const { buildConnectorSaveParams, createActionSnapshot } = require('./connector-action-update');
const { assertNoExplicitSystemToken } = require('./yida-system-token');

function showUsage() {
  console.log(`
用法:
  # 创建新连接器
  openyida connector create "连接器名称" "https://api.example.com" --operations ./ops.json [选项]

  # 更新现有连接器
  openyida connector create --id <connector-id> [--desc "新描述"]

选项:
  --id <id>                  连接器 ID（用于更新现有连接器）
  --auth <type>              鉴权方式: 无身份验证, 基本身份验证, API密钥, 钉钉开放平台验证, 阿里云API网关, 钉钉零信任网关
  --username <username>      Basic Auth 用户名
  --password <password>      Basic Auth 密码
  --api-key-label <label>    API Key 显示标签 (默认: API Key)
  --api-key-name <name>      API Key 参数名 (默认: X-API-Key)
  --api-key-location <loc>   API Key 位置: HEADER 或 QUERY (默认: HEADER)
  --desc <description>       连接器描述
  --icon <icon>              图标代码 (默认: chaxun%%#FFA200)
  --operations <file>        执行动作配置文件 (JSON 格式，创建时必填)

示例:
  openyida connector create "测试API" "api.example.com" --operations ./ops.json
  openyida connector create "内部系统" "internal.company.com" --auth "基本身份验证" --username admin --password 123456 --operations ./ops.json
  openyida connector create "第三方API" "api.example.com" --auth "API密钥" --api-key-label "Authorization" --api-key-name "X-API-Key" --operations ./ops.json
  openyida connector create --id 910241 --desc "新的描述内容"
`);
}

const AUTH_TYPE_MAP = {
  '无身份验证': 'NONE',
  '基本身份验证': 'BASIC',
  'API密钥': 'API_KEY',
  '钉钉开放平台验证': 'DINGTALK',
  '阿里云API网关': 'ALIYUN',
  '钉钉零信任网关': 'DINGTRUST',
};

function parseArgs(args) {
  const idIndex = args.indexOf('--id');
  const isUpdateMode = idIndex !== -1;

  if (!isUpdateMode) {
    if (args.length < 2) {
      fail('错误: 创建连接器需要提供名称和域名');
      showUsage();
      process.exit(1);
    }
    if (args[0].startsWith('--')) {
      fail(`错误: 第一个参数 "${args[0]}" 看起来是一个选项，应该是连接器名称`);
      process.exit(1);
    }
    if (args[1].startsWith('--')) {
      fail(`错误: 第二个参数 "${args[1]}" 看起来是一个选项，应该是域名`);
      process.exit(1);
    }
  }

  const options = {
    isUpdateMode,
    connectorId: isUpdateMode ? args[idIndex + 1] : null,
    name: isUpdateMode ? null : args[0],
    baseUrl: isUpdateMode ? null : args[1],
    authType: 'NONE',
    apiKeyLocation: 'HEADER',
    apiKeyName: 'X-API-Key',
    icon: 'chaxun%%#FFA200',
  };

  for (let i = isUpdateMode ? 0 : 2; i < args.length; i++) {
    switch (args[i]) {
      case '--help':
      case '-h':
        showUsage();
        process.exit(0);
        break;
      case '--id':
        options.connectorId = args[++i];
        break;
      case '--auth':
        options.authType = AUTH_TYPE_MAP[args[++i]] || args[i].toUpperCase();
        break;
      case '--username':
        options.username = args[++i];
        break;
      case '--password':
        options.password = args[++i];
        break;
      case '--api-key-label':
        options.apiKeyLabel = args[++i];
        break;
      case '--api-key-name':
        options.apiKeyName = args[++i];
        break;
      case '--api-key-location':
        options.apiKeyLocation = args[++i]?.toUpperCase();
        break;
      case '--app-key':
      case '--app-secret':
        throw new CliError(t('connector_auth.create_rejects_credentials'), {
          code: 'CONNECTOR_CREDENTIALS_NOT_ACCEPTED_ON_CREATE',
          details: {
            retrySafe: true,
            sideEffectState: 'none',
            nextStep: t('connector_auth.create_credentials_next_step'),
          },
        });
      case '--desc':
        options.description = args[++i];
        break;
      case '--icon':
        options.icon = args[++i];
        break;
      case '--operations':
        options.operationsFile = args[++i];
        break;
      case '--json':
        options.json = true;
        break;
    }
  }

  return options;
}

function buildSecuritySchemes(options) {
  switch (options.authType) {
    case 'BASIC':
      return JSON.stringify({
        BasicAuth: {
          username: '用户名',
          password: '密码',
          type: 'http',
          scheme: 'basic',
        },
      });
    case 'API_KEY':
      return JSON.stringify({
        ApiKeyAuth: {
          label: options.apiKeyLabel || 'API Key',
          name: options.apiKeyName || 'X-API-Key',
          location: options.apiKeyLocation === 'QUERY' ? 'query' : 'header',
        },
      });
    case 'DINGTALK':
      return JSON.stringify({ DingAuth: {} });
    case 'ALIYUN':
      return JSON.stringify({ AliyunApiGateway: {} });
    case 'DINGTRUST':
      return JSON.stringify({ DingTrustGW: {} });
    case 'NONE':
    default:
      return '{}';
  }
}

function parseBaseUrl(url) {
  try {
    const urlWithScheme = url.match(/^https?:\/\//) ? url : 'https://' + url;
    const parsed = new URL(urlWithScheme);
    return {
      scheme: parsed.protocol.replace(':', ''),
      host: parsed.hostname,
      basePath: parsed.pathname || '/',
    };
  } catch (e) {
    return {
      scheme: 'https',
      host: url.replace(/^https?:\/\//, '').replace(/\/.*$/, ''),
      basePath: '/',
    };
  }
}

function generateConnectorName() {
  return 'Http_' + Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15);
}

async function run(args) {
  if (!args || args.length === 0 || args[0] === '--help' || args[0] === '-h') {
    showUsage();
    process.exit(0);
  }

  const options = parseArgs(args);
  const log = options.json ? () => {} : console.log;
  const authRef = getAuthRef();

  // 更新模式
  if (options.isUpdateMode) {
    log('🔧 正在更新连接器...\n');
    log(`连接器 ID: ${options.connectorId}\n`);

    const connector = await findConnectorById(options.connectorId, authRef);
    if (!connector) {
      fail('未找到该连接器');
      process.exit(1);
    }

    log(`找到连接器: ${connector.displayName}`);
    log(`当前描述: ${connector.connectorDesc || '(空)'}\n`);

    const detail = await getConnectorDetail(connector.connectorName, authRef);
    const snapshot = createActionSnapshot({
      ...detail,
      id: detail.id === undefined ? connector.id : detail.id,
    });
    const currentOperations = snapshot.operations;
    const newDesc = buildConnectorDesc(options.description, connector.connectorDesc, authRef, currentOperations);

    await saveConnector({
      ...buildConnectorSaveParams(snapshot, currentOperations),
      connectorDesc: newDesc,
    }, authRef);

    log('✅ 连接器更新成功!');
    log('\n新描述:');
    log(newDesc);
    return;
  }

  // 创建模式
  log('🔧 正在创建连接器...\n');
  log(`名称: ${options.name}`);
  log(`基础域名: ${options.baseUrl}`);
  log(`鉴权方式: ${options.authType}`);

  if (!options.operationsFile) {
    fail('错误: 创建连接器必须提供 --operations 参数');
    fail('宜搭平台不允许创建没有执行动作的连接器');
    process.exit(1);
  }

  let operations;
  try {
    const opsContent = fs.readFileSync(options.operationsFile, 'utf-8');
    operations = JSON.parse(opsContent);
    if (!Array.isArray(operations) || operations.length === 0) {
      fail('错误: operations 文件不能为空数组');
      process.exit(1);
    }
    operations = normalizeOperations(operations);
    assertNoExplicitSystemToken(operations, { source: '--operations' });
    log(`✓ 已加载 ${operations.length} 个执行动作`);
  } catch (e) {
    fail(`读取执行动作配置文件失败: ${e.message}`);
    process.exit(1);
  }

  const urlInfo = parseBaseUrl(options.baseUrl);
  const securitySchemes = buildSecuritySchemes(options);
  const connectorName = generateConnectorName();
  const connectorDesc = buildConnectorDesc(options.description, null, authRef, operations);

  const saveResult = await saveConnector({
    operations: JSON.stringify(operations),
    displayName: options.name,
    iconUrl: options.icon,
    connectorDesc,
    host: urlInfo.host,
    baseUrl: urlInfo.basePath,
    scheme: urlInfo.scheme,
    tongxunluTemplateId: '',
    faasTemplateId: '0',
    securitySchemes,
    connectorMode: String(HTTP_CONNECTOR_MODE),
    connectorName,
    category: 'http',
  }, authRef);

  const connectorId = saveResult.connectorId;
  const detailUrl = connectorId
    ? `https://yidalogin.aliwork.com/platformManage/customConnectorFactory/update?id=${connectorId}&connectorName=${connectorName}&mode=http`
    : null;
  const accountManageUrl = connectorName
    ? `https://yidalogin.aliwork.com/platformManage/customConnectorFactory?connectorName=${encodeURIComponent(connectorName)}&action=accountManage`
    : null;

  log('\n✅ 连接器创建成功!');
  if (connectorId) {
    log(`\nID: ${connectorId}`);
  }
  log(`连接器名称: ${connectorName}`);
  log(`显示名称: ${options.name}`);
  log(`基础域名: ${options.baseUrl}`);
  if (detailUrl) {
    log(`\n🔗 查看连接器:\n   ${detailUrl}`);
    log(`\n🔐 添加授权账号:\n   ${accountManageUrl}`);
  } else {
    log('\n💡 下一步: 查看连接器列表获取 ID');
    log('   openyida connector list');
  }
  const result = {
    success: true,
    connectorId,
    connectorName,
    displayName: options.name,
    detailUrl,
    accountManageUrl,
    readbackVerified: saveResult.readbackVerified === true,
  };
  if (options.json) {
    console.log(safeJsonStringify(result, 0));
  }
  return result;
}

module.exports = { buildSecuritySchemes, parseArgs, parseBaseUrl, run };
