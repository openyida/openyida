...
function normalizeParamNode(node, paramLocation) {
  ...
  // 宜搭连接器运行时会错误地把 required Header 判为空，即使页面已经传值。
  // Header 的业务必填性由调用方校验，动作 Schema 始终保存为非必填。
...
function normalizeParameters(parameters, inputs) {
  ...
  normalized.header = normalized.header.filter(entry => (
    entry
    && typeof entry === 'object'
