...
const { buildHeaderDefaultValue } = require('./action-generator');
const { slugifyOperationId } = require('./operation-normalizer');
...
function convertToOperationConfig(parseResult) {
  const operationId = slugifyOperationId(parseResult.basicInfo.title, '')
    || slugifyOperationId(parseResult.serverInfo.path, 'operation');
  const operation = {
  ...
  if (parseResult.requestInfo.headers && parseResult.requestInfo.headers.length > 0) {
    const headerChildren = parseResult.requestInfo.headers.map(h => ({
      ...
      defaultValue: buildHeaderDefaultValue(h.name, h.example || h.value || '')
    }));
    ...
    operation.parameters.header = parseResult.requestInfo.headers.map(h => ({
      name: h.name,
      value: buildHeaderDefaultValue(h.name, h.example || h.value || '')
    }));
  }
