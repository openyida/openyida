...
const action_generator_1 = require("./action-generator");
const desc_generator_1 = require("./desc-generator");
function showUsage() {
    console.log(`
用法: openyida connector smart-create --curl "curl命令" [选项]
...
function phase1Parse(options) {
    ...
    const curlData = (0, curl_parser_1.parseCurl)(options.curl);
    const authType = (0, curl_parser_1.detectAuthType)(curlData.headers);
    console.log('✅ 解析结果:');
...
async function phase2Match(curlData, authType, authRef) {
    ...
    const { connectors } = await (0, api_1.listConnectors)({ pageSize: 100 }, authRef);
    ...
    if (matchingConnectors.length > 0) {
        ...
        (0, api_1.printTable)(headers, rows);
        console.log('\n💡 请选择操作方式:');
...
function phase3Generate(curlData) {
    ...
    const relevantHeaders = (0, curl_parser_1.filterBrowserHeaders)(curlData.headers);
    const operation = (0, action_generator_1.generateOperation)(curlData, relevantHeaders);
    const connectorDesc = (0, desc_generator_1.generateConnectorDesc)(curlData, operation);
    console.log('✅ 生成的配置:');
...
function phase4Output(options, curlData, authType, matchingConnectors, operation, connectorDesc) {
    ...
    fs_1.default.writeFileSync(tempFile, JSON.stringify([operation], null, 2));
...
async function run(args) {
    ...
    const authRef = (0, api_1.getAuthRef)();
...
//# sourceMappingURL=connector-smart-create.js.map