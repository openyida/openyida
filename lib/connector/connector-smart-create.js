/**
 * connector smart-create - 生成安全连接器动作草稿（不创建远端资源）
 *
...
说明: 仅生成脱敏的动作草稿和后续命令，不创建或更新远端连接器。
...
function phase4Output(options, curlData, authType, matchingConnectors, operation, connectorDesc) {
  const draftRoot = path.join(process.cwd(), '.cache', 'openyida', 'connector-drafts');
  fs.mkdirSync(draftRoot, { recursive: true });
  const draftDir = fs.mkdtempSync(path.join(draftRoot, 'draft-'));
  const tempFile = path.join(draftDir, 'operations.json');
  fs.writeFileSync(tempFile, JSON.stringify([operation], null, 2));

  console.log(`💾 脱敏动作草稿已保存到: ${tempFile}`);
  console.log();
  console.log('═══════════════════════════════════════════════════\n');
  console.log('💡 下一步操作:\n');
