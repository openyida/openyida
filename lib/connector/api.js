...
async function saveConnector(params, authRef) {
  ...
  const bodyParams = {
    ...
    iconUrl: valueOr(params.iconUrl, 'chaxun%%#FFA200'),
    connectorDesc: valueOr(params.connectorDesc, ''),
    host: params.host,
    baseUrl: valueOr(params.baseUrl, '/'),
    scheme: valueOr(params.scheme, 'https'),
    tongxunluTemplateId: valueOr(params.tongxunluTemplateId, ''),
    faasTemplateId: valueOr(params.faasTemplateId, '0'),
    securitySchemes: valueOr(params.securitySchemes, '{}'),
    connectorMode: valueOr(params.connectorMode, String(HTTP_CONNECTOR_MODE)),
    connectorName: params.connectorName,
    category: valueOr(params.category, 'http'),
  };
  ...
  if (result.hasError || !result.success) {
    const error = new Error(result.errorMsg || result.message || t('connector_api.save_failed'));
    error.code = 'CONNECTOR_WRITE_REJECTED';
  ...
  const detailConnectorId = detail && (detail.id || detail.connectorId);

  return {
    ...result,
    connectorId: detailConnectorId || connectorId,
    connectorName,
    detail,
