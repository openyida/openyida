...
exports.getAuthRef = getAuthRef;
exports.printTable = printTable;
exports.buildConnectorDesc = buildConnectorDesc;
exports.buildOperationsSummary = buildOperationsSummary;
exports.connectorGet = connectorGet;
exports.connectorPost = connectorPost;
exports.listConnectors = listConnectors;
exports.findConnectorById = findConnectorById;
exports.getConnectorDetail = getConnectorDetail;
exports.saveConnector = saveConnector;
...
function buildConnectorDesc(userDesc, originalDesc, authRef, operations) {
    ...
    if (originalDesc) {
        ...
        if (createTimeMatch) {
            createTime = createTimeMatch[1].trim();
        }
        if (createUserMatch) {
            createUserId = createUserMatch[1].trim();
        }
    }
    const metaInfo = [
        '',
...
async function connectorGet(apiPath, authRef) {
    return (0, utils_1.requestWithAutoLogin)((ref) => (0, utils_1.httpGet)(ref.baseUrl, apiPath, null, ref.cookies), authRef);
}
/**
 * 发送 POST 请求到宜搭连接器 API（application/x-www-form-urlencoded）
 */
async function connectorPost(apiPath, bodyParams, authRef) {
    const postData = querystring_1.default.stringify(bodyParams);
    return (0, utils_1.requestWithAutoLogin)((ref) => (0, utils_1.httpPost)(ref.baseUrl, apiPath, postData, ref.cookies), authRef);
}
// ── 连接器列表 ────────────────────────────────────────
/**
 * 获取连接器列表
...
async function saveConnector(params, authRef) {
    ...
    const result = await connectorPost('/query/newconnector/createOrUpdateConnector.json?_api=ConnectorFactory.createOrUpdateConnector', bodyParams, authRef);
...
async function createConnection(params, authRef) {
    ...
    const result = await connectorPost('/query/newconnector/createConnection.json?_api=ConnectorFactory.createConnection', bodyParams, authRef);
    if (result.hasError || !result.success) {
...
async function testConnector(params, authRef) {
    ...
    const result = await connectorPost('/query/newconnector/testConnector.json?_api=ConnectorFactory.testConnector', bodyParams, authRef);
    return result;
}
