...
function buildConnectorTestPayload(params) {
  ...
  if (method !== 'get' && method !== 'delete') {
    // testOperation.json forwards payload.body verbatim as the outbound
    // request body, so it must be pre-serialized JSON; objects reach the
    // target API unserialized (HTTP 400 Invalidbody).
