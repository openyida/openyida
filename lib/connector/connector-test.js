...
async function run(args) {
  ...
  if (isYidaSystemTokenAction) {
    assertNoExplicitSystemToken(testParams, {
      connectorId: options.connectorId,
      actionId: options.actionId,
    });
    if (!options.systemTokenApp) {
      throw commandError(
        'YIDA_SYSTEM_TOKEN_RESOLUTION_REQUIRED',
        '该宜搭 OpenAPI 动作需要 --system-token-app，由 CLI 内部安全解析 systemToken。'
      );
    }
    const systemToken = await resolveYidaSystemToken(authRef, options.systemTokenApp);
    testParams.body = {
      ...(testParams.body && typeof testParams.body === 'object' && !Array.isArray(testParams.body)
        ? testParams.body
        : {}),
      systemToken,
    };
  } else if (options.systemTokenApp) {
    throw commandError(
      'YIDA_SYSTEM_TOKEN_BODY_INPUT_REQUIRED',
      '连接器动作没有声明 body.systemToken。'
    );
  }
  ...
  const result = redactSensitive({
    success: true,
    ...response,
    ...(isYidaSystemTokenAction
      ? { systemTokenResolved: true, systemTokenInjected: true, credentialExposed: false }
      : {}),
  });
  if (options.json) {
